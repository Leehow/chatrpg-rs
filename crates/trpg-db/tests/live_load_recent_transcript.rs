...
async fn recent_transcript_contains_prior_turn_content_in_chronological_order() {
    ...
    let transcript = db
        .load_recent_transcript(SESSION_TWO_TURNS, 12)
        .await
        .unwrap()
    ...
    assert!(
        transcript.contains("我推开门"),
        "transcript 必含第一回合玩家输入: {transcript}"
    );
    assert!(
        transcript.contains("门后是浓雾"),
        "transcript 必含第一回合 GM 叙事: {transcript}"
    );
    assert!(
        transcript.contains("我点亮提灯") && transcript.contains("火光照出走廊"),
        "transcript 必含第二回合: {transcript}"
    );
    ...
    assert!(
        ...
        "transcript 必按时间顺序（旧→新）: {transcript}"
    );

    sqlx::query("delete from turns where session_id=$1")
        .bind(SESSION_TWO_TURNS)
        .execute(&db.pool)
        .await
        .unwrap();
}
...
async fn empty_session_yields_none() {
    ...
    assert_eq!(
        db.load_recent_transcript(SESSION_EMPTY, 12).await.unwrap(),
        None
    );
}
...
async fn respects_turn_limit_keeping_most_recent() {
    ...
    let transcript = db
        .load_recent_transcript(SESSION_LIMIT, 2)
        .await
        .unwrap()
        .expect("必有 transcript");
    assert!(
        !transcript.contains("最旧输入"),
        "limit=2 必裁掉最旧回合: {transcript}"
    );
    assert!(
        transcript.contains("中间输入") && transcript.contains("最新输入"),
        "必保留最近 2 回合: {transcript}"
    );

...
fn ev(event_id: &str, turn_id: &str, kind: DomainEventKind, data: serde_json::Value) -> DomainEvent {
    DomainEvent {
...
async fn domain_events_append_list_idempotent_and_turn_filter() {
    ...
    // 无记录时 list 返空。
    assert!(db.list_domain_events(SESSION, 100).await.unwrap().is_empty(), "空会话 list 返空");

    let turn_a = "turn_eventlog_a";
    let turn_b = "turn_eventlog_b";
    let started = ev("de_turn_a_TurnStarted", turn_a, DomainEventKind::TurnStarted, json!({"input": "look"}));
    let finalized = ev("de_turn_a_TurnFinalized", turn_a, DomainEventKind::TurnFinalized, json!({"signal": "turn_complete"}));
    ...
    assert_eq!(listed[1], finalized, "seq 升序次条 = TurnFinalized（字节等价）");
    ...
    let scene = ev("de_turn_b_SceneTransitioned", turn_b, DomainEventKind::SceneTransitioned, json!({"from": "sc01", "to": "sc02"}));

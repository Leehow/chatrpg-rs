...
async fn mechanic_due_roundtrip_and_status_update() {
    ...
    let got = open
        .iter()
        .find(|d| d.due_id == due.due_id)
        .expect("list_open must contain the inserted due");
    ...
    assert_eq!(
        got.followup_procedure_id.as_deref(),
    ...
    db.update_mechanic_due_status(&due.due_id, "waived", Some("reason"), Some("scene"))
        .await
        .unwrap();
    let open_after = db.list_open_mechanic_dues(SESSION).await.unwrap();
    assert!(
        !open_after.iter().any(|d| d.due_id == due.due_id),
        "waived due must leave list_open"
    );
    let waived = db
        .list_mechanic_dues_with_status(SESSION, "waived")
        .await
        .unwrap();
    let w = waived
        .iter()
        .find(|d| d.due_id == due.due_id)
        .expect("list_mechanic_dues_with_status(waived) must contain it");
    ...
    let row = sqlx::query("select waive_reason, waive_scope from mechanic_dues where due_id=$1")
        ...
        .unwrap();
    assert_eq!(
        row.get::<Option<String>, _>("waive_reason").as_deref(),
        Some("reason"),
        "waive_reason must land in the row"
    );
    assert_eq!(
        row.get::<Option<String>, _>("waive_scope").as_deref(),
        Some("scene"),
        "waive_scope must land in the row"
    );
...
async fn reopen_scene_waived_dues_reopens_only_scene_scope() {
    ...
    .await
    .unwrap();
    db.update_mechanic_due_status(&turn_due.due_id, "waived", Some("narrative"), Some("turn"))
        .await
        .unwrap();
    assert!(
        db.list_open_mechanic_dues(REOPEN_SESSION)
            .await
            .unwrap()
            .is_empty(),
        "both waived → no open dues"
    );
    ...
    assert!(
        open.iter().any(|d| d.due_id == scene_due.due_id),
        "scene-waived due must be open again"
    );
    assert!(
        !open.iter().any(|d| d.due_id == turn_due.due_id),
        "turn-waived due must stay waived"
    );
    let waived = db
        .list_mechanic_dues_with_status(REOPEN_SESSION, "waived")
        .await
        .unwrap();
    assert!(
        waived.iter().any(|d| d.due_id == turn_due.due_id),
        "turn-waived due must remain in waived list"
    );
    // 重开后 waive 记账列清空（open 行不残留 scene 标记，避免未来查询误抑制）。
    let row = sqlx::query("select waive_reason, waive_scope from mechanic_dues where due_id=$1")
        .bind(&scene_due.due_id)
        .fetch_one(&db.pool)
        .await
        .unwrap();
    assert_eq!(
        row.get::<Option<String>, _>("waive_reason"),
        None,
        "reopened row must clear waive_reason"
    );
    assert_eq!(
        row.get::<Option<String>, _>("waive_scope"),
        None,
        "reopened row must clear waive_scope"
    );

...
async fn pp_lifecycle_streaming_critical_done_complete_roundtrip() {
    ...
    sqlx::query("delete from turns where session_id=$1").bind(SESSION_ROUNDTRIP).execute(&db.pool).await.unwrap();

    // 无 turn 时 load_last 返 None（fail-closed：守卫据此立即放行）。
    assert_eq!(db.load_last_turn_pp_lifecycle(SESSION_ROUNDTRIP).await.unwrap(), None);

    // save_turn 写入后默认 pp_lifecycle = streaming（迁移 default 值）。
    db.save_turn(SESSION_ROUNDTRIP, turn, "look around", "you see fog", json!({}), "ready").await.unwrap();
    assert_eq!(db.load_last_turn_pp_lifecycle(SESSION_ROUNDTRIP).await.unwrap().as_deref(), Some(PP_STREAMING));

    // critical 末：set → critical_done。
    db.set_turn_pp_lifecycle(turn, PP_CRITICAL_DONE).await.unwrap();
    assert_eq!(db.load_last_turn_pp_lifecycle(SESSION_ROUNDTRIP).await.unwrap().as_deref(), Some(PP_CRITICAL_DONE));

    // heavy 末：set → complete。
    db.set_turn_pp_lifecycle(turn, PP_COMPLETE).await.unwrap();
    assert_eq!(db.load_last_turn_pp_lifecycle(SESSION_ROUNDTRIP).await.unwrap().as_deref(), Some(PP_COMPLETE));

    // postprocess_status（ready）未被 pp_lifecycle 流转污染——两列正交。
    let st: (String,) = sqlx::query_as("select postprocess_status from turns where session_id=$1 and turn_id=$2")
        .bind(SESSION_ROUNDTRIP).bind(turn).fetch_one(&db.pool).await.unwrap();
...
async fn load_last_returns_most_recent_turn_by_created_at() {
    ...
    sqlx::query("delete from turns where session_id=$1").bind(SESSION_LOAD_LAST).execute(&db.pool).await.unwrap();

    db.save_turn(SESSION_LOAD_LAST, turn_a, "i", "o", json!({}), "ready").await.unwrap();
    db.set_turn_pp_lifecycle(turn_a, PP_COMPLETE).await.unwrap();
    // turn_b 后写 → created_at 更新 → load_last 必取 turn_b（仍 streaming）。
    db.save_turn(SESSION_LOAD_LAST, turn_b, "i2", "o2", json!({}), "ready").await.unwrap();
    assert_eq!(db.load_last_turn_pp_lifecycle(SESSION_LOAD_LAST).await.unwrap().as_deref(), Some(PP_STREAMING),

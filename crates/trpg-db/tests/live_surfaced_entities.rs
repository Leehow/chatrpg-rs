...
async fn list_surfaced_entities_returns_distinct_pairs() {
    ...
    // 空会话 → 空投影。
    assert!(db.list_surfaced_entities(SESSION).await.unwrap().is_empty(), "空会话投影返空");

    // append 2 个 EntitySurfaced（1 clue + 1 npc）。
    db.append_domain_event(&surfaced("de_surfaced_sess_truthgraph_t1_surfaced_clue_letter", "clue_letter", "clue")).await.unwrap();
    db.append_domain_event(&surfaced("de_surfaced_sess_truthgraph_t1_surfaced_npc_ras", "npc_ras", "npc")).await.unwrap();
    ...
    assert_eq!(
        got,
        vec![("clue_letter".to_string(), "clue".to_string()), ("npc_ras".to_string(), "npc".to_string())],

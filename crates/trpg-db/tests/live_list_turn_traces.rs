...
async fn list_turn_traces_returns_session_traces_in_order() {
    ...
    // 空会话 → 空 Vec。
    assert!(db.list_turn_traces(SESSION, 100).await.unwrap().is_empty(), "无 trace 时返回空 Vec");

    // 写两个回合的 trace（turns 行先建满足外键）。
    for (turn, signal, life) in [("t_cov_1", "turn_complete", "complete"), ("t_cov_2", "turn_failed", "failed")] {
        db.save_turn(SESSION, turn, "act", "narr", json!({}), "ready").await.unwrap();

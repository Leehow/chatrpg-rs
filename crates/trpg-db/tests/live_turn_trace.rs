...
async fn turn_trace_upsert_load_roundtrip_and_failure_kind() {
    ...
    sqlx::query("delete from turn_traces where session_id=$1").bind(SESSION).execute(&db.pool).await.unwrap();
    sqlx::query("delete from turns where session_id=$1").bind(SESSION).execute(&db.pool).await.unwrap();

    // 无记录时 load 返 None。
    assert_eq!(db.load_turn_trace(turn).await.unwrap(), None, "无 trace 时 load 返 None");

    // 写 turn 行（failure_kind 默认 NULL=成功）。
    db.save_turn(SESSION, turn, "look around", "you see fog", json!({}), "ready").await.unwrap();
    let fk: (Option<String>,) = sqlx::query_as("select failure_kind from turns where turn_id=$1")
        .bind(turn).fetch_one(&db.pool).await.unwrap();
    ...
    let loaded = db.load_turn_trace(turn).await.unwrap().expect("trace 应已落库");

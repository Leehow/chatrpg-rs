...
async fn dice_core_success_bands_override_heals_missing_critical() {
    ...
    let ids: Vec<String> = loaded
        ...
        .map(|a| {
            a.iter()
                .filter_map(|b| b.get("id").and_then(|x| x.as_str()).map(String::from))
                .collect()
        })
        .unwrap_or_default();
    ...
    assert!(
        ids.iter().any(|i| i == "critical"),
        "override must inject the missing `critical` band; got {ids:?}"
    );
    assert!(
        ids.iter().any(|i| i == "failure"),
        "override should replace the whole success_bands array; got {ids:?}"
    );
...
async fn dnd_kernel_has_valid_hp_track_after_override() {
    ...
    let kernel = match db.load_rule_kernel(DND).await.ok().flatten() {
        Some(k) => k,
        None => {
            eprintln!("SKIP: no {DND} kernel in this DB");
            return;
        }
    };
    ...
    if !ov.exists() {
        eprintln!(
            "SKIP: dnd5e override not found at {ov:?} (set TRPG_DATA_DIR to the D&D data dir)"
        );
        return;
    }
    assert!(
        kernel
            .resource_tracks
            .iter()
            .all(|t| t.get("field_id").is_none()),
        "sheet-field track survived normalize"
    );
    assert_eq!(
        trpg_model::hp_resource_track_id(&kernel.resource_tracks).as_deref(),
        Some("hit_points"),
        "D&D HP track not resolvable after override"
    );
}
...
async fn triangle_kernel_gains_commendation_demerit_economy_tracks_via_override() {
    ...
    let kernel = match db.load_rule_kernel("triangle_agency").await.ok().flatten() {
        Some(k) => k,
        None => {
            eprintln!("SKIP: no triangle_agency kernel in this DB");
            return;
        }
    };
    let dir = std::env::var("TRPG_DATA_DIR").unwrap_or_else(|_| "data".into());
    let ov =
        std::path::Path::new(&dir).join("parsed/rules/triangle_agency.rule_kernel.override.json");
    if !ov.exists() {
        eprintln!("SKIP: triangle override not found at {ov:?} (set TRPG_DATA_DIR to the Triangle data dir)");
        return;
    }
    let track = |id: &str| {
        kernel
    ...
    for (label, t) in [("commendations", comm), ("demerits", dem)] {
        assert_eq!(
            t.get("owner_kind").and_then(|v| v.as_str()),
            Some("actor"),
            "{label} must be a per-Agent track"
        );
        assert_eq!(
            t.get("initial").and_then(|v| v.as_i64()),
            Some(0),
            "{label} must seed at 0 so load_resource_current resolves without a sheet seed"
        );
    }
    assert!(
        dem.get("thresholds")
    ...
    assert!(
        track("chaos").is_some(),
        "override merge must not clobber the reader-extracted chaos track"
    );
}
...
async fn coc_kernel_gains_luck_and_magic_point_economy_tracks_via_override() {
    ...
    let kernel = match db
        .load_rule_kernel("call_of_cthulhu_7e")
        .await
        .ok()
        .flatten()
    {
        Some(k) => k,
        None => {
            eprintln!("SKIP: no call_of_cthulhu_7e kernel in this DB");
            return;
        }
    };
    let dir = std::env::var("TRPG_DATA_DIR").unwrap_or_else(|_| "data".into());
    let ov = std::path::Path::new(&dir)
        .join("parsed/rules/call_of_cthulhu_7e.rule_kernel.override.json");
    if !ov.exists() {
        eprintln!("SKIP: CoC override not found at {ov:?} (set TRPG_DATA_DIR to the CoC data dir)");
        return;
    }
    let track = |id: &str| {
        kernel
    ...
    assert_eq!(
        luck.get("max").and_then(|v| v.as_i64()),
        Some(99),
        "Luck may never exceed 99"
    );
    for (label, t) in [("luck", luck), ("mp", mp)] {
        assert!(
            t.get("on_outcome").is_none(),
            "{label} is a judgment-spend economy: an on_outcome would be a dead rule"
        );
        assert!(
            t.get("initial").is_none(),
            "{label} is per-investigator: no static initial (fail-closed sheet seed)"
        );
    }
    assert!(
        track("sanity").is_some() && track("hp").is_some(),
        "override merge must not clobber reader-extracted sanity/hp"
    );
}
...
async fn orc_kernel_gains_core_pools_via_override() {
    ...
    let kernel = match db.load_rule_kernel("brp_orc").await.ok().flatten() {
        Some(k) => k,
        None => {
            eprintln!("SKIP: no brp_orc kernel in this DB");
            return;
        }
    };
    let dir = std::env::var("TRPG_DATA_DIR").unwrap_or_else(|_| "data".into());
    ...
    if !ov.exists() {
        eprintln!("SKIP: BRP override not found at {ov:?} (set TRPG_DATA_DIR to the ORC data dir)");
        return;
    }
    assert_eq!(
        trpg_model::hp_resource_track_id(&kernel.resource_tracks).as_deref(),
    ...
    assert!(
        track("power_points").is_some(),
        "power_points spend economy missing after override"
    );
    for id in ["power_points", "fatigue_points", "sanity"] {
        let t = track(id).unwrap_or_else(|| panic!("{id} track missing after override"));
        assert!(
            t.get("on_outcome").is_none(),
            "{id} losses are GM adjudication: no on_outcome"
        );
    }
}
...
async fn dnd_kernel_gains_inspiration_economy_track_via_override() {
    ...
    assert_eq!(
        insp.get("initial").and_then(|v| v.as_i64()),
        Some(0),
        "inspiration starts unawarded"
    );
    assert_eq!(
        insp.get("max").and_then(|v| v.as_i64()),
        Some(1),
        "you either have inspiration or you don't — it does not stack"
    );
    assert!(
        insp.get("on_outcome").is_none(),
        "inspiration is a pure GM-judgment economy: no on_outcome"
    );
    assert_eq!(
        trpg_model::hp_resource_track_id(&kernel.resource_tracks).as_deref(),
        Some("hit_points"),
    ...
    assert!(
        track("xp").is_none() && track("experience_points").is_none(),
...
async fn cyberpunk_kernel_gains_luck_and_humanity_via_override() {
    ...
    let dir = std::env::var("TRPG_DATA_DIR").unwrap_or_else(|_| "data".into());
    let ov =
        std::path::Path::new(&dir).join("parsed/rules/cyberpunk_red.rule_kernel.override.json");
    if !ov.exists() {
        eprintln!("SKIP: cyberpunk override not found at {ov:?} (set TRPG_DATA_DIR to the cyberpunk data dir)");
        return;
    }
    let track = |id: &str| {
        kernel
    ...
    for (label, t) in [("luck", luck), ("humanity", hum)] {
        assert!(
            t.get("on_outcome").is_none(),
            "{label} spends/losses are GM adjudication: no on_outcome"
        );
        assert!(
            t.get("initial").is_none(),
            "{label} is per-character (LUCK stat / EMP x10): no static initial"
        );
    }
    assert!(
        hum.get("thresholds")
    ...
    assert!(
        trpg_model::hp_resource_track_id(&kernel.resource_tracks).is_some(),
        "override merge must not clobber the reader-extracted HP track"
    );
    assert!(
        track("improvement_points").is_none() && track("ip").is_none(),
...
async fn dnd_actor_resolves_real_hp_after_fix() {
    ...
    let kernel = match db.load_rule_kernel(DND).await.ok().flatten() {
        Some(k) => k,
        None => {
            eprintln!("SKIP: no {DND} kernel");
            return;
        }
    };
    let dir = std::env::var("TRPG_DATA_DIR").unwrap_or_else(|_| "data".into());
    if !std::path::Path::new(&dir)
        .join("parsed/rules/dnd5e.rule_kernel.override.json")
        .exists()
    {
        eprintln!("SKIP: dnd5e override not reachable (set TRPG_DATA_DIR)");
        return;
    }
    let hp_id = match trpg_model::hp_resource_track_id(&kernel.resource_tracks) {
        Some(id) => id,
        None => {
            eprintln!("SKIP: no HP track");
            return;
        }
    };
    ...
    sqlx::query("delete from runtime_actor_parameters where session_id=$1 and actor_id=$2")
        .bind(SESSION)
        .bind(ACTOR)
        .execute(&db.pool)
        .await

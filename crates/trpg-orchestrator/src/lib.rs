...
impl TurnOrchestrator {
    ...
    pub async fn reduce_turn(&self, input: TurnOrchestratorInput<'_>) -> Result<TurnOrchestrationResult> {
        ...
        let reconcile = lifecycle.reconcile_session(input.session_id).await.unwrap_or_default();
        let repairs = reconcile.repairs;
        let open_gate = self.db.get_open_interaction_gate(input.session_id).await.ok().flatten();
        let pending = self.db.get_open_pending_check(input.session_id).await.ok().flatten();
        let active_frame = self.db.list_active_state_frames(input.session_id, 1).await.ok().and_then(|mut v| v.pop());
        let semantic = SemanticIntentService::from_env(self.db.clone()).classify_turn(SemanticIntentRequest {
            ...
            active_frame_summary: active_frame.as_ref().map(|f| serde_json::to_value(f).unwrap_or_else(|_| json!({}))).unwrap_or_else(|| json!(null)),
            active_gate_summary: open_gate.as_ref().map(|g| serde_json::to_value(g).unwrap_or_else(|_| json!({}))).unwrap_or_else(|| json!(null)),
        ...
        if matches!(intent.gate_relation, GateRelation::SupersedingIntent | GateRelation::UnrelatedNewAction)
            && open_gate.is_some()
            && !route.should_resolve_gate_first
        {
            if let Some(gate) = open_gate.as_ref() {
                let reason = match route.route_kind {
                    TurnRouteKind::SupersedeGateThenStartFrame => SupersededReason::NewFrame.as_str(),
                    TurnRouteKind::SupersedeGateThenContinueFrame => SupersededReason::TerminalIntent.as_str(),
                    TurnRouteKind::SupersedeGateThenObjectInteraction => SupersededReason::TerminalIntent.as_str(),
                    _ => SupersededReason::TerminalIntent.as_str(),
                };
                lifecycle.supersede_gate_for_terminal_intent(gate, reason, input.user_input).await.ok();
                superseded_gate_id = Some(gate.gate_id.clone());
                if let Some(p) = pending.as_ref() {
                    self.db.supersede_pending_check(&p.check_id, reason, Some(time.world_tick)).await.ok();
...
fn classify_turn_intent(input: TurnOrchestratorInput<'_>, active_frame: Option<&StateFrame>, gate: Option<&InteractionGate>, pending: Option<&PendingCheck>, semantic: Option<&SemanticIntentResult>) -> TurnIntent {
    ...
    let semantic_sustained = semantic.map(semantic_is_sustained_combat_action).unwrap_or(false);
    ...
    let lexical_hypothetical_or_assessment = contains_any(&lower, &["能不能", "可不可以", "是否", "判断", "分析", "评估", "安全吗", "安全切断", "可安全", "can i", "could i", "is it safe", "assess", "analyze", "evaluate"]);
    let lexical_gm_secret_request = contains_any(&lower, &["gm 暗中", "gm暗中", "暗中处理", "暗投", "秘密处理", "不要提醒", "secretly", "secret roll", "gm secretly"]);
    let mentions_objectish = contains_any(&lower, &["线", "线缆", "电缆", "cable", "wire", "门", "锁", "设备", "无人机", "drone", "object", "device", "lock"]);
    let lexical_assessment_check = lexical_hypothetical_or_assessment && mentions_objectish && !lexical_gm_secret_request;
    let is_assessment_check = semantic_assessment || (allow_lexical_fallback && lexical_assessment_check);

    let lexical_attack_with_possessed_object = contains_any(&lower, &["夺来的", "抢来的", "刚夺", "刚抢", "picked up", "grabbed", "taken"]) && contains_any(&lower, &["开火", "射击", "攻击", "打", "shoot", "fire", "attack"]);
    let lexical_object_action = is_actual_object_action(&lower) && !lexical_hypothetical_or_assessment && !lexical_gm_secret_request && !lexical_attack_with_possessed_object;
    ...
    let possession_maneuver = contains_any(&lower, &["缴械", "打掉武器", "夺下武器", "夺刀", "夺枪", "下械", "disarm"]) ||
        (contains_any(&lower, &["夺", "抢", "抓", "grab", "snatch", "steal", "wrest", "pry"]) && contains_any(&lower, &["枪", "武器", "刀", "剑", "weapon", "gun", "pistol", "knife", "blade", "rifle"]));
    let is_object_action = (semantic_object_action || (allow_lexical_fallback && lexical_object_action)) && (!semantic_attack || possession_maneuver);

    let lexical_enemy_attack = contains_any(&lower, &["朝我开火", "向我开火", "攻击我", "堵住我", "开始开火", "场面进入战斗", "under attack", "attacks me", "opens fire", "shoots at me", "blocks me"]);
    ...
    let lexical_attack = contains_any(&lower, &["开枪", "开火", "射击", "射他", "射它", "攻击", "还击", "拔枪", "打它", "打他", "猛打", "补枪", "补一枪", "再补一枪", "再来一枪", "再开一枪", "继续开火", "继续攻击", "继续打", "扣扳机", "扣动扳机", "砍", "shoot", "fire", "attack", "return fire", "keep firing", "another shot", "press the attack", "撃", "攻撃"]);
    let is_attack = semantic_attack || (allow_lexical_fallback && lexical_attack);

    let lexical_continue = contains_any(&lower, &["继续", "接着", "还是", "不停", "持续", "keep", "continue", "press"]);
    let is_continue = semantic_sustained || (allow_lexical_fallback && lexical_continue) || (semantic_attack && active_frame.is_some());
    let is_director = contains_any(&lower, &["观察", "看看", "不知道", "能做什么", "整理", "已知", "线索", "recap", "what can", "observe", "look"]);
    ...
    let is_named_check = named_check_param.is_some() && !is_exit && !is_enemy_attack && !is_object_action && !is_attack;

    let mut gate_relation = if is_roll { GateRelation::RollReply }
        else if gate.is_none() { GateRelation::NoActiveGate }
        ...
        else if is_direction_gate(gate.unwrap()) && (is_continue || is_exit || is_attack) { GateRelation::SupersedingIntent }
        else if is_exit || is_object_action || is_enemy_attack || is_attack { GateRelation::SupersedingIntent }
        else if is_director || is_assessment_check || is_named_check { GateRelation::UnrelatedNewAction }
        else { GateRelation::Ambiguous };
    ...
    let (mut frame_relation, mut action_kind, mut target_kind, mut object_kind) = if is_exit {
        ...
        (FrameRelation::ExitAttempt, if lower.contains("投降") || lower.contains("surrender") { SituationActionKind::Surrender } else if lower.contains("和解") || lower.contains("停火") || lower.contains("truce") || lower.contains("ceasefire") { SituationActionKind::Negotiate } else { SituationActionKind::Flee }, "scene_or_opposition".into(), None)
    } else if is_named_check {
        evidence.push("explicit_named_check".into());
        named_parameter = named_check_param.clone();
        (if active_frame.is_some() { FrameRelation::InsideFrameAction } else { FrameRelation::OutsideFrameAction }, SituationActionKind::InvestigateDuringConflict, "named_check".into(), None)
    } else if is_assessment_check {
        evidence.push("technical_or_object_risk_assessment".into());
        (if active_frame.is_some() { FrameRelation::InsideFrameAction } else { FrameRelation::OutsideFrameAction }, SituationActionKind::InvestigateDuringConflict, "assessment_check".into(), None)
        ...
        (if active_frame.is_some() { FrameRelation::InsideFrameAction } else { FrameRelation::OutsideFrameAction }, SituationActionKind::EnemyInitiatedConflict, "opposition".into(), None)
    } else if is_object_action {
        evidence.push("object_interaction".into());
        (if active_frame.is_some() { FrameRelation::InsideFrameAction } else { FrameRelation::OutsideFrameAction }, SituationActionKind::UseItem, "object".into(), Some(infer_object_interaction_kind(&lower)))
    } else if is_attack {
        evidence.push("attack_or_continue_pressure".into());
        (if active_frame.is_some() { FrameRelation::InsideFrameAction } else { FrameRelation::OutsideFrameAction }, SituationActionKind::Attack, "opposition".into(), None)
    } else if is_director {
        evidence.push("director_guidance".into());
        (if active_frame.is_some() { FrameRelation::PauseAndObserve } else { FrameRelation::OutsideFrameAction }, SituationActionKind::AskQuestion, "scene".into(), None)
    } else {
        (if active_frame.is_some() { FrameRelation::InsideFrameAction } else { FrameRelation::OutsideFrameAction }, SituationActionKind::Unknown, "unknown".into(), None)
    };
    ...
    if let Some(sem) = semantic {
        ...
        let rust_has_frame_or_terminal = !evidence.is_empty() && (evidence.iter().any(|e| e == "exit_or_deescalation" || e == "enemy_initiated_conflict" || e == "attack_or_continue_pressure"));
        ...
        if sem.classifier == "semantic_unavailable_no_route" {
        ...
        } else if sem.confidence != RulingConfidence::Low || !sem.materialization_requests.is_empty() {
            if !rust_has_strong_object && !rust_has_assessment_check && !rust_has_named_check && !rust_has_frame_or_terminal && !is_roll {
                action_kind = sem.primary_action_kind;
                frame_relation = sem.frame_relation;
                gate_relation = parse_gate_relation(&sem.gate_relation).unwrap_or(gate_relation);
            }
            if sem.materialization_requests.iter().any(|r| r.target_kind == RuleBindingTargetKind::AbilityDefinition) && !rust_has_strong_object && !rust_has_assessment_check && !rust_has_named_check {
                target_kind = "ability".into();
                action_kind = if matches!(sem.primary_action_kind, SituationActionKind::ActivateAbility | SituationActionKind::TriggerAbility) { sem.primary_action_kind } else { SituationActionKind::ActivateAbility };
            }
            ...
            if sem.materialization_requests.iter().any(|r| r.target_kind == RuleBindingTargetKind::ObjectDefinition)
                && object_kind.is_none()
    ...
    TurnIntent {
        ...
        named_parameter,
        confidence: if evidence.is_empty() { RulingConfidence::Low } else { RulingConfidence::Medium },
        evidence_terms: evidence,
        created_at: Utc::now(),
    }
}

fn route_for(intent: &TurnIntent, active_frame: Option<&StateFrame>, gate: Option<&InteractionGate>, _pending: Option<&PendingCheck>) -> TurnRoute {
    let has_gate = gate.is_some();
    let frame_starting = active_frame.is_none() && matches!(intent.action_kind, SituationActionKind::Attack | SituationActionKind::UnderAttack | SituationActionKind::EnemyInitiatedConflict | SituationActionKind::SceneEntersConflict | SituationActionKind::Defend | SituationActionKind::Counterattack | SituationActionKind::Hack | SituationActionKind::DisableDevice | SituationActionKind::Rescue | SituationActionKind::CastOrUsePower);
    let frame_continuing = active_frame.is_some() && matches!(intent.action_kind, SituationActionKind::Attack | SituationActionKind::UnderAttack | SituationActionKind::EnemyInitiatedConflict | SituationActionKind::SceneEntersConflict | SituationActionKind::Defend | SituationActionKind::Counterattack | SituationActionKind::TakeCover | SituationActionKind::Move | SituationActionKind::Hack | SituationActionKind::DisableDevice | SituationActionKind::Rescue | SituationActionKind::UseItem | SituationActionKind::InvestigateDuringConflict);
    let object = intent.object_interaction_kind.is_some();
    let ability = matches!(intent.action_kind, SituationActionKind::ActivateAbility | SituationActionKind::TriggerAbility) || intent.target_kind == "ability";
    let advisory_direction_gate = gate.map(is_direction_gate).unwrap_or(false);
    let required_gate = gate.map(|g| g.required || matches!(g.gate_kind, GateKind::RequiredReactionChoice | GateKind::PlayerRollRequired)).unwrap_or(false) && !advisory_direction_gate;
    let terminal = is_terminal_or_exit_intent(intent);
    let assessment_check = intent.target_kind == "assessment_check" || intent.target_kind == "named_check";
    ...
    if advisory_direction_gate && frame_continuing && !object {
        return route(TurnRouteKind::SupersedeGateThenContinueFrame, false, true, false, false, false, "advisory direction/stalemate gate absorbed by clear semantic frame action");
    }
    if advisory_direction_gate && terminal && active_frame.is_some() {
        return route(TurnRouteKind::ContinueFrameFirst, false, true, false, false, false, "terminal/exit intent bypasses advisory direction gate");
    }

    if matches!(intent.gate_relation, GateRelation::RollReply | GateRelation::ChoiceReply) {
        return route(TurnRouteKind::ResolveGateFirst, true, false, false, false, false, "input is a direct hard-gate reply or roll reply");
    }
    ...
    if ability && !required_gate {
        return TurnRoute { route_kind: TurnRouteKind::AbilityActivationFirst, should_resolve_gate_first: false, prefer_conflict_first: false, prefer_object_first: false, prefer_ability_first: true, force_director: false, allow_agent_plan: true, reason: "semantic ability activation/materialization route".into() };
    }
    if assessment_check {
    ...
    if has_gate && frame_starting {
        return route(TurnRouteKind::SupersedeGateThenStartFrame, false, true, false, false, false, "new frame-worthy combat intent supersedes an old non-required gate");
    }
    ...
    if has_gate && frame_continuing && !object {
        return route(TurnRouteKind::SupersedeGateThenContinueFrame, false, true, false, false, false, "frame action supersedes an unrelated non-required gate");
    }
    if has_gate && object {
        return route(TurnRouteKind::SupersedeGateThenObjectInteraction, false, false, true, false, false, "object interaction supersedes an unrelated non-required gate");
    }
    if terminal && active_frame.is_some() {
        return route(TurnRouteKind::ContinueFrameFirst, false, true, false, false, false, "terminal/exit intent belongs to the active frame reducer");
    }
    ...
    if object {
        return route(TurnRouteKind::ObjectInteractionFirst, false, false, true, false, false, if active_frame.is_some() { "object interaction inside an active frame is attached to the frame as a child action" } else { "object interaction route selected by deterministic route invariant" });
    }
    if matches!(intent.action_kind, SituationActionKind::AskQuestion) {
        return route(TurnRouteKind::DirectorFirst, false, false, false, true, true, "player is asking for scene guidance or observation");
    }
    route(TurnRouteKind::StandaloneCheckOrAgent, false, false, false, false, true, "no frame-worthy or object route detected")
}

fn route(kind: TurnRouteKind, resolve_gate: bool, conflict: bool, object: bool, director: bool, agent: bool, reason: &str) -> TurnRoute {
    TurnRoute { route_kind: kind, should_resolve_gate_first: resolve_gate, prefer_conflict_first: conflict, prefer_object_first: object, prefer_ability_first: false, force_director: director, allow_agent_plan: agent, reason: reason.into() }
}

fn semantic_object_interaction_hint(sem: &SemanticIntentResult) -> Option<String> {
    sem.raw_json.get("object_interaction_kind").and_then(|v| v.as_str()).map(str::to_string)
        .or_else(|| {
            let text = serde_json::to_string(&sem.raw_json).unwrap_or_default().to_lowercase();
            if text.contains("disarm") { Some("disarm".into()) }
            else if text.contains("grab") || text.contains("snatch") { Some("grab_held_object".into()) }
            else if text.contains("pick_up") || text.contains("pickup") { Some("pick_up".into()) }
            else if text.contains("cut") && (text.contains("cable") || text.contains("wire")) { Some("cut_connection".into()) }
            else { None }
        })
}

fn is_terminal_or_exit_intent(intent: &TurnIntent) -> bool {
    intent.evidence_terms.iter().any(|e| e == "exit_or_deescalation")
        || matches!(intent.frame_relation, FrameRelation::ExitAttempt | FrameRelation::DeescalationAttempt | FrameRelation::Surrender | FrameRelation::Flee | FrameRelation::HideToDisengage)
        || matches!(intent.action_kind, SituationActionKind::Surrender | SituationActionKind::Flee | SituationActionKind::Negotiate | SituationActionKind::EndConflict | SituationActionKind::LeaveScene)
}

fn looks_like_roll(text: &str) -> bool {
    let t = text.trim();
    t.starts_with("/roll") || t.parse::<i32>().is_ok() || t.contains("1d") || t.contains("d4") || t.contains("d6") || t.contains("d8") || t.contains("d10") || t.contains("d12") || t.contains("d20") || t.contains("d100") || t.contains("掷") || t.contains("骰") || t.contains("总计") || t.contains("合计")
}
...
fn semantic_is_confident(sem: &SemanticIntentResult) -> bool {
    matches!(sem.confidence, RulingConfidence::High | RulingConfidence::Medium)
}

fn semantic_route_bool(sem: &SemanticIntentResult, key: &str) -> bool {
    sem.raw_json.get("route_cues")
...
fn semantic_is_combat_action(sem: &SemanticIntentResult) -> bool {
    semantic_route_bool(sem, "combat_action")
        || matches!(sem.primary_action_kind,
...
fn semantic_is_sustained_combat_action(sem: &SemanticIntentResult) -> bool {
    semantic_route_bool(sem, "sustained_combat_action")
        || matches!(sem.primary_action_kind, SituationActionKind::Attack | SituationActionKind::Counterattack)
}
...
fn semantic_is_object_interaction(sem: &SemanticIntentResult) -> bool {
    semantic_route_bool(sem, "object_interaction")
        || sem.raw_json.get("object_interaction_kind")
...
fn semantic_named_check_parameter(sem: &SemanticIntentResult) -> Option<String> {
    let reqs = sem.raw_json.get("materialization_requests").and_then(|v| v.as_array())?;
...
fn semantic_is_assessment_check(sem: &SemanticIntentResult) -> bool {
    semantic_route_bool(sem, "assessment_check")
        || sem.raw_json.get("primary_action_kind")
            .and_then(serde_json::Value::as_str)
...
fn is_direction_gate(gate: &InteractionGate) -> bool {
    matches!(gate.gate_kind, GateKind::ChooseActionMode)
        || gate.advice_refs.iter().any(|r| r.contains("direction") || r.contains("stalemate"))
        || gate.prompt_public.contains("改变局面")
        || gate.prompt_public.contains("局势")
}

fn is_actual_object_action(text: &str) -> bool {
    contains_any(text, &["缴械", "打掉武器", "夺下武器", "抢", "夺", "抓", "捡", "拾起", "拿起", "丢", "扔下", "放下", "装备", "穿上", "戴上", "换弹", "装弹", "剪线", "剪断", "切断", "撬锁", "开锁", "搜身", "搜尸", "翻找", "disarm", "grab", "snatch", "steal", "pickup", "pick up", "drop", "equip", "reload", "cut cable", "cut wire", "unlock", "lockpick", "loot", "frisk"])
}

fn infer_object_interaction_kind(text: &str) -> String {
    if contains_any(text, &["缴械", "disarm"]) { "disarm".into() }
    else if contains_any(text, &["抢", "夺", "grab", "steal"]) { "grab_held_object".into() }
    else if contains_any(text, &["捡", "拾", "pickup", "pick up"]) { "pick_up".into() }
    else if contains_any(text, &["剪", "切断", "cable", "cut"]) { "cut_connection".into() }
    else if contains_any(text, &["装备", "穿", "equip"]) { "equip".into() }
    else if contains_any(text, &["换弹", "reload"]) { "reload".into() }
    else if contains_any(text, &["开锁", "撬锁", "unlock"]) { "unlock".into() }
    else { "use".into() }
}
...
mod named_check_tests {
    ...
    fn sem(raw: serde_json::Value) -> SemanticIntentResult {
        SemanticIntentResult {
            semantic_id: "s".into(), session_id: "sess".into(), turn_id: "t".into(), ruleset_id: "call_of_cthulhu_7e".into(),
            primary_action_kind: SituationActionKind::Unknown, gate_relation: "no_active_gate".into(),
            frame_relation: FrameRelation::OutsideFrameAction, target_refs: vec![], object_refs: vec![], ability_refs: vec![],
            materialization_requests: vec![], secrecy_policy: "gm_only".into(), confidence: RulingConfidence::Medium,
            classifier: "test".into(), rationale_brief: None, raw_json: raw, created_at: Utc::now(),
        }
    }
    fn input<'a>() -> TurnOrchestratorInput<'a> {
        TurnOrchestratorInput { session_id: "sess", turn_id: "t", ruleset_id: "call_of_cthulhu_7e", module_id: None, user_input: "I make a Sanity roll" }
    }
    ...
    fn prefers_actor_parameter_label() {
        ...
        assert_eq!(semantic_named_check_parameter(&s).as_deref(), Some("Sanity"));
    }
    ...
    fn falls_back_to_check_contract_label_verbatim() {
        let s = sem(json!({"materialization_requests":[{"target_kind":"check_contract","target_label":"Spot Hidden"}]}));
        assert_eq!(semantic_named_check_parameter(&s).as_deref(), Some("Spot Hidden"));
    }
    ...
    fn uses_cjk_actor_parameter_label_verbatim() {
        let s = sem(json!({"materialization_requests":[{"target_kind":"actor_parameter","target_label":"理智"}]}));
    ...
    fn no_param_request_is_not_named_check() {
        let s = sem(json!({"materialization_requests":[{"target_kind":"object_definition","target_label":"Lever"}]}));

...
impl NeedSignal {
    ...
    pub const NONE: NeedSignal = NeedSignal {
        needs_rule: false,
    ...
    pub fn any(&self) -> bool {
        self.needs_rule || self.needs_material || self.needs_scene
    }
...
impl SemanticNeedClassifier {
    ...
    pub fn classify(sem: &SemanticIntentResult) -> NeedSignal {
        ...
        let needs_rule = mats.iter().any(|m| {
            matches!(
                m.target_kind,
        ...
        let needs_material = mats.iter().any(|m| {
            matches!(
                m.target_kind,
...
mod tests {
    ...
    fn combat_action_needs_rule() {
        ...
        assert!(
            SemanticNeedClassifier::classify(&s).needs_rule,
            "attack should need a rule"
        );
    }
    ...
    fn object_definition_request_needs_material() {
        ...
        assert!(
            SemanticNeedClassifier::classify(&s).needs_material,
            "object definition should need material"
        );
    }
    ...
    fn object_interaction_cue_needs_material() {
        let s = result(
            ...
            RulingConfidence::Medium,
            "llm",
            json!({"route_cues":{"object_interaction":true}}),
        );
    ...
    fn confidence_maps_from_semantic_confidence() {
        let high = SemanticNeedClassifier::classify(&result(
            SituationActionKind::Attack,
            RulingConfidence::High,
            "llm",
            json!({}),
        ));
        let medium = SemanticNeedClassifier::classify(&result(
            SituationActionKind::Attack,
            RulingConfidence::Medium,
            "llm",
            json!({}),
        ));
        let low = SemanticNeedClassifier::classify(&result(
            SituationActionKind::Attack,
        ...
        assert!(
            ...
            "monotone High>Medium>Low"
        );
        assert!(
            high.is_confident(0.7) && !medium.is_confident(0.7),
            "0.7 threshold splits High from Medium"
        );
        assert!(
            medium.is_confident(0.5) && !low.is_confident(0.5),
            "0.5 threshold splits Medium from Low"
        );
    }

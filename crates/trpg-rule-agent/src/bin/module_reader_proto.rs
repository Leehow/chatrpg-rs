...
async fn main() -> Result<()> {
    ...
    let units_path = PathBuf::from(
        args.get(1)
            .expect("usage: module_reader_proto <units.jsonl> [sidecar.md] [budget] [ruleset]"),
    );
    ...
    eprintln!(
        "⏱  MINIMAL PLAYABLE UNIT PARSE: {:.2}s (wall-clock, Pass A + Pass B)",
        total.as_secs_f64()
    );
    eprintln!(
    ...
    for v in readout
        ...
        .iter()
        .chain(&readout.clues)
        .chain(&readout.locations)
        .chain(&readout.encounters)
        .chain(&readout.handouts)
    {
    ...
    let bp2 = readout
        ...
        .iter()
        .filter(|v| class_of(v) == "bp2_custom_rule")
        .count();
        ...
        .find(|s| s.extraction_status == SceneExtractionStatus::DeepExtracted)
    {
        ...
        eprintln!(
            ...
            entry
                .links
                .iter()
                .map(|l| format!("{}({:?})", l.to_node_id, l.link_type))
                .collect::<Vec<_>>()
        );
    } else {

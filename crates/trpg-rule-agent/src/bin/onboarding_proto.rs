...
async fn main() -> Result<()> {
    ...
    let units = load_units(&PathBuf::from(
        args.get(1).expect("usage: onboarding_proto <units.jsonl> <merged.md> [budget] [role_field] [skills_csv]"),
    ))?;
    ...
    let ctx = OnboardingCtx { units: &units, sidecar_text: sidecar, role_field, skill_fields, option_catalogs: serde_json::json!([]) };

...
pub async fn deep_extract_scene_in_place(
    ...
    let pre_existing: HashSet<String> = readout.scenes[idx]
        .links
        .iter()
...
再补闭包实体详情，最后 **必须调用 submit_deep** 提交。\n\n=== 页面文本 ===\n{page_text}",
    ...
    let resp = client
        .complete_with_tools(msgs, vec![submit_deep])
        .await
        .ok()?;
    let tcs = resp
...
pub(super) async fn run_gleaning_rounds(
    ...
    for _ in 0..max_gleanings {
        ...
        let Some(glean) = run_module_loop(
            client,
            GLEAN_SYS,
            &seed,
            ctx,
...
pub async fn complete_skeleton_stubs(
    ...
    for stub in &stubs {
        let Some(id) = stub.get("node_id").and_then(Value::as_str) else {
            continue;
        };

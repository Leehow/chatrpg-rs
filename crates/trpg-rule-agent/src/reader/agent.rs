...
Since you already read the rules pages, ALSO fill the machine-readable `core` in the same pass (it feeds the deterministic engine): core.dice, core.direction (roll_high/roll_under/pool_count), core.compare_to, core.success_rule, AND the TYPED success operator so the engine resolves WITHOUT re-reading prose: core.compare (meet_or_beat|roll_under|count_faces) plus the numbers it needs — for count_faces give core.target_face (which die face counts, e.g. 3) and core.success_threshold (how many needed, e.g. 1); for a FIXED meet_or_beat/roll_under target give core.target_number (omit when target is per-skill or a DV table). Examples: Triangle 6d4-count-3s-on-1+ => {dice:"6d4",direction:"pool_count",compare:"count_faces",target_face:3,success_threshold:1}; CoC d100-under-skill => {dice:"1d100",direction:"roll_under",compare:"roll_under"} (no target_number, it's the skill). If the game has graded success LEVELS (not just pass/fail), ALSO fill core.success_bands = [{id,label,rank(int,higher=better),test:{kind, ...}}] so the engine reports the tier: test.kind ∈ roll_under_or_equal | roll_under_fraction(denominator) | meet_or_beat_fraction(numerator,denominator) | exact(value) | in_range(min,max) | otherwise, with optional guard when/unless {target_lt|target_gte}. CoC example: [{id:"critical",label:"critical",rank:5,test:{kind:"exact",value:1}},{id:"extreme",rank:4,test:{kind:"roll_under_fraction",denominator:5}},{id:"hard",rank:3,test:{kind:"roll_under_fraction",denominator:2}},{id:"regular",rank:2,test:{kind:"roll_under_or_equal"}},{id:"fumble",rank:0,test:{kind:"in_range",min:96,max:100,when:{target_lt:50}}},{id:"fumble",rank:0,test:{kind:"in_range",min:100,max:100,when:{target_gte:50}}},{id:"failure",rank:1,test:{kind:"otherwise"}}]. Omit success_bands entirely for pure pass/fail games. Also fill core.resource_tracks for EVERY persistent numeric track the rules keep score with — survival/state meters (HP/Sanity/Chaos/Humanity/...) AND reward/discipline/advancement economies the rules accrue or spend (XP-style points, merit/demerit currencies, standing ladders) — WITH the engine-update rule so they change without hardcoding: each = {id, name, owner_kind (actor|scene), initial, max, zero_means, on_outcome:[{trigger, op, amount, when?, mitigation?}], thresholds:[{at|loss_in_one_go, consequence}]}. on_outcome.trigger = on_success|on_failure|always; amount = a dice expr like "1d6" (rolled), or "=value" (reads the `when` outcome field), or "=field" (reads that outcome field directly), or a fixed int. The ONLY outcome fields that exist (legal for `when` and after "="): total, target, success, success_count, pool_miss_count, success_tier_rank — written bare ("=total", never "=<total>"). NEVER reference a derived_formulas field_id or any invented name ("=damage", "=damage_after_armor" read NOTHING — the rule is dead and gets dropped). An amount that comes from a SEPARATE roll (e.g. weapon damage) is not expressible here: OMIT the on_outcome rule, the combat/effect path applies it. Examples — Triangle Chaos: {id:"chaos", owner_kind:"scene", initial:0, max:12, on_outcome:[{trigger:"always", when:"pool_miss_count", op:"add", amount:"=value"}], thresholds:[{at:12, consequence:"Chaos overflows; anomaly escalates"}]}. CoC Sanity: {id:"sanity", owner_kind:"actor", initial:50, max:99, on_outcome:[{trigger:"on_failure", op:"subtract", amount:"1d6"}], thresholds:[{loss_in_one_go:5, consequence:"temporary insanity"},{at:0, direction:"at_or_below", consequence:"permanent insanity"}]}. Cyberpunk HP: {id:"hp", owner_kind:"actor", zero_means:"mortally wounded"} with NO on_outcome — weapon damage is a separate roll, not a check-outcome field (do NOT invent amount:"=damage_after_armor"). A GM-judgment economy (a merit/demerit-style currency awarded for behavior "as determined by the GM") is likewise a track WITHOUT on_outcome — still list it (id/name/owner_kind/initial/thresholds) so the engine can address it; the GM applies the awards as effects. Also fill core.derived_formulas — executable formulas grounded in the pages, engine shape {field_id, formula, depends_on[], evaluator, notes}. Examples: ranged_attack_roll formula "1d10 + REF + ranged_weapon_skill"; skill_check formula "1d100 (succeed if <= skill)"; damage_after_armor formula "max(0, weapon_damage - SP)". derived_formulas field_ids are a SEPARATE namespace from check-outcome fields — never usable as on_outcome amounts. Only include formulas you can ground in a page you read; omit what the game lacks (e.g. a no-combat game has no attack/damage formulas).
...
mod tests {
    #[test]
    fn reader_prompt_lists_the_engine_outcome_vocabulary() {
        // The SYSTEM on_outcome docs must enumerate exactly the legal `=field`

...
impl FacilClient {
    fn submitting(args: Value) -> Self {
        Self {
            tool_name: "submit_facilitation".into(),
            args,
...
async fn extracts_facts_from_entry_scene() {
    ...
    let cfg = extract_facilitation_facts(&client, &out, 0)
        .await
        .expect("应抽出非空引导事实");
    ...
    assert!(
        client.seen_user.lock().unwrap().contains("门半开着"),
        "入口 read_aloud 应进 prompt"
    );
}
...
async fn no_tool_call_is_fail_closed_none() {
    ...
    let out = readout_with_entry();
    assert!(
        extract_facilitation_facts(&client, &out, 0).await.is_none(),
        "没调工具→None"
    );
}

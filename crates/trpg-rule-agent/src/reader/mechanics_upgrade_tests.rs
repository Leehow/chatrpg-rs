...
fn thresholds_upgrade_broken_ref_skipped() {
    ...
    assert!(
        msgs.iter().any(|m| m.code == "threshold_followup_broken"),
        "msgs: {msgs:?}"
    );
}
...
fn thresholds_upgrade_no_match_no_create() {
    ...
    assert_eq!(
        msgs.iter()
            .filter(|m| m.code == "threshold_upgrade_unmatched")
            .count(),
        2,
        "msgs: {msgs:?}"
    );
...
fn bands_upgrade_fills_semantics_not_overwrite() {
    ...
    assert_eq!(
        band.get("semantics").and_then(Value::as_str),
        Some("S"),
        "missing key filled"
    );
    assert_eq!(
        band.get("label").and_then(Value::as_str),
        Some("L"),
        "existing key never overwritten"
    );
}
...
fn bands_upgrade_appends_missing_band_with_report() {
    let mut k = kernel_fixture();
    let fumble =
        json!({"id":"fumble","label":"Fumble","semantics":"catastrophic backfire","rank":0});
    ...
    assert!(
        msgs.iter()
            .any(|m| m.code == "success_band_completed" && m.target.as_deref() == Some("fumble")),
        "msgs: {msgs:?}"
    );
}
...
fn field_notes_only_fill_empty() {
    ...
    assert_eq!(
        fields[1].get("notes").and_then(Value::as_str),
        Some("luck note"),
        "missing notes filled"
    );
    assert_eq!(
        fields[2].get("notes").and_then(Value::as_str),
        Some("mp note"),
        "empty-string notes filled"
    );
    assert_eq!(fields.len(), 3, "no field is ever created");
    assert!(
        msgs.iter()
            .any(|m| m.code == "field_note_unmatched" && m.target.as_deref() == Some("ghost")),
        "msgs: {msgs:?}"
    );
}
...
fn kernel_round_trips_after_all_upgrades() {
    ...
    assert!(
        kernel_round_trip_guard(&k).is_ok(),
        "guard passes on an upgraded kernel"
    );

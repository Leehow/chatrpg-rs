...
fn ent_set<'a>(n: &'a ScenarioNode) -> HashSet<&'a str> {
    n.referenced_npc_ids
        .iter()
        .chain(&n.referenced_clue_ids)
        .chain(&n.referenced_location_ids)
        .chain(&n.referenced_encounter_ids)
        .map(|s| s.as_str())
        .collect()
}
...
fn unit_title_keys_match(u: &Unit, scene_key: &str) -> bool {
    let cand =
        std::iter::once(u.title.as_str()).chain(u.heading_context.iter().map(|s| s.as_str()));
    for c in cand {
...
pub fn dedup_entities_by_key(
    entities: &mut Vec<serde_json::Value>,
    scenes: &mut [ScenarioNode],
) -> usize {
    ...
    *entities = keep;
    if !remap.is_empty() {
        let rewrite = |v: &mut Vec<String>| {
            for x in v.iter_mut() {
                if let Some(c) = remap.get(x) {
                    *x = c.clone();
                }
            }
        };
...
mod tests {
    ...
    fn dedup_merges_by_key_and_rewrites_refs() {
        ...
        let mut scenes = vec![{
            let mut n = ScenarioNode::default();
            n.node_id = "s1".into();
            n.referenced_npc_ids = vec!["npc_brenner2".into()];
            n
        }];

...
fn role_of(dv: &Value) -> &str {
    dv.get("role").and_then(Value::as_str).unwrap_or("")
}
...
fn is_resource_role(role: &str) -> bool {
    role == "resource" || role == "resource_max"
}
...
pub fn aligned_track_id(dv_id: &str, role: &str, tracks: &[Value]) -> Option<String> {
    ...
    if let Some(t) = tracks.iter().find(|t| {
        t.get("derived_from")
            .and_then(Value::as_str)
            .map(|s| s.eq_ignore_ascii_case(dv_id))
            .unwrap_or(false)
    }) {
    ...
    if base == "hp" || base.starts_with("hit_point") {
        if let Some(id) = trpg_model::hp_resource_track_id(tracks) {
            return Some(id);
        }
    }
...
pub fn audit_alignment(derived_values: &[Value], resource_tracks: &[Value]) -> AlignmentReport {
    ...
    for dv in derived_values {
        ...
        match aligned_track_id(&dv_id, role, resource_tracks) {
            Some(tid) => {
                ...
                {
                    if !track_has_behavior(t) && !r.behavior_gaps.contains(&tid) {
                        r.behavior_gaps.push(tid.clone());
                    }
                }
    ...
    for t in resource_tracks {
        if let Some(tid) = id_of(t) {
            if !matched_tracks.iter().any(|m| m.eq_ignore_ascii_case(&tid)) {
                r.orphan_tracks.push(tid);
            }
        }
    }
    r
...
pub fn validate_emitted_on_outcome(rules: &[Value]) -> Vec<Value> {
    rules
        .iter()
        .filter(|r| match r.get("amount").and_then(Value::as_str) {
...
pub async fn fill_behavior_from_prose(
    ...
    for track_id in &report.behavior_gaps {
        ...
        if let Some(submitted) = run_behavior_loop(client, &seed, &ctx, budget).await {
            let oo: Vec<Value> = submitted
                .get("on_outcome")
                .and_then(Value::as_array)
                .cloned()
                .unwrap_or_default();
            let th: Vec<Value> = submitted
                .get("thresholds")
                .and_then(Value::as_array)
                .cloned()
                .unwrap_or_default();
...
mod tests {
    ...
    fn fail_closed_orphan_formula_and_behavior_gap() {
        ...
        assert_eq!(
            r.behavior_gaps,
            vec!["hit_points".to_string()],
            "hit_points 缺行为应记 gap: {r:?}"
        );
    }
    ...
    fn align_stamps_derived_from_on_existing_track() {
        let dvs = vec![json!({"id":"hp_max","role":"resource_max"})];
        let mut tracks =
            vec![json!({"id":"hit_points","kind":"health","on_outcome":[{"op":"subtract"}]})];
        let report = align(&dvs, &mut tracks);
        assert_eq!(
            tracks[0].get("derived_from").and_then(Value::as_str),
            Some("hp_max"),
        ...
        assert_eq!(
            report.aligned,
            vec![("hp_max".to_string(), "hit_points".to_string())]
        );
    }
    ...
    fn align_creates_stub_track_without_fabricating_behavior() {
        ...
        assert!(
            tracks[0].get("on_outcome").is_none() && tracks[0].get("thresholds").is_none(),
            "fail-closed：无源不臆造行为"
        );
    ...
    fn apply_emitted_behavior_never_overwrites_existing() {
        let mut t_existing =
            json!({"id":"sanity","on_outcome":[{"op":"subtract","amount":"=total"}]});
        apply_emitted_behavior(&mut t_existing, &[json!({"op":"add"})], &[]);
        assert_eq!(
            t_existing["on_outcome"].as_array().unwrap().len(),
            1,
            "既有行为不覆盖"
        );

        let mut t_empty = json!({"id":"luck","derived_from":"luck"});
        apply_emitted_behavior(
            &mut t_empty,
            &[json!({"op":"subtract","amount":"=total"})],
    ...
    fn coc_override_shape_aligns_hp_and_mp() {
        ...
        assert!(
            r.orphan_formulas.is_empty() && r.orphan_tracks.is_empty(),
            "{r:?}"
        );
    ...
    fn dnd_hp_aligns_by_base_id_without_data_change() {
        ...
        assert_eq!(
            r.aligned,
            vec![("hit_points_max".to_string(), "hit_points".to_string())],
            "{r:?}"
        );

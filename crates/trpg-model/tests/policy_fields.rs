...
fn combat_profile_round_trips() {
    let p = CombatProfile {
        ...
        reaction_windows: vec![
            serde_json::json!({"advice_id":"generic.required_defense_choice.v1_3"}),
        ],
...
fn mode_policy_and_module_config_round_trip() {
    let mp = CombatModePolicy {
        rules: vec![CombatModeRule {
            mode: CombatMode::HorrorEncounter,
            when_action_kinds: vec![],
            when_evidence_contains: vec![],
        }],
        fallback_mode: CombatMode::TheaterOfMind,
    };
    let back: CombatModePolicy =
        serde_json::from_str(&serde_json::to_string(&mp).unwrap()).unwrap();
    assert_eq!(mp, back);

    let mc = ModuleConfig {
        npc_actor_bindings: vec![NpcActorBinding {
            matcher: vec!["drone".into(), "无人机".into()],
            actor_id: "npc.athena_drone".into(),
            display_name: Some("rogue drone".into()),
        }],
        technical_option_table: Some(vec![TechOption {
            matcher: vec!["cable".into()],
            dv: 14,
        }]),
        scene_entity_aliases: vec![EntityAlias {
            canonical_id: "athena".into(),
            aliases: vec!["雅典娜".into()],
        }],
        module_search_profile: Some(SearchProfile {
            preferred_sections_by_skill: [(
                "npc_stat_block".to_string(),
                vec!["Redesigned NPC cards".into()],
            )]
            .into(),
        }),
...
fn generic_defaults_match_legacy_generic_branch() {
    ...
    assert_eq!(
        GENERIC_REFEREE_BANDS.damage_band,
        "system-specific; exact object/ability entry required"
    );
    assert_eq!(GENERIC_REFEREE_BANDS.damage_plausible_range, (1, 30));
    assert_eq!(
        GENERIC_REFEREE_BANDS.difficulty_band,
        serde_json::json!({"common_target_band":"ruleset-specific"})
    );
    ...
    assert_eq!(
        GENERIC_COMBAT_PROFILE.action_economy,
        serde_json::json!({"policy":"fiction_first"})
    );
    ...
    assert_eq!(
        GENERIC_COMBAT_MODE_POLICY.fallback_mode,

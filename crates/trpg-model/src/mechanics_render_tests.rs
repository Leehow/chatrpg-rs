...
fn kernel_bp1_view_strips_catalog_only() {
    ...
    assert!(
        view.mechanics_catalog.is_empty(),
        "view must strip mechanics_catalog"
    );
...
fn catalog_index_text_is_deterministic_one_line_per_entry() {
    ...
    for (line, e) in lines.iter().zip(&entries) {
        ...
        assert_eq!(
            segments.len(),
            3,
            "line must have `id | name | when_to_use` segments: {line}"
        );
...
fn catalog_index_over_limit_keeps_procedures_hooks_and_pm() {
    ...
    for i in 0..=limit {
        ...
        assert!(
            !text.contains(&id),
            "over-limit long-tail SkillCheck `{id}` must be omitted"
        );
    }
    assert!(
        text.contains("m.subsystem"),
        "SubsystemProcedure entry must be kept"
    );
    assert!(text.contains("m.hooked"), "entry with hooks must be kept");
    assert!(
        text.contains("m.passive"),
        "entry with passive_projection must be kept"
    );
    let tail = text.lines().last().unwrap();
    assert!(
        tail.contains("lookup_mechanic"),
        "tail line must mention lookup_mechanic: {tail}"
    );
}
...
fn passive_projection_line_substitutes_value_or_skips() {
    ...
    assert_eq!(
        passive_projection_line(&e, &sheet),
        Some("信用评级 55".to_string())
    );
    ...
    assert_eq!(
        passive_projection_line(&upper, &sheet),
        Some("信用评级 55".to_string())
    );

    let missing = json!({"skills": {}});
    assert_eq!(
        passive_projection_line(&e, &missing),
        None,
    ...
    assert_eq!(
        passive_projection_line(&no_template, &sheet),
        None,
        "no passive_projection -> None"
    );
}
...
fn band_semantics_line_renders_from_success_tier_or_band() {
    let dice_core =
        json!({"success_bands":[{"id":"extreme","label":"极难成功","semantics":"贯穿/卓越效果"}]});
    ...
    for seg in ["extreme", "极难成功", "贯穿/卓越效果"] {
        assert!(
            by_tier.contains(seg),
            "line must contain `{seg}`: {by_tier}"
        );
    }
    // outcome carries "band" (gm_loop fixture shape) — both real-world forms
    let by_band =
        band_semantics_line(&dice_core, &json!({"band":"extreme"})).expect("band form must render");
    for seg in ["extreme", "极难成功", "贯穿/卓越效果"] {
        assert!(
            by_band.contains(seg),
            "line must contain `{seg}`: {by_band}"
        );
    }
...
fn band_without_semantics_is_none() {
    ...
    assert_eq!(
        band_semantics_line(&dice_core, &json!({"success_tier":"extreme"})),
        None
    );
}
...
fn scene_intents_text_renders_one_line_per_intent() {
    let intents = vec![
        intent(
            "homecoming.lawmen.cut_cable_force",
            "brawling",
            Some(json!({"kind":"dv","value":13})),
        ),
    ...
    assert!(
        lines[0].contains("homecoming.lawmen.cut_cable_force"),
        "line must carry intent_id: {}",
        lines[0]
    );
    assert!(
        lines[0].contains("brawling"),
        "line must carry tested_parameter: {}",
        lines[0]
    );
    assert!(
    ...
    assert!(
        lines[1].contains("homecoming.lawmen.sneak_past"),
        "line must carry intent_id: {}",
        lines[1]
    );
    assert!(
        lines[1].contains("stealth"),
        "line must carry tested_parameter: {}",
        lines[1]
    );
    assert!(
        lines[1].ends_with("| -"),
        "difficulty None must render as `-`: {}",
        lines[1]
    );
}
...
fn scene_intents_text_empty_returns_none() {
    assert_eq!(
        scene_intents_text(&[]),
        None,
        "empty slice -> None (caller emits no block)"
    );
}

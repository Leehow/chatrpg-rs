...
impl MaterializationService {
    ...
    pub async fn materialize_turn(
        &self,
        input: MaterializationTurnInput<'_>,
        ...
        let semantic = SemanticIntentService::from_env(self.db.clone())
            .classify_turn(sem_req)
            .await
            .ok();
        let mut requests = semantic
            .as_ref()
            .map(|s| s.materialization_requests.clone())
            .unwrap_or_default();
        ...
        for req in requests {
            if req.target_label.trim().is_empty() {
                continue;
            }
            let key = (
                req.target_kind.as_str().to_string(),
                req.target_id.clone(),
                normalize_label(&req.target_label),
            );
            if unique.iter().any(|x: &MaterializationRequest| {
                (
                    x.target_kind.as_str().to_string(),
                    x.target_id.clone(),
                    normalize_label(&x.target_label),
                ) == key
            }) {
                continue;
            }
        ...
        if unique.is_empty() {
            return Ok(MaterializationTurnResult {
                semantic,
                ..Default::default()
            });
        }
        let mut out = MaterializationTurnResult {
            handled: true,
            semantic,
            phases: vec!["materialization_kernel".into()],
            ..Default::default()
        };
        for req in unique {
            let demand = self
                .create_demand(input, &req, out.semantic.as_ref())
                .await?;
            ...
            let writeback = self
                .write_back(&demand, &packet, &extraction, &verification)
                .await?;
            self.update_demand_status(&demand.demand_id, status_from_verification(&verification))
                .await
                .ok();
        ...
        out.phases.extend([
            "materialization_demand_created".into(),
            "mechanics_search_planned".into(),
            "source_evidence_collected".into(),
            "extraction_run_created".into(),
            "binding_verification_created".into(),
        ]);
        if out.writebacks.is_empty() {
            out.phases
                .push("source_evidence_retained_no_runtime_writeback".into());
        } else {
            out.phases.extend([
                "runtime_writeback_applied".into(),
                "parameter_facets_written".into(),
            ]);
        }
    ...
    fn demands_from_runtime_context(
        &self,
        input: MaterializationTurnInput<'_>,
        ...
        if let Some(sem) = semantic {
            ...
            let primary_search_key = sem
                .raw_json
                .get("object_search_keys")
            ...
            let weapon_label = sem
                .object_refs
                .iter()
                .map(|s| s.trim())
                .find(|s| !s.is_empty())
                .map(String::from)
                .unwrap_or_else(|| "currently used weapon or attack source".into());
            if matches!(
            ...
            if matches!(
                ...
                let label = sem
                    ...
                    .first()
                    .cloned()
                    .unwrap_or_else(|| "ability referenced by current input".into());
                out.push(MaterializationRequest {
                    request_id: format!("matreq_{}", Uuid::new_v4().simple()),
            ...
            let search_keys: Vec<String> = sem
                .raw_json
                .get("object_search_keys")
                .and_then(|v| v.as_array())
                .map(|a| {
    ...
    async fn create_demand(
        ...
        let search_key = req
            ...
            .or_else(|| {
                ...
                let idx = sem
                    .object_refs
                    .iter()
                    .position(|o| o.trim().eq_ignore_ascii_case(req.target_label.trim()))?;
                keys.get(idx)
                    .and_then(|k| k.as_str())
                    .map(str::trim)
                    .filter(|s| !s.is_empty())
        ...
        let demand = MaterializationDemand {
            ...
            module_id: input.module_id.map(str::to_string),
            requested_fields: if req.requested_fields.is_empty() {
                required_fields_for_target(target_kind)
                    .into_iter()
                    .map(str::to_string)
                    .collect()
            } else {
                req.requested_fields.clone()
            },
            urgency,
    ...
    async fn collect_evidence(
        ...
        let kernel = self
            ...
            .ok()
            .flatten();
        let module_config = if let Some(mid) = demand.module_id.as_deref() {
        ...
        if let Some(search) = &self.search {
            ...
            for q in queries.iter().take(10) {
                let resp = search
                    .search_async(&SearchRequest {
                        query: q.clone(),
                        domains: vec![
                            demand.ruleset_id.clone(),
                            demand.module_id.clone().unwrap_or_default(),
                            "rules".into(),
                            "modules".into(),
                            "source".into(),
                            "parsed".into(),
                        ...
                        limit: 8,
                        intent: Some(
                            "real_materialization_candidate_retrieval_v1_15_4_semantic_units"
                                .into(),
                        ),
                        ..Default::default()
                    })
                    ...
                    .unwrap_or_default();
                for h in resp.hits {
                    if hit_is_noise_or_low_signal(&h) {
                        continue;
                    }
                    ...
                    if hit_is_foreign_ruleset(&h.scopes, &demand.ruleset_id) {
                        continue;
                    }
                    ...
                    if candidates.iter().any(|c: &SourceCandidate| {
                        c.source_document_id == h.search_doc_id && c.excerpt_hash == evidence_hash
                    }) {
                        continue;
                    }
                    let raw_meta = h.metadata.get("raw").cloned().unwrap_or_else(|| json!({}));
                    let semantic_meta = raw_meta
                        .get("metadata")
                        .cloned()
                        .unwrap_or_else(|| json!({}));
            ...
            for q in name_queries.iter().take(6) {
                for h in search.grep_layout_tables(q, 5) {
                    let evidence_text = if h.context.is_empty() {
                        h.row.clone()
                    } else {
                        format!("{}\n{}", h.context.join("\n"), h.row)
                    };
                    let evidence_hash = sha256_hex(&evidence_text);
                    if candidates.iter().any(|c: &SourceCandidate| {
                        c.source_document_id == h.source_id && c.excerpt_hash == evidence_hash
                    }) {
                        continue;
                    }
                    candidates.push(SourceCandidate {
        ...
        let bundle = SourceEvidenceBundle {
            bundle_id: format!("evidence_bundle_{}", Uuid::new_v4().simple()),
            demand_id: demand.demand_id.clone(),
            candidates,
            query_plan_json: plan,
            source_priority_order: vec![
                SourceKind::Module,
                SourceKind::Rulebook,
                SourceKind::Unknown,
            ],
    ...
    async fn extract(
        ...
        let mut extracted = if let Some(client) = &self.llm {
            self.extract_with_llm(client, demand, bundle, extractor_kind)
                .await
                .unwrap_or_else(|_| fallback_extraction(demand, bundle))
        } else {
        ...
        let source_refs = bundle
            .candidates
            .iter()
            .flat_map(|c| c.source_refs.clone())
            .collect::<Vec<_>>();
                ...
                .unwrap_or(true)
        {
            if let Some(obj) = extracted.as_object_mut() {
                obj.insert(
                    "source_refs".into(),
                    serde_json::to_value(&source_refs).unwrap_or_else(|_| json!([])),
                );
            }
        }
        let missing_preview = missing_required_fields(demand, &extracted);
        let status = if bundle.candidates.is_empty() {
            BindingStatus::FailedNoSource
        } else if !missing_preview.is_empty() {
            BindingStatus::NeedsReview
        } else {
            BindingStatus::BoundExact
        };
        let confidence = if bundle.candidates.is_empty() || !missing_preview.is_empty() {
            RulingConfidence::Low
        } else {
            RulingConfidence::Medium
        };
        let run = ExtractionRun {
            extraction_run_id: format!("extraction_{}", Uuid::new_v4().simple()),
            demand_id: demand.demand_id.clone(),
            extractor_kind,
            input_candidate_ids: bundle
                .candidates
                .iter()
            ...
            model_id: self
                .llm
                .as_ref()
                .map(|_| "semantic_extractor_v1_15_4".into()),
            created_at: Utc::now(),
        };
    ...
    async fn binding_packet(
        ...
        let target_id = demand
            .target_id
            .clone()
            .unwrap_or_else(|| make_target_id(demand));
        let source_hits = bundle
            .candidates
            .iter()
            .map(candidate_to_search_hit)
            .collect::<Vec<_>>();
        let status = if bundle.candidates.is_empty() {
            BindingStatus::FailedNoSource
        } else {
        ...
        let packet = RuleBindingPacket {
            ...
            target_kind,
            target_id,
            ruleset_id: demand.ruleset_id.clone(),
            semantic_request_json: json!({"demand": demand, "request": req, "evidence_bundle_id": bundle.bundle_id, "extraction_run_id": extraction.extraction_run_id}),
            retrieval_queries: query_strings(&bundle.query_plan_json),
            source_hits,
            extracted_json: extraction.extracted_json.clone(),
            source_refs: extraction.source_refs.clone(),
            confidence: extraction.confidence,
            verification_status: status,
            created_at_tick: demand.world_tick,
            created_at: Utc::now(),
        };
        SemanticRuleBindingService::from_env(self.db.clone(), self.search.clone())
            .insert_rule_binding_packet(session_id, Some(turn_id), &packet)
            .await?;
        self.link_rule_binding_to_demand(
            &packet.binding_id,
            &demand.demand_id,
    ...
    async fn verify(
        ...
        let status = if packet.source_refs.is_empty() && packet.source_hits.is_empty() {
            BindingVerificationStatus::RejectedNoSource
        } else if extraction
            .extracted_json
            .get("extraction_status")
            .and_then(Value::as_str)
            == Some("insufficient_source_for_parameters")
        {
        ...
        let result = BindingVerificationResult {
            ...
            status,
            missing_required_fields: missing.clone(),
            contradictory_sources: vec![],
            visibility_issues: visibility_issues(demand, packet),
            confidence_adjustment: if missing.is_empty() {
                None
            } else {
                Some(RulingConfidence::Low)
            },
            repair_suggestions: repair_suggestions_for(demand.target_kind, &missing),
            verifier_json: json!({"demand_id": demand.demand_id, "extraction_run_id": extraction.extraction_run_id, "required_fields": required_fields_for_target(demand.target_kind), "missing": missing, "writeback_policy":"only verified_exact or explicitly allowed verified_partial may write runtime parameters"}),
            created_at: Utc::now(),
        };
        self.insert_verification(&result).await?;
        self.link_verification(&packet.binding_id, &result.verification_id)
            .await
            .ok();
        Ok(result)
    }
    ...
    async fn write_back(
        ...
        {
            self.insert_blocked_materialization_event(demand, packet, extraction, verification)
                .await
                .ok();
            return Ok(vec![]);
        }
        ...
        match demand.target_kind {
            ...
            MaterialTargetKind::ObjectDefinition
            | MaterialTargetKind::ObjectInstance
            | MaterialTargetKind::DamageProfile
            | MaterialTargetKind::ArmorProfile => out.push(
                self.writeback_object(demand, packet, extraction, verification)
                    .await?,
            ),
            MaterialTargetKind::AbilityDefinition | MaterialTargetKind::AbilityInstance => out
                .push(
                    self.writeback_ability(demand, packet, extraction, verification)
                        .await?,
                ),
            MaterialTargetKind::CheckTarget => {
                if let Some(w) = self.writeback_check(demand, packet).await? {
                    out.push(w);
                }
            }
            MaterialTargetKind::EffectProfile => {
                if let Some(w) = self.writeback_effect(demand, packet).await? {
                    out.push(w);
                }
            }
            _ => {}
        ...
        for w in &out {
            self.insert_parameter_facet_bindings(demand, packet, extraction, verification, w)
                .await
                .ok();
        }
        Ok(out)
    ...
    async fn writeback_actor(
        &self,
        demand: &MaterializationDemand,
        packet: &RuleBindingPacket,
        extraction: &ExtractionRun,
        verification: &BindingVerificationResult,
    ) -> Result<RuntimeBindingWriteback> {
        let actor_id = demand.target_id.clone().unwrap_or_else(|| {
            if demand.target_label.to_lowercase().contains("pc") {
                "pc.current".into()
            } else {
                "npc.opposition".into()
            }
        });
        let actor_kind = if actor_id.starts_with("pc") {
            "player_character"
        } else {
            "npc"
        };
        ...
        let actor_param_id = format!(
            "actor_params.{}.{}",
            safe_id(&demand.session_id),
            safe_id(&actor_id)
        );
        sqlx::query(r#"
        ...
        let writeback = RuntimeBindingWriteback {
            writeback_id: format!("writeback_{}", Uuid::new_v4().simple()),
            binding_id: packet.binding_id.clone(),
            target_kind: packet.target_kind,
            target_id: actor_id.clone(),
            table_name: "runtime_actor_parameters".into(),
            writeback_json: profile.clone(),
            status: verification.status.as_str().into(),
            created_at_tick: demand.world_tick,
        };
        self.insert_runtime_binding(
    ...
    async fn writeback_object(
        ...
        let object_def_id = demand.target_id.clone().unwrap_or_else(|| {
            format!(
                "{}.object.{}",
                safe_id(&demand.ruleset_id),
                safe_id(&demand.target_label)
            )
        });
        ...
        let def = ObjectDefinition {
            object_def_id: object_def_id.clone(),
            ruleset_id: demand.ruleset_id.clone(),
            name: demand.target_label.clone(),
            object_kind: kind,
            tags: vec!["materialized_v1_10".into()],
            mechanical_profile: profile.clone(),
            rule_bindings: vec![ObjectRuleBinding {
                binding_id: packet.binding_id.clone(),
                trigger: ObjectRuleTrigger::OnAttack,
                rule_packet_id: None,
                lookup_recipe_id: Some("real_materialization_extractor_v1_10".into()),
                source_refs: packet.source_refs.clone(),
                effect_json: extraction.extracted_json.clone(),
                confidence: packet.confidence,
            }],
            default_affordances: vec![],
            equip_slots: vec![],
            visibility_default: demand.visibility,
            source_refs: packet.source_refs.clone(),
        };
        sqlx::query(r#"
        ...
        let writeback = RuntimeBindingWriteback {
            ...
            binding_id: packet.binding_id.clone(),
            target_kind: packet.target_kind,
            target_id: object_def_id.clone(),
            table_name: "object_definitions".into(),
            writeback_json: profile,
            status: verification.status.as_str().into(),
            created_at_tick: demand.world_tick,
        };
        self.insert_runtime_binding(
    ...
    async fn writeback_ability(
        ...
        let ability_def_id = demand.target_id.clone().unwrap_or_else(|| {
            format!(
                "{}.ability.{}",
                safe_id(&demand.ruleset_id),
                safe_id(&demand.target_label)
        ...
        let def = AbilityDefinition {
            ability_def_id: ability_def_id.clone(),
            ruleset_id: demand.ruleset_id.clone(),
            name: demand.target_label.clone(),
            ability_kind: infer_ability_kind(&demand.ruleset_id, &demand.target_label),
            tags: vec!["materialized_v1_10".into()],
            source_kind: AbilitySourceKind::RulebookEntry,
            source_refs: packet.source_refs.clone(),
            activation: AbilityActivationModel {
                activation_kind: profile
                    .pointer("/activation/kind")
                    .and_then(Value::as_str)
                    .unwrap_or("manual")
                    .into(),
                action_cost: profile
                    .pointer("/activation/action_cost")
                    .and_then(Value::as_str)
                    .map(str::to_string),
                timing: profile
                    .pointer("/activation/timing")
                    .and_then(Value::as_str)
                    .map(str::to_string),
                notes: None,
            },
            cost: AbilityCostModel {
                resource: profile
                    .pointer("/cost/resource")
                    .and_then(Value::as_str)
                    .map(str::to_string),
                amount_json: profile.get("cost").cloned().unwrap_or_else(|| json!({})),
                consumes_use: profile
                    .pointer("/cost/consumes_use")
                    .and_then(Value::as_bool)
                    .unwrap_or(false),
                notes: None,
            },
            target: AbilityTargetModel {
                target_kind: profile
                    .pointer("/target/kind")
                    .and_then(Value::as_str)
                    .unwrap_or("unknown")
            ...
            effect: AbilityEffectModel {
                effect_kind: profile
                    .pointer("/effect/kind")
                    .and_then(Value::as_str)
                    .unwrap_or("rules_bound_effect")
                    .into(),
                summary: profile
                    .pointer("/effect/summary")
                    .and_then(Value::as_str)
                    .unwrap_or("Source-backed ability effect; verify before applying patches.")
                    .into(),
                creates_check: profile
                    .pointer("/effect/creates_check")
                    .and_then(Value::as_bool)
                    .unwrap_or(false),
                creates_effect: true,
                duration: profile
                    ...
                    .and_then(Value::as_str)
                    .map(str::to_string),
                raw_json: profile.clone(),
            },
            rule_bindings: vec![AbilityRuleBinding {
                binding_id: packet.binding_id.clone(),
                trigger: AbilityTriggerKind::ManualActivation,
                rule_packet_id: None,
                lookup_recipe_id: Some("real_materialization_extractor_v1_10".into()),
                source_refs: packet.source_refs.clone(),
                effect_json: extraction.extracted_json.clone(),
            ...
            visibility_default: demand.visibility,
            mechanical_profile: profile.clone(),
            binding_status: if matches!(
                verification.status,
                BindingVerificationStatus::VerifiedExact
                    | BindingVerificationStatus::VerifiedPartial
            ) {
        ...
        let writeback = RuntimeBindingWriteback {
            ...
            binding_id: packet.binding_id.clone(),
            target_kind: packet.target_kind,
            target_id: ability_def_id.clone(),
            table_name: "ability_definitions".into(),
            writeback_json: profile,
            status: verification.status.as_str().into(),
            created_at_tick: demand.world_tick,
        };
        self.insert_runtime_binding(
            "ability_runtime_bindings",
            &writeback,
            &demand.session_id,
            None,
            Some(&ability_def_id),
    ...
    async fn writeback_check(
        &self,
        demand: &MaterializationDemand,
        packet: &RuleBindingPacket,
    ) -> Result<Option<RuntimeBindingWriteback>> {
        let Some(check_id) = demand.target_id.as_ref() else {
            return Ok(None);
        };
        ...
        Ok(Some(RuntimeBindingWriteback {
            ...
            table_name: "check_contracts".into(),
            writeback_json: json!({"rule_binding_id":packet.binding_id,"demand_id":demand.demand_id}),
            status: "attached".into(),
            created_at_tick: demand.world_tick,
        }))
    }
    async fn writeback_effect(
        &self,
        demand: &MaterializationDemand,
        packet: &RuleBindingPacket,
    ) -> Result<Option<RuntimeBindingWriteback>> {
        let Some(effect_id) = demand.target_id.as_ref() else {
            return Ok(None);
        };
        ...
        Ok(Some(RuntimeBindingWriteback {
            ...
            table_name: "effect_contracts".into(),
            writeback_json: json!({"rule_binding_id":packet.binding_id,"demand_id":demand.demand_id}),
            status: "attached".into(),
            created_at_tick: demand.world_tick,
        }))
    ...
    pub async fn materialization_context_block(
        &self,
        session_id: &str,
        world_tick: i64,
    ) -> Result<ContextBlock> {
        ...
        let mut block = ContextBlock::new(
            format!("runtime.materialization.{}", session_id),
            BlockKind::MaterializationGraph,
            "Runtime Materialization / Binding Verifier + Mechanics Search Skills",
            BlockContent::Json(
                json!({"world_tick":world_tick,"demands":demands,"binding_verifications":verifications,"mechanics_query_plans":mechanics_query_plans,"parameter_facet_bindings":parameter_facets,"cache_policy":"BP3 dynamic: active materialization demands, SearchSkill query plans, extraction/verifier results, and facet writebacks. Stable verified facets may graduate to BP2 definitions."}),
            ),
            Visibility::GmOnly,
    ...
    async fn insert_mechanics_query_plan(&self, bundle: &SourceEvidenceBundle) -> Result<()> {
        let Some(plan) = bundle.query_plan_json.get("plan") else {
            return Ok(());
        };
        let demand_id = plan
            .get("demand_id")
            .and_then(Value::as_str)
            .unwrap_or(&bundle.demand_id);
        let session_id: String =
            sqlx::query_scalar("select session_id from materialization_demands where demand_id=$1")
                .bind(demand_id)
        ...
        let plan_id = plan
            .get("plan_id")
            .and_then(Value::as_str)
            .unwrap_or("mechanics_query_plan_unknown");
        let search_skill = bundle
            .query_plan_json
            .get("search_skill")
            .and_then(Value::as_str)
            .or_else(|| plan.get("skill_kind").and_then(Value::as_str))
            .unwrap_or("generic_mechanical_search");
        let target_kind = bundle
            .query_plan_json
            .get("target_kind")
            .and_then(Value::as_str)
            .unwrap_or("unknown");
    ...
    async fn insert_parameter_facet_bindings(
        &self,
        demand: &MaterializationDemand,
        packet: &RuleBindingPacket,
        extraction: &ExtractionRun,
        verification: &BindingVerificationResult,
        writeback: &RuntimeBindingWriteback,
    ) -> Result<()> {
    ...
    async fn update_demand_status(
        &self,
        demand_id: &str,
        status: MaterializationDemandStatus,
    ) -> Result<()> {
        sqlx::query(
            "update materialization_demands set status=$2, updated_at=now() where demand_id=$1",
        )
        .bind(demand_id)
        .bind(status.as_str())
        .execute(&self.db.pool)
    ...
    async fn link_rule_binding_to_demand(
        &self,
        binding_id: &str,
        demand_id: &str,
        extraction_id: &str,
    ) -> Result<()> {
        sqlx::query("update rule_binding_packets set demand_id=$2, extraction_run_id=$3 where binding_id=$1").bind(binding_id).bind(demand_id).bind(extraction_id).execute(&self.db.pool).await?;
        Ok(())
    }
    async fn link_verification(&self, binding_id: &str, verification_id: &str) -> Result<()> {
        sqlx::query("update rule_binding_packets set verification_id=$2 where binding_id=$1")
            .bind(binding_id)
    ...
    async fn insert_runtime_binding(
        &self,
        table: &str,
        writeback: &RuntimeBindingWriteback,
        session_id: &str,
        actor_id: Option<&str>,
        object_or_ability_id: Option<&str>,
        runtime_target_id: &str,
    ) -> Result<()> {
        let table = match table {
            "actor_runtime_bindings" => "actor_runtime_bindings",
            "object_runtime_bindings" => "object_runtime_bindings",
            "ability_runtime_bindings" => "ability_runtime_bindings",
            _ => return Ok(()),
        };
...
fn search_skill_for_demand(demand: &MaterializationDemand) -> SearchSkillKind {
    ...
    let label = format!(
        "{} {} {}",
        demand.target_label, demand.target_description, fields
    )
    .to_lowercase();
    match demand.target_kind {
        ...
        | MaterialTargetKind::EncounterCard => SearchSkillKind::NpcStatblock,
        MaterialTargetKind::ObjectDefinition | MaterialTargetKind::ObjectInstance => {
            if label.contains("armor")
                ...
                || label.contains("护甲")
            {
                SearchSkillKind::ArmorDefense
            } else if label.contains("weapon")
                || label.contains("damage")
                || label.contains("gun")
                || label.contains("pistol")
                || label.contains("sword")
                || label.contains("枪")
                || label.contains("刀")
                || label.contains("武器")
            {
                SearchSkillKind::WeaponParameter
            } else {
                SearchSkillKind::SceneObject
            }
        }
        ...
        MaterialTargetKind::AbilityDefinition | MaterialTargetKind::AbilityInstance => {
            SearchSkillKind::AbilityActivation
        }
        MaterialTargetKind::CheckTarget => SearchSkillKind::CombatResolution,
        MaterialTargetKind::EffectProfile => {
            if label.contains("san")
                || label.contains("sanity")
                || label.contains("chaos")
                || label.contains("harm")
                ...
                || label.contains("混沌")
            {
                SearchSkillKind::ConditionResource
            } else {
                SearchSkillKind::CombatResolution
            }
        }
...
fn mechanics_query_plan_for(
    ...
    let fields = if demand.requested_fields.is_empty() {
        required_fields_for_target(demand.target_kind)
            .into_iter()
            .map(str::to_string)
            .collect::<Vec<_>>()
    } else {
        demand.requested_fields.clone()
    };
    ...
    let preferred = aliases
        ...
        .collect::<Vec<_>>();
    let field_aliases = aliases
        .get("field_aliases")
        .cloned()
        .unwrap_or_else(|| json!({}));
    let locator_queries = if preferred.is_empty() {
        vec![format!("{} {} rules locator", ruleset, skill.as_str())]
    } else {
        preferred
            .iter()
    ...
    steps.push(QueryPlanStep {
        step_kind: QueryPlanStepKind::Field,
        queries: fields
            .iter()
            .flat_map(|f| {
                vec![
                    format!("{} {} {}", ruleset, label, f),
                    format!("{} {} {}", ruleset, skill.as_str(), f),
                ]
            })
            .collect(),
        purpose:
            "Retrieve requested parameter fields without assuming a system-specific table name."
                .into(),
        stop_when: Some("Required fields are present or verified absent for this ruleset.".into()),
        metadata: json!({"field_aliases": field_aliases}),
    });
    ...
    MechanicsQueryPlan {
        plan_id: format!("mechplan_{}", Uuid::new_v4().simple()),
        demand_id: Some(demand.demand_id.clone()),
        ruleset_id: ruleset,
        module_id: demand.module_id.clone(),
        skill_kind: skill,
        target_kind: demand.target_kind,
        target_label: demand.target_label.clone(),
        requested_fields: fields,
        steps,
        ruleset_aliases: aliases,
        module_source_preferences: module_prefs,
        writeback_targets: writeback_targets_for_skill(skill),
        created_at_tick: demand.world_tick,
    }
}
...
fn queries_for_step(plan: &MechanicsQueryPlan, step_kind: QueryPlanStepKind) -> Vec<String> {
    plan.steps
        .iter()
        .filter(|s| s.step_kind == step_kind)
        .flat_map(|s| s.queries.clone())
        .filter(|q| !q.trim().is_empty())
        .collect()
}

fn procedure_queries_for(
    skill: SearchSkillKind,
    ruleset: &str,
    label: &str,
    desc: &str,
) -> Vec<String> {
    match skill {
        SearchSkillKind::CombatResolution => vec![
            format!(
                "{} attack combat resolution opposed check defense target number",
                ruleset
            ),
            format!("{} {} attack hit miss defense range cover", ruleset, label),
            format!("{} {}", ruleset, desc),
        ],
        SearchSkillKind::WeaponParameter => vec![
            format!("{} weapon damage range ammo attack skill", ruleset),
            format!("{} {} damage weapon table", ruleset, label),
            format!("{} damage armor hit weapon", ruleset),
        ],
        SearchSkillKind::ArmorDefense => vec![
            format!("{} armor defense protection damage reduction", ruleset),
            format!("{} {} armor shield defense", ruleset, label),
        ],
        SearchSkillKind::AbilityActivation => vec![
            format!(
                "{} ability spell activation cost target effect trigger",
                ruleset
            ),
            format!("{} {} ability spell effect", ruleset, label),
        ],
        SearchSkillKind::ConditionResource => vec![
            format!(
                "{} condition resource harm sanity chaos wound recovery",
                ruleset
            ),
            format!("{} {} resource condition track", ruleset, label),
        ],
        SearchSkillKind::NpcStatblock => vec![
            format!(
                "{} npc stat block monster card enemy stats hp defense attacks",
                ruleset
            ),
            format!("{} {} npc card stat block", ruleset, label),
        ],
        SearchSkillKind::ModuleCard => vec![
            format!(
                "{} module npc card object card vehicle card encounter",
                ruleset
            ),
            format!("{} {} module card", ruleset, label),
        ],
        SearchSkillKind::SceneObject => vec![
            format!(
                "{} scene object interaction hazard device lock cable affordance",
                ruleset
            ),
            format!("{} {} scene object", ruleset, label),
        ],
        SearchSkillKind::GenericMechanical => {
            vec![format!("{} {} rules procedure", ruleset, label)]
        }
    }
}
...
fn ruleset_aliases_for(kernel: Option<&RuleKernel>, skill: SearchSkillKind) -> Value {
    let mut base = match skill {
        SearchSkillKind::CombatResolution => {
            json!({"preferred_sections":["combat","attack","defense","opposed check","resolving actions"],"field_aliases":{"target_number":["DC","DV","AC","defense","evasion","resistance"],"opposition":["opposed","contest","save","resist"]}})
        }
        SearchSkillKind::WeaponParameter => {
            json!({"preferred_sections":["weapons","equipment","item data","weapon table"],"field_aliases":{"damage":["damage","DMG","power","damage dice"],"range":["range","base range","distance"],"ammo":["ammo","magazine","uses"]}})
        }
        SearchSkillKind::ArmorDefense => {
            json!({"preferred_sections":["armor","defense","protection","damage reduction"],"field_aliases":{"armor":["AC","SP","armor","defense","protection","soak","ablation"]}})
        }
        SearchSkillKind::AbilityActivation => {
            json!({"preferred_sections":["spellcasting","spells","abilities","feats","techniques","programs"],"field_aliases":{"activation":["action","reaction","trigger","timing"],"cost":["slot","resource","cost"],"effect":["effect","damage","condition"]}})
        }
        SearchSkillKind::ConditionResource => {
            json!({"preferred_sections":["conditions","damage","healing","sanity","harm","chaos","resources"],"field_aliases":{"resource":["HP","SAN","harm","chaos","wound","condition"]}})
        }
        SearchSkillKind::NpcStatblock => {
            json!({"preferred_sections":["NPC","monster","creature statistics","stat block","cards"],"field_aliases":{"hp":["HP","hit points","health"],"defense":["AC","DV","defense","armor"],"attack":["attack","weapon","action"]}})
        }
    ...
    if let Some(sp) = kernel.and_then(|k| k.search_profile.as_ref()) {
        ...
        if let Some(base_obj) = base["field_aliases"].as_object_mut() {
            for src in [
                sp.field_aliases_by_skill.get("*"),
                sp.field_aliases_by_skill.get(key),
            ] {
...
fn module_preferences_for(
    module_config: Option<&ModuleConfig>,
    skill: SearchSkillKind,
) -> Vec<String> {
    let mut prefs = match skill {
        SearchSkillKind::NpcStatblock => {
            vec!["npc card", "stat block", "enemy", "creature", "monster"]
        }
        SearchSkillKind::ModuleCard => vec![
            "npc card",
            "vehicle card",
        ...
        SearchSkillKind::CombatResolution => {
            vec!["combat note", "encounter", "enemy behavior", "tactics"]
        }
...
fn writeback_targets_for_skill(skill: SearchSkillKind) -> Vec<RuleBindingTargetKind> {
    match skill {
        ...
        SearchSkillKind::NpcStatblock | SearchSkillKind::ModuleCard => vec![
            RuleBindingTargetKind::ActorParameter,
            RuleBindingTargetKind::ObjectDefinition,
            RuleBindingTargetKind::AbilityDefinition,
        ],
...
fn facet_kinds_for_demand(demand: &MaterializationDemand) -> Vec<ParameterFacetKind> {
    match demand.target_kind {
        ...
        | MaterialTargetKind::NpcStatBlock
        | MaterialTargetKind::VehicleCard
        | MaterialTargetKind::EncounterCard => vec![
            ParameterFacetKind::ActorCheckFacet,
            ParameterFacetKind::ActorDefenseFacet,
            ParameterFacetKind::ActorResourceTrack,
        ],
        MaterialTargetKind::ObjectDefinition | MaterialTargetKind::ObjectInstance => vec![
            ParameterFacetKind::ObjectAttackFacet,
            ParameterFacetKind::ObjectDamageFacet,
            ParameterFacetKind::ObjectArmorFacet,
            ParameterFacetKind::ObjectDurabilityFacet,
        ],
        MaterialTargetKind::DamageProfile => vec![
            ParameterFacetKind::ObjectDamageFacet,
            ParameterFacetKind::EffectDamageBinding,
        ],
        MaterialTargetKind::ArmorProfile => vec![
            ParameterFacetKind::ObjectArmorFacet,
            ParameterFacetKind::ActorDefenseFacet,
        ],
        MaterialTargetKind::AbilityDefinition | MaterialTargetKind::AbilityInstance => vec![
            ParameterFacetKind::AbilityActivationFacet,
            ParameterFacetKind::AbilityTriggerFacet,
            ParameterFacetKind::AbilityCostFacet,
            ParameterFacetKind::AbilityEffectFacet,
        ],
        MaterialTargetKind::CheckTarget => vec![
            ParameterFacetKind::CheckResolutionBinding,
            ParameterFacetKind::ContestResolutionBinding,
        ],
        MaterialTargetKind::EffectProfile => vec![
            ParameterFacetKind::EffectDamageBinding,
            ParameterFacetKind::EffectResourceBinding,
            ParameterFacetKind::EffectConditionBinding,
        ],
        MaterialTargetKind::ConditionDefinition => vec![
            ParameterFacetKind::ActorConditionTrack,
            ParameterFacetKind::EffectConditionBinding,
            ParameterFacetKind::EffectResourceBinding,
        ],
...
fn material_kind_from_rule_kind(kind: RuleBindingTargetKind, meta: &Value) -> MaterialTargetKind {
    if let Some(s) = meta.get("material_target_kind").and_then(Value::as_str) {
        return parse_material_target_kind(s);
    }
    match kind {
...
fn parse_material_target_kind(s: &str) -> MaterialTargetKind {
    match s {
        "actor_profile" => MaterialTargetKind::ActorProfile,
        "object_definition" => MaterialTargetKind::ObjectDefinition,
        "object_instance" => MaterialTargetKind::ObjectInstance,
        "ability_definition" => MaterialTargetKind::AbilityDefinition,
        "ability_instance" => MaterialTargetKind::AbilityInstance,
        "npc_stat_block" => MaterialTargetKind::NpcStatBlock,
        "vehicle_card" => MaterialTargetKind::VehicleCard,
        "encounter_card" => MaterialTargetKind::EncounterCard,
        "check_target" => MaterialTargetKind::CheckTarget,
        "effect_profile" => MaterialTargetKind::EffectProfile,
...
fn urgency_from_request(req: &MaterializationRequest) -> MaterializationUrgency {
    match req.urgency {
        RuntimeUrgency::Immediate | RuntimeUrgency::BeforeResolution => {
            MaterializationUrgency::BlockingMechanicalResolution
        }
        RuntimeUrgency::Background => MaterializationUrgency::BackgroundPrewarm,
        RuntimeUrgency::Soon => MaterializationUrgency::BeforeNarration,
    }
}
fn query_strings(plan: &Value) -> Vec<String> {
    ...
    for key in [
        ...
        if let Some(arr) = plan.get(key).and_then(Value::as_array) {
            for v in arr {
                if let Some(s) = v.as_str() {
                    if !s.trim().is_empty() && !out.contains(&s.to_string()) {
...
fn source_kind_from_hit(hit: &SearchHit) -> SourceKind {
    ...
    match meta
        .get("source_kind")
        .and_then(Value::as_str)
        .or_else(|| raw.get("source_kind").and_then(Value::as_str))
        .unwrap_or("")
    {
    ...
    if hit.domain.contains("module") || hit.origin.contains("module") {
        SourceKind::Module
    } else if hit.domain.contains("rule") || hit.origin.contains("rule") {
        SourceKind::Rulebook
    } else {
        SourceKind::Unknown
    }
...
fn hit_is_noise_or_low_signal(hit: &SearchHit) -> bool {
    ...
    let category = meta
        ...
        .or_else(|| raw.get("semantic_category"))
        .and_then(Value::as_str)
        .unwrap_or("");
    matches!(
        category,
        "toc" | "index" | "copyright" | "credits" | "front_matter" | "page_header_footer"
    )
}
...
fn evidence_score_from_hit(hit: &SearchHit, base_score: f32) -> f32 {
    ...
    if meta.get("unit_kind").and_then(Value::as_str) == Some("semantic_unit") {
        score += 0.25;
    }
    if meta.get("signal_class").and_then(Value::as_str) == Some("signal") {
        score += 0.15;
    }
    match meta
        .get("semantic_category")
        ...
        .unwrap_or("")
    {
        "stat_block" | "npc" | "object" | "ability" | "mechanic_rule" | "table" | "scene"
        | "clue" => score += 0.2,
        _ => {}
    }
    score
}
fn candidate_to_search_hit(c: &SourceCandidate) -> SearchHit {
    SearchHit {
        hit_id: c.candidate_id.clone(),
        search_doc_id: c.source_document_id.clone(),
        origin: c.source_kind.as_str().into(),
        domain: c.source_kind.as_str().into(),
        logical_kind: "materialization_candidate".into(),
        title: c
            .heading_path
            .clone()
        ...
        tags: vec!["materialization".into()],
        visibility: Visibility::GmOnly,
        source_refs: c.source_refs.clone(),
        metadata: c.metadata.clone(),
        explain: json!({"retrieval_reason": c.retrieval_reason}),
    }
}
fn extractor_for(kind: MaterialTargetKind) -> ExtractorKind {
    match kind {
        MaterialTargetKind::ActorProfile => ExtractorKind::ActorProfile,
        MaterialTargetKind::NpcStatBlock => ExtractorKind::NpcCard,
        MaterialTargetKind::VehicleCard => ExtractorKind::VehicleCard,
        MaterialTargetKind::ObjectDefinition | MaterialTargetKind::ObjectInstance => {
            ExtractorKind::ObjectEntry
        }
        MaterialTargetKind::ArmorProfile => ExtractorKind::ArmorProfile,
        MaterialTargetKind::DamageProfile => ExtractorKind::DamageProfile,
        MaterialTargetKind::AbilityDefinition | MaterialTargetKind::AbilityInstance => {
            ExtractorKind::AbilityEntry
        }
...
fn extractor_schema(kind: ExtractorKind) -> &'static str {
    match kind {
        ExtractorKind::ActorProfile | ExtractorKind::NpcCard => {
            r#"{entry_name, actor_kind, archetype, threat_tier, stats, skills, hp, defense, equipment_refs, ability_refs, source_refs, missing_fields, confidence}"#
        }
        ExtractorKind::ObjectEntry | ExtractorKind::DamageProfile | ExtractorKind::ArmorProfile => {
            r#"{entry_name, object_kind, aliases, mechanical_profile:{damage,range,rof,ammo,armor,properties,special_rules}, rule_bindings, source_refs, missing_fields, confidence}"#
        }
        ExtractorKind::AbilityEntry => {
            r#"{entry_name, ability_kind, activation, cost, target, effect, trigger_bindings, source_refs, missing_fields, confidence}"#
        }
        _ => r#"{entry_name, fields, source_refs, missing_fields, confidence}"#,
    }
}
...
fn required_fields_for_target(kind: MaterialTargetKind) -> Vec<&'static str> {
    match kind {
        MaterialTargetKind::ActorProfile | MaterialTargetKind::NpcStatBlock => {
            vec!["actor_kind", "hp", "defense", "skills", "equipment_refs"]
        }
        MaterialTargetKind::ObjectDefinition | MaterialTargetKind::ObjectInstance => vec![
            "entry_name",
            "object_kind",
        ...
        MaterialTargetKind::DamageProfile => vec![
            "source_actor",
            "target_actor",
            "source_object_or_ability",
            "damage_expression_or_effect",
        ],
        MaterialTargetKind::ArmorProfile => vec!["armor_type", "defense_or_sp", "coverage_or_slot"],
        MaterialTargetKind::AbilityDefinition | MaterialTargetKind::AbilityInstance => {
            vec!["activation", "cost", "target", "effect", "source_refs"]
        }
        MaterialTargetKind::CheckTarget => {
            vec!["check_label", "target", "dice_or_resolution_model"]
        }
...
fn missing_required_fields(demand: &MaterializationDemand, extracted: &Value) -> Vec<String> {
    required_fields_for_target(demand.target_kind)
        .into_iter()
        .filter(|f| !json_has_field(extracted, f))
        .map(str::to_string)
        .collect()
}
fn json_has_field(v: &Value, field: &str) -> bool {
    ...
    if v.get(field).is_some_and(meaningful) {
        return true;
    }
    if v.get("fields")
...
fn status_from_verification(v: &BindingVerificationResult) -> MaterializationDemandStatus {
    match v.status {
        BindingVerificationStatus::VerifiedExact => MaterializationDemandStatus::Verified,
        BindingVerificationStatus::VerifiedPartial => MaterializationDemandStatus::Bound,
        BindingVerificationStatus::ProvisionalNeedsAudit => {
            MaterializationDemandStatus::Provisional
        }
        _ => MaterializationDemandStatus::Failed,
    }
}
fn visibility_issues(demand: &MaterializationDemand, packet: &RuleBindingPacket) -> Vec<String> {
    if demand.visibility == Visibility::PlayerVisible
        && packet
            .extracted_json
            .get("playwalled")
            .and_then(Value::as_bool)
            .unwrap_or(false)
    {
        vec!["playwalled material cannot be player-visible".into()]
    } else {
        vec![]
    }
}
fn repair_suggestions_for(kind: MaterialTargetKind, missing: &[String]) -> Vec<String> {
    if missing.is_empty() {
        vec![]
    } else {
        vec![format!(
            "Run a narrower {} extractor for missing fields: {}",
            kind.as_str(),
            missing.join(", ")
        )]
    }
}
fn profile_actor_json(
    demand: &MaterializationDemand,
    extraction: &ExtractionRun,
    verification: &BindingVerificationResult,
) -> Value {
    let hp = extracted_i64(
        ...
        &[
            ...
            "/fields/hp",
            "/fields/hp_max",
            "/mechanical_profile/hp",
            "/mechanical_profile/hp_max",
        ],
    );
    let defense = extraction
        .extracted_json
        .get("defense")
        .or_else(|| extraction.extracted_json.pointer("/fields/defense"))
        .cloned();
    let skills = extraction
        .extracted_json
        .get("skills")
        .or_else(|| extraction.extracted_json.pointer("/fields/skills"))
        .cloned();
    let equipment_refs = extraction
        .extracted_json
        .get("equipment_refs")
        .or_else(|| extraction.extracted_json.pointer("/fields/equipment_refs"))
        .cloned();
    ...
    if equipment_refs.is_none() {
        missing.push("equipment_refs");
    }
    json!({
...
fn profile_object_json(
    demand: &MaterializationDemand,
    extraction: &ExtractionRun,
    verification: &BindingVerificationResult,
) -> Value {
    json!({"ruleset":demand.ruleset_id,"label":demand.target_label,"extracted":extraction.extracted_json,"binding_verification":verification.status.as_str(),"source_policy":"source_backed_only_no_synthetic_defaults","source":"real_materialization_extractor_v1_10"})
}
fn profile_ability_json(
    demand: &MaterializationDemand,
    extraction: &ExtractionRun,
    verification: &BindingVerificationResult,
) -> Value {
...
fn infer_object_kind(label: &str, extracted: &Value) -> ObjectKind {
    ...
    if l.contains("armor") || l.contains("护甲") {
        ObjectKind::Armor
    } else if l.contains("shield") {
        ObjectKind::Shield
    } else if l.contains("gun")
        || l.contains("pistol")
        || l.contains("shotgun")
        || l.contains("revolver")
        || l.contains("rifle")
        ...
        || l.contains("枪")
        || l.contains("刀")
        || extracted.pointer("/mechanical_profile/damage").is_some()
        || extracted.get("damage").is_some()
    {
        ObjectKind::Weapon
    } else if l.contains("vehicle") || l.contains("车") {
        ObjectKind::Vehicle
    } else if l.contains("cable") || l.contains("线缆") {
        ObjectKind::Device
    } else {
        ObjectKind::Unknown
    }
...
fn infer_ability_kind(_ruleset_id: &str, label: &str) -> AbilityKind {
    ...
    {
        AbilityKind::Spell
    } else if l.contains("program") || l.contains("netrun") || l.contains("daemon") {
        AbilityKind::Program
    } else if l.contains("requisition") || label.contains("申请") || label.contains("异常") {
        AbilityKind::Requisition
    } else if l.contains("technique") || l.contains("stunt") {
        AbilityKind::Technique
    } else if l.contains("skill") {
        AbilityKind::SkillUse
    } else {
        AbilityKind::Unknown
    }
}
...
fn make_target_id(demand: &MaterializationDemand) -> String {
    format!(
        "{}.{}.{}",
        safe_id(&demand.ruleset_id),
        demand.target_kind.as_str(),
        safe_id(&demand.target_label)
    )
}
...
mod tests {
    ...
    fn kernel_with_cyberpunk_search_profile() -> RuleKernel {
        ...
        fa.insert(
            "combat_resolution".to_string(),
            json!({"target_number": ["DV","range DV"]}),
        );
    ...
    fn profile_sections_override_generic_base() {
        ...
        assert!(
            sections
                .iter()
                .any(|v| v.as_str() == Some("Friday Night Firefight")),
            "cyberpunk search_profile must supply FNF section; got {:?}",
            sections
        );
    }
    ...
    fn profile_field_aliases_merged_onto_base() {
        let k = kernel_with_cyberpunk_search_profile();
        let aliases = ruleset_aliases_for(Some(&k), SearchSkillKind::CombatResolution);
        let tn = aliases["field_aliases"]["target_number"]
            .as_array()
            .unwrap();
        assert!(
            tn.iter().any(|v| v.as_str() == Some("DV")),
            "field_aliases must include DV from search_profile override; got {:?}",
            tn
        );
    }
    ...
    fn no_profile_falls_back_to_generic_base() {
        ...
        let aliases = ruleset_aliases_for(Some(&k), SearchSkillKind::CombatResolution);
        let sections = aliases["preferred_sections"].as_array().unwrap();
        let has_fnf = sections
            .iter()
            .any(|v| v.as_str() == Some("Friday Night Firefight"));
        assert!(
            !has_fnf,
            "generic kernel must NOT have cyberpunk-specific FNF section"
        );
        assert!(
            !sections.is_empty(),
            "generic base must still provide sections"
        );
    }
    ...
    fn wildcard_override_applies_to_uncovered_skills() {
        ...
        for skill in [
            ...
            assert_eq!(
                aliases["preferred_sections"], expected,
                "uncovered skill {:?} must get the cyberpunk flat list via wildcard",
                skill
            );
            // field_aliases set on ALL skills (legacy SET behavior).
            assert_eq!(
                aliases["field_aliases"]["target_number"],
                json!(["DV", "range DV", "difficulty value"]),
                "wildcard target_number must apply to {:?}",
                skill
            );
            assert_eq!(
                aliases["field_aliases"]["armor"],
                json!(["SP", "armor", "ablation"]),
                "wildcard armor must apply to {:?}",
                skill
            );
        }
        ...
        if let Some(sp) = k2.search_profile.as_mut() {
            sp.preferred_sections_by_skill.insert(
                "weapon_parameter".to_string(),
                vec!["Weapons and Armor".to_string()],
            );
        }
        let wp = ruleset_aliases_for(Some(&k2), SearchSkillKind::WeaponParameter);
        assert_eq!(
            wp["preferred_sections"],
            json!(["Weapons and Armor"]),
    ...
    fn module_config_with_homecoming_prefs() -> ModuleConfig {
        ...
        let mut by_skill = std::collections::HashMap::new();
        by_skill.insert(
            "npc_stat_block".to_string(),
            vec![
                "Redesigned NPC cards".to_string(),
                "vehicle cards".to_string(),
            ],
        );
        cfg.module_search_profile = Some(SearchProfile {
            preferred_sections_by_skill: by_skill,
        });
        cfg
    }
    ...
    fn module_config_prefs_extend_base_for_skill() {
        ...
        assert!(
            prefs.contains(&"npc card".to_string()),
            "base 'npc card' must remain"
        );
        assert!(
            prefs.contains(&"Redesigned NPC cards".to_string()),
            "homecoming-specific section must be present; got {:?}",
            prefs
        );
    }
    ...
    fn no_module_config_gives_generic_base_only() {
        ...
        assert!(
            prefs.contains(&"npc card".to_string()),
            "generic base must include 'npc card'"
        );
        assert!(
            !prefs.iter().any(|s| s.contains("Redesigned NPC")),
            "generic base must NOT include homecoming-specific sections; got {:?}",
            prefs
        );
    }
    ...
    fn module_config_wrong_skill_key_gives_base_only() {
        ...
        assert!(
            !prefs.iter().any(|s| s.contains("Redesigned NPC")),
            "combat_resolution skill must not get npc sections from homecoming config; got {:?}",
            prefs
        );
    }
}

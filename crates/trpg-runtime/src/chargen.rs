...
pub async fn generate_starter_character(
    ...
    let steps: Vec<String> = pack
        ...
        .collect();
    let catalogs: Vec<String> = pack.option_catalogs.iter().map(|c| c.title.clone()).collect();
    ...
    let user_msg = if concept.trim().is_empty() {
        "Generate a fresh, interesting starter character. Make all the choices yourself.".to_string()
    } else {
...
pub fn refresh_mechanical_profile(profile: &mut Value, sheet: &Value) {
    let Some(p) = profile.as_object_mut() else { return };
    p.insert("stats".into(), sheet.get("stats").cloned().unwrap_or_else(|| json!({})));
    p.insert("skills".into(), sheet.get("skills").cloned().unwrap_or_else(|| json!({})));
...
fn build_chargen_inputs(sheet: &Value) -> serde_json::Map<String, Value> {
    ...
    for bucket in ["stats", "skills"] {
        if let Some(m) = sheet.get(bucket).and_then(|v| v.as_object()) {
            for (k, v) in m { inputs.insert(k.clone(), v.clone()); }
        }
    }
    if let Some(p) = sheet.get("player") { inputs.insert("player".into(), p.clone()); }
    ...
    if let Some(o) = sheet.as_object() {
        ...
        let have: std::collections::HashSet<String> = inputs.keys().map(|k| k.to_ascii_lowercase()).collect();
        for (k, v) in o {
...
fn route_chargen_results(report: &trpg_formula::ChargenReport, obj: &mut serde_json::Map<String, Value>) {
    for r in &report.values {
        ...
        if let Some(b) = obj.get_mut(bucket).and_then(|v| v.as_object_mut()) {
            let target = b.keys().find(|k| k.eq_ignore_ascii_case(&r.id)).cloned().unwrap_or_else(|| r.id.clone());
...
pub fn apply_chargen_formulas(records: &[Value], sheet: &mut Value) -> trpg_formula::ChargenReport {
    ...
    let Some(obj) = sheet.as_object_mut() else { return report; };
    route_chargen_results(&report, obj);
    obj.insert("derived_spec".into(), json!(report.values.iter().map(derived_spec_entry).collect::<Vec<_>>()));
...
pub fn recompute_live_derived(sheet: &mut Value) -> bool {
    let Some(spec) = sheet.get("chargen_spec").and_then(|v| v.as_array()).cloned() else { return false; };
    let live: Vec<Value> = spec
        .into_iter()
        .filter(|r| r.get("recompute").and_then(|v| v.as_str()).map(|s| s != "once").unwrap_or(true))
        .collect();
    if live.is_empty() { return false; }
    let inputs = build_chargen_inputs(sheet);
    let report = trpg_formula::evaluate_chargen(&live, &inputs);
    let Some(obj) = sheet.as_object_mut() else { return false; };
    ...
    if let Some(arr) = obj.get_mut("derived_spec").and_then(|v| v.as_array_mut()) {
        for r in &report.values {
            ...
            match arr.iter_mut().find(|e| e.get("id").and_then(|x| x.as_str()) == Some(r.id.as_str())) {
...
pub fn apply_track_change_to_sheet(
    ...
    if bucket == "field" {
        obj.insert(id.to_string(), match text { Some(t) => json!(t), None => json!(amount) });
        return Ok(());
    }
    ...
    if bucket == "tracks" {
        ...
        let new_val = match text {
            Some(t) => json!(t),
            None => { let cur = entry.get("value").and_then(|v| v.as_f64()).unwrap_or(0.0); json!(if op == "add" { cur + amount } else { amount }) }
        };
        ...
        let v = match text {
            Some(t) => json!(t),
            None => { let cur = bm.get(id).and_then(|v| v.as_f64()).unwrap_or(0.0); json!(if op == "add" { cur + amount } else { amount }) }
        };
...
impl RuntimeEngine {
    ...
    pub async fn create_and_bind_character(
        ...
        let pack = self
            ...
            .await?
            .ok_or_else(|| anyhow!("no character onboarding pack for '{ruleset_id}'; parse the ruleset first"))?;
        ...
        let status = if validation.status == "ok" { "ready" } else { "draft_needs_rules_source" };
        ...
        let session_id = match session_id {
            Some(s) => s.to_string(),
            None => self.start_session(ruleset_id, None).await?,
        };
        let world_tick = self.current_world_time(&session_id).await.map(|w| w.world_tick).unwrap_or(0);
...
mod chargen_formula_tests {
    ...
    fn coc_spec() -> Vec<Value> {
        serde_json::from_str(include_str!("../../../data/parsed/characters/call_of_cthulhu_7e.chargen.json")).expect("CoC chargen spec parses")
    }
    ...
    fn coc_chargen_computes_derived_overwriting_llm_guesses() {
        ...
        assert_eq!(sheet["resources"]["hp_max"], json!(12), "floor((60+65)/10)=12");
        assert_eq!(sheet["resources"]["mp_max"], json!(13), "floor(65/5)=13");
        assert_eq!(sheet["resources"]["sanity"], json!(65), "starting SAN = POW = 65 (not the kernel's 50)");
        assert_eq!(sheet["skills"]["Dodge"], json!(35), "Dodge=floor(70/2)=35, OVERWRITES the LLM guess 99 in place");
        assert!(sheet["skills"].get("dodge").is_none(), "no lowercase duplicate key");
        assert_eq!(sheet["stats"]["damage_bonus"], json!(0), "STR55+SIZ65=120 -> DB 0");
        assert_eq!(sheet["stats"]["build"], json!(0));
        assert!(report.gaps.is_empty());
        assert!(report.values.iter().all(|r| r.status == "provisional"), "pages unverified -> provisional (guardrail 4)");
    }
    ...
    fn coc_chargen_missing_input_degrades_not_fabricates() {
        ...
        assert!(sheet.get("resources").and_then(|r| r.get("hp_max")).is_none(), "no fabricated hp_max without CON/SIZ");
        assert_eq!(sheet["skills"]["dodge"], json!(35), "dodge.attr_derived needs only DEX -> 35");
    }
    ...
    fn live_recompute_updates_derived_on_base_change() {
        ...
        let spec = vec![json!({"id":"hp_max","role":"resource_max","input_kind":"derived","recompute":"live","result_type":"int","expr":"floor(({{con}}+{{siz}})/10)"})];
        let mut sheet = json!({"stats":{"CON":60,"SIZ":60}});
        apply_chargen_formulas(&spec, &mut sheet);
        assert_eq!(sheet["resources"]["hp_max"], json!(12), "chargen: floor((60+60)/10)=12");
        assert!(sheet.get("chargen_spec").is_some(), "full spec stored for live recompute");
        sheet["stats"]["CON"] = json!(40); // CON drained mid-play
        assert!(recompute_live_derived(&mut sheet), "hp_max changed");
        assert_eq!(sheet["resources"]["hp_max"], json!(10), "live: floor((40+60)/10)=10 after drain");
    }
    ...
    fn dnd_class_keyed_hp_is_engine_computed_from_player_choice() {
        ...
        assert_eq!(sheet["stats"]["constitution_modifier"], json!(1), "CON 13 -> +1");
        assert_eq!(sheet["resources"]["hit_points_max"], json!(7), "wizard d6(6) + CON mod(1) = 7, engine-computed (case-insensitive)");
        assert!(report.gaps.is_empty());
        // §10.1 live linkage: raising CON to 16 (+3) re-derives HP with NO LLM.
        sheet["stats"]["constitution_score"] = json!(16);
        assert!(recompute_live_derived(&mut sheet), "HP re-derives on CON change");
        assert_eq!(sheet["resources"]["hit_points_max"], json!(9), "live: wizard d6(6) + CON mod(3) = 9");
        ...
        assert_eq!(sheet["resources"]["hit_points_max"], json!(15), "barbarian d12(12) + CON mod(3) = 15");
    }

    #[test]
    fn apply_track_change_sets_adds_and_creates() {
        let mut sheet = json!({"tracks":{"fighter":{"value":4,"kind":"class_level"}}});
        apply_track_change_to_sheet(&mut sheet, "tracks", "fighter", "add", 1.0, None, None, None).unwrap();
        assert_eq!(sheet["tracks"]["fighter"]["value"], json!(5.0), "add to existing");
        apply_track_change_to_sheet(&mut sheet, "tracks", "rogue", "set", 1.0, None, Some("class_level"), None).unwrap();
        ...
        apply_track_change_to_sheet(&mut s2, "stats", "dexterity", "add", 1.0, None, None, None).unwrap();
    ...
    fn accumulator_base_value_not_clobbered_by_live_recompute() {
        ...
        apply_chargen_formulas(&spec, &mut sheet);
        apply_track_change_to_sheet(&mut sheet, "tracks", "fighter", "add", 1.0, None, None, None).unwrap();
        apply_track_change_to_sheet(&mut sheet, "stats", "hit_points", "add", 7.0, None, None, None).unwrap();
        assert!(recompute_live_derived(&mut sheet));
    ...
    fn live_recompute_refreshes_derived_spec_view() {
        ...
        let spec = vec![json!({"id":"hp_max","role":"resource_max","input_kind":"derived","recompute":"live","expr":"floor(({{con}}+{{siz}})/10)"})];
        let mut sheet = json!({"stats":{"CON":60,"SIZ":60}});
        apply_chargen_formulas(&spec, &mut sheet);
        let val = |s: &Value| s["derived_spec"].as_array().unwrap().iter()
            .find(|e| e["id"] == "hp_max").unwrap()["value"].clone();
        assert_eq!(val(&sheet), json!(12), "create: floor((60+60)/10)=12");
        sheet["stats"]["CON"] = json!(40);
        recompute_live_derived(&mut sheet);
        assert_eq!(val(&sheet), json!(10), "derived_spec view refreshed live (not stale 12)");
    }
    ...
    fn refresh_mechanical_profile_reprojects_from_sheet_preserving_metadata() {
        ...
        assert_eq!(profile["stats"]["proficiency_bonus"], json!(4), "grown derived now visible to combat");
        assert_eq!(profile["stats"]["total_level"], json!(9));
        assert_eq!(profile["skills"]["Stealth"], json!(5));
        assert_eq!(profile["fields"]["tracks"]["fighter"]["value"], json!(9), "tracks re-projected into fields");
        assert_eq!(profile["ruleset"], json!("dnd5e"), "metadata preserved");
        assert_eq!(profile["source_quality"], json!("created_character_v1"), "metadata preserved");
        assert!(profile["fields"].get("stats").is_none(), "fields excludes stats (no duplication)");
    }

    #[test]
    fn live_recompute_skips_once_snapshots() {
        // recompute=once keeps its chargen snapshot even if the base changes.
        let spec = vec![json!({"id":"sanity","role":"resource","input_kind":"derived","recompute":"once","result_type":"int","expr":"{{pow}}"})];
        let mut sheet = json!({"stats":{"POW":50}});
        apply_chargen_formulas(&spec, &mut sheet);
        assert_eq!(sheet["resources"]["sanity"], json!(50));
        sheet["stats"]["POW"] = json!(70);
        assert!(!recompute_live_derived(&mut sheet), "once record not recomputed");
        assert_eq!(sheet["resources"]["sanity"], json!(50), "once snapshot unchanged despite POW change");
    }
    ...
    fn cyberpunk_role_rank_track_feeds_derived() {
        ...
        apply_track_change_to_sheet(&mut sheet, "tracks", "solo", "add", 1.0, None, None, None).unwrap();
        assert!(recompute_live_derived(&mut sheet));
    ...
    fn categorical_field_set_drives_race_keyed_derived() {
        ...
        let spec = vec![json!({"id":"breath_damage_type","role":"attribute","input_kind":"derived","recompute":"live","result_type":"dice_or_int",
            "expr":"lookup(ancestry_dmg,{{draconic_ancestry}})","lookup_tables":{"ancestry_dmg":{"ranges":[
                {"key":"red","value":"fire"},{"key":"blue","value":"lightning"}]}}})];
        let mut sheet = json!({});
        apply_track_change_to_sheet(&mut sheet, "field", "draconic_ancestry", "set", 0.0, Some("red"), None, None).unwrap();
        apply_chargen_formulas(&spec, &mut sheet);
        assert_eq!(sheet["stats"]["breath_damage_type"], json!("fire"), "red ancestry -> fire");
        // switch ancestry to blue -> live re-derive to lightning
        apply_track_change_to_sheet(&mut sheet, "field", "draconic_ancestry", "set", 0.0, Some("blue"), None, None).unwrap();
        assert!(recompute_live_derived(&mut sheet), "categorical change re-derives");
        assert_eq!(sheet["stats"]["breath_damage_type"], json!("lightning"), "blue ancestry -> lightning (live)");
    }
...
mod compiler_runtime_tests {
    ...
    fn spec_from_template_maps_field_id_to_id_and_skips_prose_only() {
        ...
        t.derived_values = vec![
            ...
            DerivedValue { field_id: "luck".into(), formula: "3D6 x 5".into(), ..Default::default() },
        ];

...
//! # 严格无操作保证
//!
//! 当 [`MaterializationAffordanceMode::Off`] 时，此模块**不触碰**
//! `active_npc_ids`——调用方供给的任何值（含空 Vec）原样保留，字节一致。
//!
//! # 激活策略（Shadow / Enforce）
//!
...
///
/// - `Off`：严格无操作，直接返回，`active_npc_ids` 不被修改。
/// - `Shadow` / `Enforce`：仅当 `active_npc_ids` 为空时，从当前场景的
///   `referenced_npc_ids` 派生（stable-dedup，fail-closed）。
pub fn apply_npc_activation(
    ...
    // OFF = 严格无操作（s17 修正：不得替换调用方供给的任何值）
    if !mode.is_active() {
        return;
    }
...
mod tests {
    ...
    // ===== 1. 纯投影：referenced_npc_ids → active_npc_ids =====
    ...
    #[test]
    fn shadow_derives_from_scene_when_caller_empty() {
        let modules = make_modules("m1", "sc02", vec!["npc_bob"]);
        let mut ids: Vec<String> = Vec::new();
        apply_npc_activation(
            &mut ids,
            &modules,

...
pub fn parse_synthesis_response(
    ...
    let value = resp
        .get("value")
        .cloned()
        .ok_or_else(|| anyhow::anyhow!("persona-judge returned no value"))?;
    let reason = resp
        .get("reason")
...
pub async fn synthesize_npc_parameter(
    ...
    let resp = llm
        .complete_json(
            build_synthesis_messages(persona, parameter_id, check_context, ruleset_id),
            0.4,
        )
        .await?;
...
pub fn write_synthesized_param(sheet: &mut Value, bucket: &str, param: &str, p: &SynthesizedParam) {
    let obj = match sheet.as_object_mut() {
        Some(o) => o,
        None => return,
    };
    let prov = obj
        .entry("npc_param_provenance")
        .or_insert_with(|| json!({}));
    let existing_rank = prov
        .get(param)
        .and_then(|x| x.get("status"))
        .and_then(Value::as_str)
        .map(tier_rank)
        .unwrap_or(0);
    if tier_rank(&p.status) < existing_rank {
        return;
    } // no downgrade
    if let Some(pm) = prov.as_object_mut() {
        let mut entry = p.provenance.clone();
        if let Some(em) = entry.as_object_mut() {
            em.insert("status".into(), json!(p.status));
        }
        pm.insert(param.to_string(), entry);
    }
    let b = obj.entry(bucket.to_string()).or_insert_with(|| json!({}));
    if let Some(bm) = b.as_object_mut() {
        bm.insert(param.to_string(), p.value.clone());
    }
}
...
pub fn resolve_tiered_non_llm(
    source_value: Option<Value>,
    archetypes: &[(String, Value)],
    param: &str,
) -> Option<SynthesizedParam> {
    if let Some(v) = source_value {
        return Some(SynthesizedParam {
            value: v,
            status: "source_backed".into(),
    ...
    if let Some((v, aid)) = archetype_param(archetypes, param) {
        return Some(SynthesizedParam {
            value: v,
            status: "source_backed_archetype".into(),
...
pub async fn resolve_param_tiered(
    ...
    if let Some(p) = resolve_tiered_non_llm(source_value, archetypes, param) {
        return Ok(p);
    }
...
pub fn persona_synthesis_enabled() -> bool {
    std::env::var("TRPG_NPC_PERSONA_SYNTHESIS")
        .ok()
        .map(|v| {
            !matches!(
...
mod tests {
    ...
    fn parse_response_is_flagged_provisional_with_audit() {
        let persona = NpcPersona {
            actor_id: "npc.guard_1".into(),
            name: "City Watch Officer".into(),
        ...
        let out = parse_synthesis_response(
            &resp,
            &persona,
            "anti_theft_dv",
            "player attempts to pickpocket",
        )
        .unwrap();
    ...
    fn build_messages_grounds_in_persona_and_param() {
        ...
        let joined = msgs
            ...
            .join("\n");
        assert!(
            joined.contains("anti_theft_dv")
                && joined.contains("trained guard")
                && joined.contains("call_of_cthulhu_7e")
        );
    }
    ...
    fn provisional_write_lands_in_sheet_and_survives_refresh() {
        ...
        assert_eq!(
            sheet["npc_param_provenance"]["anti_theft_dv"]["tier"],
            json!("persona_judge")
        );
        assert_eq!(
            sheet["npc_param_provenance"]["anti_theft_dv"]["status"],
            json!("provisional")
        );
        // a later SOURCE value upgrades (overwrites) and records source_backed
        let src = SynthesizedParam {
            value: json!(70),
            status: "source_backed".into(),
            provenance: json!({"tier":"source"}),
        };
        ...
        let prov_again = SynthesizedParam {
            value: json!(50),
            status: "provisional".into(),
            provenance: json!({"tier":"persona_judge"}),
        };
        write_synthesized_param(&mut sheet, "skills", "anti_theft_dv", &prov_again);
        assert_eq!(
            sheet["skills"]["anti_theft_dv"],
            json!(70),
            "source_backed not downgraded"
        );
        assert_eq!(
            sheet["npc_param_provenance"]["anti_theft_dv"]["status"],
            json!("source_backed")
    ...
    fn non_llm_tiers_pick_source_then_archetype_else_none() {
        let r1 = resolve_tiered_non_llm(
            Some(json!(70)),
            &[("guard".into(), json!({"params":{"x":50}}))],
            "x",
        )
        .unwrap();
        assert_eq!(r1.value, json!(70));
        assert_eq!(r1.status, "source_backed");
        let r2 = resolve_tiered_non_llm(None, &[("guard".into(), json!({"params":{"x":50}}))], "x")
            .unwrap();
        assert_eq!(r2.value, json!(50));
        assert_eq!(r2.status, "source_backed_archetype");
        assert_eq!(r2.provenance["archetype_id"], json!("guard"));
        assert!(
            resolve_tiered_non_llm(None, &[("guard".into(), json!({"params":{}}))], "x").is_none()
        );
    }

    #[test]
    fn archetype_tier_returns_value_when_printed_else_none() {
        let archetypes: Vec<(String, Value)> = vec![(
            "guard".into(),
            json!({"fit_tags":["combat"],"params":{"hp":12}}),
        )];

...
async fn synthesized_npc_value_drives_opposed_verdict() {
    ...
    let pc = params.load_actor_parameters(SESSION, "pc.current").await.ok().flatten();
    if pc.as_ref().and_then(|p| p.mechanical_profile.pointer("/skills/Spot Hidden")).is_none() {
        eprintln!("SKIP: CoC fixture pc.current/Spot Hidden not found in this DB"); return;
    }
    // npc.opposition 卡必须存在,ensure_npc_parameter 才能 load 后合成。确保它在。
    params.ensure_actor_parameters(SESSION, RULESET, "npc.opposition", ActorKind::Npc, 0).await
    ...
    let persona = NpcPersona {
        actor_id: "npc.opposition".into(), name: "拉塞尔·威廉姆斯(拉斯)".into(),
        prose: "屠宰场里警觉而危险的人物,对周遭动静敏感。".into(),
    };
    let synth = engine.ensure_npc_parameter(SESSION, RULESET, &persona, "skills", "perception", "玩家试图在拉斯眼皮底下悄悄行动").await
        .expect("ensure_npc_parameter ok");
    let synth = match synth { Some(v) => v, None => { eprintln!("SKIP: synthesis off / no LLM (set TRPG_NPC_PERSONA_SYNTHESIS=true + TRPG_LLM_*)"); return; } };
    println!("[synth] npc.opposition perception = {synth}");

    // B1 断言:合成值已投影进 mechanical_profile(contest 读的是 mech)。
    let npc = params.load_actor_parameters(SESSION, "npc.opposition").await.ok().flatten().expect("npc card");
    let mech_perception = npc.mechanical_profile.pointer("/skills/perception").cloned();
    assert!(mech_perception.is_some(), "B1: 合成的 perception 必须投影进 mechanical_profile,否则 contest 读不到");
    println!("[B1] mechanical_profile.skills.perception = {:?}", mech_perception);
    ...
    let out = svc.resolve_outcome(&opposed_contract(), &roll(5), Some(&def_roll)).await.expect("resolve");
    ...
    assert!(success.is_some(), "对抗必须给出胜负,而非 Provisional-null(消费层失败)");
    let dv = opposed.and_then(|o| o.get("defender_value")).and_then(|v| v.as_i64());
    assert!(dv.is_some(), "outcome.opposed.defender_value 必须非空(= 消费了 NPC 现搓的 perception)");
    let av = opposed.and_then(|o| o.get("attacker_value")).and_then(|v| v.as_i64());
    assert_eq!(av, Some(75), "attacker_value 应为 pc.current 的 Spot Hidden=75");
    println!("PASS: NPC 现搓值 perception={:?} 驱动对抗结算,attacker(75) vs defender, success={:?}", dv, success);
}

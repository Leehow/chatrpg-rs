...
async fn critical_already_reached_returns_immediately() {
    ...
    db.save_turn(session, turn, "i", "o", json!({}), "ready").await.unwrap();
...
async fn waits_then_proceeds_when_critical_arrives_late() {
    ...
    db.save_turn(session, turn, "i", "o", json!({}), "ready").await.unwrap(); // streaming
    // 后台 ~300ms 后翻 critical_done（模拟前一回合 critical 组延迟落账）。
    let db2 = db.clone();
    let turn_owned = turn.to_string();
    tokio::spawn(async move {
        tokio::time::sleep(std::time::Duration::from_millis(300)).await;
        let _ = db2.set_turn_pp_lifecycle(&turn_owned, PP_CRITICAL_DONE).await;
    });
    let t = Instant::now();
...
async fn timeout_proceeds_fail_closed_when_critical_never_arrives() {
    ...
    db.save_turn(session, turn, "i", "o", json!({}), "ready").await.unwrap(); // 恒 streaming
    let t = Instant::now();

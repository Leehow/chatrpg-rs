...
async fn passive_defense_opposes_npc_attack_skill_not_its_defense() {
    ...
    params
        .upsert_actor_parameters(&seed_actor(
            ...
            json!({"stats": {"REF": 6}, "skills": {"evasion": 10}}),
        ))
        .await
        .expect("seed pc");
    params
        .upsert_actor_parameters(&seed_actor(
            ...
            npc_id,
            ActorKind::Npc,
            json!({"stats": {"defense": 8}, "skills": {"attack": 15}}),
        ))
        .await
        .expect("seed npc");
    ...
    let (bucket, param) = engine
        .check_param_need(RULESET, &SituationActionKind::Dodge)
        .await
        .expect("Dodge 必须需要对手参数（被动防御 → 对手攻击技能）");
    assert_eq!(
        (bucket.as_str(), param.as_str()),
        ("skills", "attack"),
    ...
    let persona = NpcPersona {
        ...
        name: "清道夫枪手".into(),
        prose: "举枪向你扫射的清道夫。".into(),
    };
    stamp_opposed_check(&mut contract, &persona, &bucket, &param);
    assert_eq!(
        contract.target_actor.as_ref().map(|a| a.actor_id.as_str()),
        Some(npc_id),
        "盖章后 target_actor=攻击你的 NPC"
    );
    assert_eq!(
        contract
            ...
            .map(|p| p.key.as_str()),
        Some("attack"),
        "opponent_tested_parameter=NPC 攻击技能"
    );
    ...
    let out = svc
        .resolve_outcome(&contract, &roll(&session, 9), Some(&roll(&session, 16)))
        .await
    ...
    assert_eq!(
        dv,
        Some(15),
        "defender_value 必须=NPC attack=15（消费攻击技能）；若=8 说明仍误测 defense（方向反）"
    );
    ...
    assert_eq!(
        success,
        Some(false),
        "玩家(attacker=闪避方)未胜 → NPC 攻击命中"
    );
    let winner = out.pointer("/opposed/winner").and_then(|v| v.as_str());
    assert_eq!(
        winner,
        Some("defender"),
        "winner=defender(NPC 攻击方)→ 玩家承受攻击，方向正确"
    );

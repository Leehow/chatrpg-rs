...
impl AbilityService {
    pub fn new(db: Db, search: Option<SearchService>) -> Self {
        Self { db, search }
    }

    pub async fn handle_turn(&self, input: AbilityTurnInput<'_>) -> Result<AbilityTurnResult> {
        if !env_bool("TRPG_ABILITY_KERNEL_ENABLE_V19", true) {
            return Ok(AbilityTurnResult::not_handled());
        }
        let semantic_req = SemanticIntentRequest {
        ...
        let semantic = SemanticIntentService::from_env(self.db.clone())
            .classify_turn(semantic_req)
            .await?;
        let ability_requests: Vec<MaterializationRequest> = semantic
            .materialization_requests
            .iter()
            .filter(|r| r.target_kind == RuleBindingTargetKind::AbilityDefinition)
            .cloned()
            .collect();
        if !matches!(
            semantic.primary_action_kind,
            SituationActionKind::ActivateAbility | SituationActionKind::TriggerAbility
        ) && ability_requests.is_empty()
        {
            return Ok(AbilityTurnResult::not_handled());
        }
        ...
        let req = ability_requests
            ...
            .unwrap_or_else(|| MaterializationRequest {
                ...
                metadata: json!({"source":"ability_service_default"}),
            });
        let ability_def_id = make_ability_def_id(input.ruleset_id, &req.target_label);
        let binding = SemanticRuleBindingService::from_env(self.db.clone(), self.search.clone())
            .bind_request(
                input.session_id,
                Some(input.turn_id),
                &ability_def_id,
                input.ruleset_id,
                &req,
                Some(input.world_tick),
            )
            .await
            .ok();
        let definition = self
            .ensure_ability_definition(
                input.ruleset_id,
                &ability_def_id,
                &req,
                binding.as_ref(),
            ...
            .await?;
        let instance = self
            .ensure_ability_instance(input.session_id, actor_id, &definition, input.world_tick)
            .await?;
        let activation = self
            .create_activation(input, actor_id, &definition, &instance, binding.as_ref())
            .await?;
        let check = self
            .build_core_mechanic_check(input, actor_id, &definition, binding.as_ref())
            .await?;
        ...
        result.phases = vec![
            "ability_kernel".into(),
            "semantic_classification".into(),
            "ability_definition_hydrated".into(),
            "ability_activation_contract_created".into(),
        ];
        if check.is_some() {
            result
                .phases
                .push("ability_core_mechanic_check_created".into());
        }
        result.check = check;
    ...
    pub async fn ensure_ability_definition(
        &self,
        ruleset_id: &str,
        ability_def_id: &str,
        req: &MaterializationRequest,
        binding: Option<&RuleBindingPacket>,
        _world_tick: i64,
    ) -> Result<AbilityDefinition> {
        if let Some(existing) = self.load_ability_definition(ability_def_id).await? {
            return Ok(existing);
        }
        let extracted = binding
            .map(|b| b.extracted_json.clone())
            .unwrap_or_else(|| json!({"extraction_status":"not_bound_yet"}));
        let kind = infer_ability_kind(ruleset_id, &req.target_label, &extracted);
        let binding_status = binding
            .map(|b| b.verification_status)
            .unwrap_or(BindingStatus::HydrationRequested);
        let def = AbilityDefinition {
            ability_def_id: ability_def_id.into(),
            ruleset_id: ruleset_id.into(),
            name: req.target_label.clone(),
            ability_kind: kind,
            tags: vec!["runtime_hydrated".into(), kind.as_str().into()],
            source_kind: AbilitySourceKind::RulebookEntry,
            source_refs: binding.map(|b| b.source_refs.clone()).unwrap_or_default(),
            activation: AbilityActivationModel {
                activation_kind: if matches!(kind, AbilityKind::Reaction) {
                    "reaction".into()
                } else {
                    "manual".into()
                },
                action_cost: extracted
                    .get("cost")
            ...
            cost: AbilityCostModel {
                resource: extracted
                    ...
                    .and_then(Value::as_str)
                    .map(str::to_string),
                amount_json: extracted
                    .get("cost_amount")
                    .cloned()
                    .unwrap_or_else(|| json!(null)),
                consumes_use: extracted
                    .get("consumes_use")
            ...
            effect: AbilityEffectModel {
                ...
                summary: extracted
                    .get("summary")
                    .or_else(|| extracted.get("effect"))
                    .and_then(Value::as_str)
                    .unwrap_or(
                        "Ability effect requires source-backed resolution before applying patches.",
                    )
                    .into(),
                creates_check: extracted
                    .get("creates_check")
                    .and_then(Value::as_bool)
                    .unwrap_or(false),
                creates_effect: true,
                duration: extracted
                    ...
                    .and_then(Value::as_str)
                    .map(str::to_string),
                raw_json: extracted.clone(),
            },
            rule_bindings: binding
                .map(|b| {
                    vec![AbilityRuleBinding {
                        binding_id: b.binding_id.clone(),
                        trigger: AbilityTriggerKind::ManualActivation,
                        rule_packet_id: None,
                        lookup_recipe_id: Some("semantic_rule_binding_v1_9".into()),
                        source_refs: b.source_refs.clone(),
                        effect_json: b.extracted_json.clone(),
                        confidence: b.confidence,
                    }]
                })
                .unwrap_or_default(),
            visibility_default: req.visibility,
            mechanical_profile: json!({"semantic_request":req,"binding":binding,"extracted":extracted}),
            binding_status,
        };
    ...
    pub async fn load_ability_definition(
        &self,
        ability_def_id: &str,
    ) -> Result<Option<AbilityDefinition>> {
        let row = sqlx::query(
            "select definition_json from ability_definitions where ability_def_id=$1 limit 1",
        )
        .bind(ability_def_id)
        .fetch_optional(&self.db.pool)
        .await?;
        Ok(row.and_then(|r| {
            serde_json::from_value::<AbilityDefinition>(r.get("definition_json")).ok()
        }))
    }
    pub async fn upsert_ability_definition(&self, def: &AbilityDefinition) -> Result<()> {
        ...
        for rb in &def.rule_bindings {
            let trig = AbilityTriggerBinding {
                trigger_id: format!("trigger_{}", Uuid::new_v4().simple()),
                ability_def_id: def.ability_def_id.clone(),
                trigger_kind: rb.trigger,
                condition_json: json!({"source":"ability_rule_binding","binding_id":rb.binding_id}),
                required: false,
                player_choice_required: matches!(
                    rb.trigger,
                    AbilityTriggerKind::OnDefense | AbilityTriggerKind::OnTakeDamage
                ),
                gm_secret_allowed: true,
                creates_gate: matches!(
                    rb.trigger,
                    AbilityTriggerKind::OnDefense | AbilityTriggerKind::OnTakeDamage
                ),
                creates_check: false,
                creates_effect: true,
                source_refs: rb.source_refs.clone(),
            };
    ...
    pub async fn ensure_ability_instance(
        &self,
        session_id: &str,
        actor_id: &str,
        def: &AbilityDefinition,
        world_tick: i64,
    ) -> Result<AbilityInstance> {
        let ability_id = format!(
            "ability.{}.{}.{}",
            safe_id(session_id),
            safe_id(actor_id),
            safe_id(&def.name)
        ...
        let instance = AbilityInstance {
            ...
            granted_by_object_id: None,
            granted_by_status_id: None,
            known_state: AbilityKnownState::KnownByActor,
            prepared_state: AbilityPreparedState::AlwaysAvailable,
            uses_state: AbilityUsesState::default(),
            cooldown_state: AbilityCooldownState::default(),
            visibility_state: AbilityVisibilityState {
                visibility: Visibility::GmOnly,
                known_state: AbilityKnownState::KnownByActor,
                reveal_conditions: vec![],
            },
            runtime_state: json!({"hydrated_from":"semantic_ability_kernel_v1_9"}),
    ...
    pub async fn create_activation(
        &self,
        input: AbilityTurnInput<'_>,
        actor_id: &str,
        def: &AbilityDefinition,
        instance: &AbilityInstance,
        binding: Option<&RuleBindingPacket>,
    ) -> Result<AbilityActivationContract> {
        let activation = AbilityActivationContract {
            activation_id: format!("ability_activation_{}", Uuid::new_v4().simple()),
            session_id: input.session_id.into(),
            turn_id: input.turn_id.into(),
            frame_id: input.frame_id.map(str::to_string),
            actor_id: actor_id.into(),
            ability_id: instance.ability_id.clone(),
            ability_def_id: def.ability_def_id.clone(),
            target_actor_ids: vec![],
            target_object_ids: vec![],
            target_zone_ids: vec![],
            activation_kind: if def.ability_kind == AbilityKind::Reaction {
                AbilityActivationKind::Reaction
            } else {
                AbilityActivationKind::Manual
            },
            check_contracts: vec![],
            ...
            actor_patches: vec![],
            cost_patches: vec![],
            source_refs: def.source_refs.clone(),
            rule_binding_ids: binding
                .map(|b| vec![b.binding_id.clone()])
                .unwrap_or_default(),
            status: AbilityActivationStatus::Created,
            created_at_tick: Some(input.world_tick),
        };
        sqlx::query(r#"
    ...
    pub async fn build_core_mechanic_check(
        &self,
        input: AbilityTurnInput<'_>,
        actor_id: &str,
        def: &AbilityDefinition,
        ...
        let kernel = match self.db.load_rule_kernel(input.ruleset_id).await? {
            Some(k) => k,
            None => return Ok(None),
        };
        let dice = kernel
        ...
        if let Some(b) = binding {
            merge_source_refs(&mut source_refs, &b.source_refs);
        }
        ...
        Ok(Some(CheckContract {
            ...
            initiator: ActorRef {
                ...
                display_name: Some("current actor".into()),
            },
            target_actor: None,
            ...
            actor_snapshot_ids: input
                .frame_id
                .map(|id| vec![id.to_string()])
                .unwrap_or_default(),
            source_refs,
            learned_packet_ids: binding
                .map(|b| vec![b.binding_id.clone()])
                .unwrap_or_default(),
            ...
            stakes: CheckStakes {
                before_roll_public: format!(
                    "You attempt to use {}. The core mechanic resolves the attempt.",
                    def.name
                ),
    ...
    pub async fn ability_context_block(
        &self,
        session_id: &str,
        world_tick: i64,
    ) -> Result<ContextBlock> {
        ...
        let abilities = rows
            .into_iter()
            .filter_map(|r| serde_json::from_value::<AbilityInstance>(r.get("instance_json")).ok())
            .collect::<Vec<_>>();
        let mut block = ContextBlock::new(
            format!("runtime.abilities.{}", session_id),
            BlockKind::AbilityGraph,
            "Runtime Ability / Trigger Graph",
            BlockContent::Json(
                json!({"world_tick":world_tick,"abilities":abilities,"cache_policy":"BP3 dynamic ability instances, uses, cooldowns, and triggers; stable names/locators remain BP2"}),
            ),
            Visibility::GmOnly,
...
fn infer_ability_label(input: &str) -> String {
    ...
    if lower.contains("shield") {
        ...
        "Fireball".into()
    } else if t.contains("护盾") {
        "护盾".into()
    } else if t.contains("火球") {
        "火球".into()
    } else if t.contains("战斗特技") {
        "战斗特技".into()
    } else if lower.contains("role ability") {
        "Role Ability".into()
    } else if lower.contains("program") {
        "Netrunning Program".into()
    } else {
        t.chars().take(48).collect()
    }
}
fn make_ability_def_id(ruleset_id: &str, label: &str) -> String {
    format!("{}.ability.{}", safe_id(ruleset_id), safe_id(label))
}
...
fn infer_ability_kind(_ruleset_id: &str, label: &str, _extracted: &Value) -> AbilityKind {
    ability_kind_from_label(label)
}
...
fn merge_source_refs(dst: &mut Vec<SourceRef>, src: &[SourceRef]) {
    for r in src {
        if !dst
            .iter()
            .any(|e| e.source_id == r.source_id && e.anchor_id == r.anchor_id)
        {
            dst.push(r.clone());
        }
    }

...
fn scene_facts_from_module_config() {
    ...
    let input = DirectorInput { request: &req, state: &state, compiled: &compiled(), user_input: "我不知道能做什么", conflict: None, module_config: Some(&cfg), participants: &[], prior_spotlights: &[] };
...
fn no_module_config_returns_generic_brief() {
    let req = minimal_request();
    let state = minimal_state();
    let input = DirectorInput { request: &req, state: &state, compiled: &compiled(), user_input: "怎么办", conflict: None, module_config: None, participants: &[], prior_spotlights: &[] };
    let brief = build_brief(input, None, GuidanceLevel::AskGoal);
...
fn npc_advice_from_module_config() {
    ...
    let req = minimal_request();
    let state = minimal_state();
    let input = DirectorInput { request: &req, state: &state, compiled: &compiled(), user_input: "怎么看", conflict: None, module_config: Some(&cfg), participants: &[], prior_spotlights: &[] };
    let advice = biased_npc_advice(input);

...
use tantivy::schema::{Field, IndexRecordOption, Schema, STORED, STRING, TEXT, Value as TantivyValue};
...
pub const TANTIVY_INDEX_SCHEMA_VERSION: &str = "chatrpg.tantivy_index.v3.runtime_load_incremental_cjk";
...
impl TantivySearchEngine {
    pub fn open(config: SearchConfig) -> Result<Self> {
        ...
        if config.index_dir.exists() {
            ...
            if existing_version.trim() != TANTIVY_INDEX_SCHEMA_VERSION {
                fs::remove_dir_all(&config.index_dir).with_context(|| {
                    format!("failed to reset stale Tantivy index dir {}", config.index_dir.display())
                })?;
            }
        }
        fs::create_dir_all(&config.index_dir)
            .with_context(|| format!("failed to create Tantivy index dir {}", config.index_dir.display()))?;
        ...
        Ok(Self {
            index,
            fields: SearchFields { search_doc_id, origin, domain, logical_kind, title, body, tags, cjk_ngrams, facets, scopes_json, visibility, stability, source_refs_json, metadata_json, updated_at },
            config,
        })
    }

    pub fn clear_all(&self) -> Result<()> {
        let mut writer = self.index.writer::<TantivyDocument>(self.config.writer_memory_budget_bytes)?;
    ...
    pub fn upsert_documents(&self, documents: &[SearchDocument]) -> Result<usize> {
        ...
        for source_doc in documents {
            writer.delete_term(Term::from_field_text(self.fields.search_doc_id, &source_doc.search_doc_id));
            let mut tdoc = doc!(
    ...
    pub fn search(&self, request: &SearchRequest) -> Result<SearchResponse> {
        ...
        let mut parser = QueryParser::for_index(&self.index, vec![self.fields.title, self.fields.body, self.fields.tags, self.fields.cjk_ngrams, self.fields.facets]);
        ...
        let top_limit = request.limit.saturating_mul(12).max(request.limit).min(1000) as usize;
    ...
    fn tantivy_doc_to_search_doc(&self, doc: &TantivyDocument) -> Result<SearchDocument> {
        ...
        let tags = doc.get_all(self.fields.tags).filter_map(|v| v.as_str().map(ToString::to_string)).collect::<Vec<_>>();
        ...
        let scopes = serde_json::from_str::<BTreeMap<String, String>>(&scopes_json).unwrap_or_default();
        let source_refs = serde_json::from_str::<Vec<SourceRef>>(&source_refs_json).unwrap_or_default();
        let metadata = serde_json::from_str::<Value>(&metadata_json).unwrap_or_else(|_| json!({}));
        Ok(SearchDocument { search_doc_id, origin, domain, logical_kind, title, body, tags, scopes, visibility, stability, source_refs, metadata, updated_at })
    }
}
...
impl SearchService {
    ...
    async fn reindex_with_mode(&self, mode: &str, incremental: bool) -> Result<SearchIndexStats> {
        ...
        let mut stats = SearchIndexStats {
            ...
            mode: mode.to_string(),
            incremental,
            watermark: if incremental { Some("per_source".to_string()) } else { None },
            index_dir: self.config.index_dir.to_string_lossy().to_string(),
            ..Default::default()
        };
        for config in configs.into_iter().filter(|c| c.enabled) {
            let watermark = if incremental { source_search_watermark(&self.pool, &config.source_config_id).await? } else { None };
            match self.load_config_documents_since(&config, watermark).await {
                Ok(documents) => {
                    ...
                    let max_updated_at = documents.iter().map(|d| d.updated_at).max().unwrap_or_else(Utc::now);
                    let indexed = self.engine.upsert_documents(&documents)?;
                    stats.indexed_documents += indexed;
                    upsert_source_search_watermark(&self.pool, &config.source_config_id, max_updated_at, indexed as i64).await.ok();
    ...
    pub async fn search_async(&self, request: &SearchRequest) -> Result<SearchResponse> {
        ...
        if !first.hits.is_empty() || !request.rewrite_query || !search_cjk_expansion_enabled() || !should_try_query_rewrites(&request.query) {
            return Ok(first);
        }
        for rewritten in rewrite_query_variants(&request.query) {
            if rewritten.trim().is_empty() || rewritten == request.query { continue; }
    ...
    pub fn load_hit_to_context_block(&self, req: &SearchLoadRequest) -> Result<ContextBlock> {
        let full_body = self.engine.get_document(&req.hit.search_doc_id)?
            .map(|doc| doc.body)
        ...
        if let Some(body) = full_body {
            ...
            block.token_estimate = Some((block.content.render_text().chars().count() as u32 / 4).max(1));
        }
        Ok(block)
    }

    async fn load_config_documents_since(&self, config: &SearchSourceConfig, watermark: Option<DateTime<Utc>>) -> Result<Vec<SearchDocument>> {
        match config.source_kind.as_str() {
            "sql_query" => load_sql_documents_since(&self.pool, config, watermark).await,
            "file_glob" => load_file_documents_since(&self.config.data_dir, config, watermark).await,
...
pub async fn load_search_source_configs(pool: &PgPool) -> Result<Vec<SearchSourceConfig>> {
    ...
    .fetch_all(pool)
    .await?;
    Ok(rows.into_iter().map(|row| SearchSourceConfig {
...
async fn load_sql_documents_since(pool: &PgPool, config: &SearchSourceConfig, watermark: Option<DateTime<Utc>>) -> Result<Vec<SearchDocument>> {
    let sql = config.config_json.get("sql").and_then(Value::as_str).ok_or_else(|| anyhow!("sql_query source missing config_json.sql"))?;
    let rows = if let Some(watermark) = watermark {
        let wrapped = format!("select * from ({}) as trpg_search_source where updated_at > $1", sql);
        sqlx::query(&wrapped).bind(watermark).fetch_all(pool).await?
    } else {
...
async fn load_file_documents_since(data_dir: &Path, config: &SearchSourceConfig, watermark: Option<DateTime<Utc>>) -> Result<Vec<SearchDocument>> {
    let root = relative_to_data_dir(data_dir, config.config_json.get("root").and_then(Value::as_str).unwrap_or("."));
    if !root.exists() { return Ok(vec![]); }
    let extensions = config.config_json.get("extensions").and_then(Value::as_array)
        .map(|arr| arr.iter().filter_map(Value::as_str).map(|s| s.to_lowercase()).collect::<HashSet<_>>())
        .unwrap_or_else(|| ["md".to_string(), "txt".to_string()].into_iter().collect());
    let domain = config.config_json.get("domain").and_then(Value::as_str).unwrap_or("source");
    let origin = config.config_json.get("origin").and_then(Value::as_str).unwrap_or("file");
    let logical_kind = config.config_json.get("logical_kind").and_then(Value::as_str).unwrap_or("source_excerpt");
    let visibility = visibility_from_str_local(config.config_json.get("visibility").and_then(Value::as_str).unwrap_or("gm_only"));
    let mut docs = Vec::new();
    for entry in WalkDir::new(&root).into_iter().filter_map(|entry| entry.ok()).filter(|e| e.file_type().is_file()) {
        let ext = entry.path().extension().and_then(|s| s.to_str()).unwrap_or_default().to_lowercase();
        if !extensions.contains(&ext) || !file_modified_after(entry.path(), watermark) { continue; }
        let text = fs::read_to_string(entry.path()).unwrap_or_default();
        if text.trim().is_empty() { continue; }
        let rel = entry.path().strip_prefix(data_dir).unwrap_or(entry.path()).to_string_lossy().to_string();
...
async fn load_jsonl_documents_since(data_dir: &Path, config: &SearchSourceConfig, watermark: Option<DateTime<Utc>>) -> Result<Vec<SearchDocument>> {
    let root = relative_to_data_dir(data_dir, config.config_json.get("root").and_then(Value::as_str).unwrap_or("parsed"));
    if !root.exists() { return Ok(vec![]); }
    let origin = config.config_json.get("origin").and_then(Value::as_str).unwrap_or("jsonl");
    let default_domain = config.config_json.get("domain").and_then(Value::as_str).unwrap_or("parsed");
    let default_visibility = visibility_from_str_local(config.config_json.get("visibility").and_then(Value::as_str).unwrap_or("gm_only"));
    let mut docs = Vec::new();
    for entry in WalkDir::new(&root).into_iter().filter_map(|entry| entry.ok()).filter(|e| e.file_type().is_file()) {
        if entry.path().extension().and_then(|s| s.to_str()) != Some("jsonl") || !file_modified_after(entry.path(), watermark) { continue; }
        let rel = entry.path().strip_prefix(data_dir).unwrap_or(entry.path()).to_string_lossy().to_string();
        let text = fs::read_to_string(entry.path()).unwrap_or_default();
        for (line_idx, line) in text.lines().enumerate() {
            ...
            add_scope_hints_from_path(&rel, &mut scopes);
            let visibility = meta.get("visibility_hint")
            ...
            let signal_class = meta.get("signal_class").and_then(Value::as_str).unwrap_or("");
            if signal_class == "noise" { tags.push("low_signal".into()); }
            tags.sort();
            tags.dedup();
            docs.push(SearchDocument {
...
fn jsonl_title(value: &Value) -> String {
    ...
    value.get("title").and_then(Value::as_str)
        ...
        .or_else(|| value.get("heading_context").and_then(Value::as_array).and_then(|arr| arr.last()).and_then(Value::as_str))
        .or_else(|| value.get("block_id").or_else(|| value.get("material_id")).and_then(Value::as_str))
...
fn jsonl_domain(value: &Value) -> Option<String> {
    value.get("domain").and_then(Value::as_str).map(str::to_string)
        .or_else(|| value.get("metadata").and_then(|m| m.get("owner_kind").and_then(Value::as_str)).map(|s| match s { "module" => "modules".to_string(), "ruleset" | "rulebook" => "rules".to_string(), _ => s.to_string() }))
}
...
fn jsonl_logical_kind(value: &Value) -> String {
    ...
    meta.and_then(|m| m.get("semantic_category").and_then(Value::as_str))
        ...
        .or_else(|| value.get("kind").or_else(|| value.get("material_type")).and_then(Value::as_str))
...
fn jsonl_tags(value: &Value) -> Vec<String> {
    let mut tags: Vec<String> = value.get("tags").and_then(Value::as_array)
    ...
    for key in ["element_types"] {
        if let Some(arr) = value.get(key).and_then(Value::as_array) {
            tags.extend(arr.iter().filter_map(Value::as_str).map(ToString::to_string));
        }
    }
    if let Some(meta) = value.get("metadata") {
        for key in ["semantic_category", "signal_class", "defined_entity", "unit_kind", "visibility_hint"] {
            if let Some(v) = meta.get(key).and_then(Value::as_str) { if !v.trim().is_empty() { tags.push(v.to_string()); } }
        }
...
fn jsonl_scopes(value: &Value, rel: &str) -> BTreeMap<String, String> {
    ...
    if let Some(scope) = value.get("scope") {
        if let Some(scope_id) = scope.get("scope_id").and_then(Value::as_str) { scopes.insert("scope_id".into(), scope_id.to_string()); }
        if let Some(scope_type) = scope.get("scope_type").and_then(Value::as_str) { scopes.insert("scope_type".into(), scope_type.to_string()); }
    }
    for key in ["ruleset_id", "module_id", "session_id", "scene_id", "location_id", "mission_id", "chapter_id", "bundle_id", "owner_id", "source_id", "chunk_id", "unit_id"] {
        if let Some(v) = value.get(key).and_then(Value::as_str) { scopes.insert(key.to_string(), v.to_string()); }
    }
    if let Some(meta) = value.get("metadata") {
        for key in ["semantic_category", "signal_class", "defined_entity", "owner_kind", "source_unit_id"] {
            if let Some(v) = meta.get(key).and_then(Value::as_str) { if !v.trim().is_empty() { scopes.insert(key.to_string(), v.to_string()); } }
        }
    }
    if let Some(page) = value.get("page").and_then(Value::as_u64) { scopes.insert("page".into(), page.to_string()); }
    if let Some(page) = value.get("page_start").and_then(Value::as_u64) { scopes.insert("page_start".into(), page.to_string()); }
    if let Some(page) = value.get("page_end").and_then(Value::as_u64) { scopes.insert("page_end".into(), page.to_string()); }
    if let Some(pages) = value.get("page_numbers").and_then(Value::as_array) {
        if let Some(first) = pages.first().and_then(Value::as_u64) { scopes.entry("page".into()).or_insert(first.to_string()); scopes.entry("page_start".into()).or_insert(first.to_string()); }
        if let Some(last) = pages.last().and_then(Value::as_u64) { scopes.entry("page_end".into()).or_insert(last.to_string()); }
    }
    if let Some(headings) = value.get("heading_context").and_then(Value::as_array) {
        let heading = headings.iter().filter_map(Value::as_str).collect::<Vec<_>>().join(" > ");
        if !heading.is_empty() { scopes.insert("heading".into(), heading); }
    }
    scopes
}

fn jsonl_source_refs(value: &Value) -> Vec<SourceRef> {
    if let Some(refs) = value.get("metadata").and_then(|m| m.get("source_refs")).or_else(|| value.get("source_refs")) {
        if let Ok(parsed) = serde_json::from_value::<Vec<SourceRef>>(refs.clone()) { if !parsed.is_empty() { return parsed; } }
    }
    let source_id = value.get("source_id").and_then(Value::as_str)
        .or_else(|| value.get("metadata").and_then(|m| m.get("source_id").and_then(Value::as_str)))
        .or_else(|| value.get("unit_id").and_then(Value::as_str).and_then(|c| c.split('.').next()))
        .or_else(|| value.get("chunk_id").and_then(Value::as_str).and_then(|c| c.split('.').next()))
        .unwrap_or("unknown")
        .to_string();
    let page = value.get("page").and_then(Value::as_u64)
        .or_else(|| value.get("page_start").and_then(Value::as_u64))
        .or_else(|| value.get("page_numbers").and_then(Value::as_array).and_then(|arr| arr.first()).and_then(Value::as_u64))
        .map(|p| p as u32);
    let section_path = value.get("heading_context").and_then(Value::as_array)
        .map(|arr| arr.iter().filter_map(Value::as_str).map(ToString::to_string).collect())
        .unwrap_or_else(Vec::new);
    vec![SourceRef { source_id, page, anchor_id: value.get("chunk_id").or_else(|| value.get("unit_id")).and_then(Value::as_str).map(str::to_string), section_path, char_start: None, char_end: None, text_hash: value.get("text_hash").and_then(Value::as_str).map(str::to_string), note: Some("jsonl_semantic_source".into()) }]
}

fn row_to_search_document(row: sqlx::postgres::PgRow, config: &SearchSourceConfig) -> Result<Option<SearchDocument>> {
    let search_doc_id = row_string(&row, "search_doc_id").unwrap_or_default();
    if search_doc_id.trim().is_empty() { return Ok(None); }
    ...
    let visibility = visibility_from_str_local(&row_string(&row, "visibility").unwrap_or_else(|| "gm_only".to_string()));
    let stability = stability_from_str_local(&row_string(&row, "stability").unwrap_or_else(|| "rarely_changed".to_string()));
    let scopes = row_json(&row, "scope_json").and_then(json_to_string_map).unwrap_or_default();
    let source_refs = row_json(&row, "source_refs").and_then(|v| serde_json::from_value::<Vec<SourceRef>>(v).ok()).unwrap_or_default();
    let metadata = row_json(&row, "metadata").unwrap_or_else(|| json!({"source_config_id": config.source_config_id}));
    let updated_at = row.try_get::<DateTime<Utc>, _>("updated_at").unwrap_or_else(|_| Utc::now());
    Ok(Some(SearchDocument { search_doc_id, origin, domain, logical_kind, title, body, tags, scopes, visibility, stability, source_refs, metadata, updated_at }))
}

fn row_string(row: &sqlx::postgres::PgRow, col: &str) -> Option<String> {
    row.try_get::<Option<String>, _>(col).ok().flatten().or_else(|| row.try_get::<String, _>(col).ok())
}

fn row_string_vec(row: &sqlx::postgres::PgRow, col: &str) -> Option<Vec<String>> {
    row.try_get::<Vec<String>, _>(col).ok().or_else(|| {
        row.try_get::<Value, _>(col).ok().and_then(|v| v.as_array().map(|arr| arr.iter().filter_map(Value::as_str).map(ToString::to_string).collect()))
    })
}

fn row_json(row: &sqlx::postgres::PgRow, col: &str) -> Option<Value> { row.try_get::<Value, _>(col).ok() }
...
async fn reset_search_watermarks(pool: &PgPool) -> Result<()> {
    sqlx::query(r#"delete from search_index_watermarks"#).execute(pool).await?;
    Ok(())
}

async fn source_search_watermark(pool: &PgPool, source_config_id: &str) -> Result<Option<DateTime<Utc>>> {
    let value = sqlx::query_scalar::<_, Option<DateTime<Utc>>>(
...
async fn upsert_source_search_watermark(pool: &PgPool, source_config_id: &str, last_doc_updated_at: DateTime<Utc>, indexed_document_count: i64) -> Result<()> {
    sqlx::query(
        r#"
...
fn file_modified_after(path: &Path, watermark: Option<DateTime<Utc>>) -> bool {
    let Some(watermark) = watermark else { return true; };
    fs_modified_at(path) > watermark
}

fn fs_modified_at(path: &Path) -> DateTime<Utc> {
    fs::metadata(path).and_then(|m| m.modified()).map(DateTime::<Utc>::from).unwrap_or_else(|_| Utc::now())
}
...
fn build_tantivy_query_string(req: &SearchRequest) -> String {
    ...
    if q.is_empty() || q == "*" { return "*".to_string(); }
    match req.mode {
...
fn expanded_query_clause(q: &str) -> String {
    ...
    if contains_cjk(q) { terms.extend(cjk_ngrams(q)); }
    terms.extend(bilingual_alias_terms(q));
    terms.sort();
    terms.dedup();
    terms.into_iter()
        .filter(|t| !t.trim().is_empty())
        .map(|t| if t.chars().any(char::is_whitespace) { quoted_query(&t) } else { t })
        .collect::<Vec<_>>()
        .join(" OR ")
}

fn build_cjk_index_terms(doc: &SearchDocument) -> String {
    let mut terms = cjk_ngrams(&format!("{} {} {}", doc.title, doc.body, doc.tags.join(" ")));
...
fn should_try_query_rewrites(query: &str) -> bool { contains_cjk(query) || !bilingual_alias_terms(query).is_empty() }

fn rewrite_query_variants(query: &str) -> Vec<String> {
    let mut variants = Vec::new();
    let grams = cjk_ngrams(query);
    if !grams.is_empty() { variants.push(grams.join(" ")); }
    let aliases = bilingual_alias_terms(query);
    if !aliases.is_empty() { variants.push(aliases.join(" ")); }
...
fn cjk_ngrams(input: &str) -> Vec<String> {
    ...
    let mut out = Vec::new();
    for c in &chars { out.push(c.to_string()); }
    for pair in chars.windows(2) { out.push(pair.iter().collect::<String>()); }
    out.sort();
    out.dedup();
    out
}
...
fn bilingual_alias_terms(input: &str) -> Vec<String> {
    ...
    for (needle, aliases) in pairs {
        if input.contains(needle) { out.extend(aliases.iter().map(|s| s.to_string())); }
    }
    if lower.contains("hack") { out.extend(["netrunning", "net", "architecture"].iter().map(|s| s.to_string())); }
    if lower.contains("athena") { out.extend(["drone", "hacking", "server", "behavior"].iter().map(|s| s.to_string())); }
    if lower.contains("basic tech") { out.extend(["tech", "repair", "dv", "skill"].iter().map(|s| s.to_string())); }
    out.sort();
    out.dedup();
    out
}

fn quoted_query(q: &str) -> String { format!("\"{}\"", q.replace('"', " ")) }

fn matches_request(doc: &SearchDocument, req: &SearchRequest) -> bool {
    if !visibility_allowed(doc.visibility, &req.viewer) { return false; }
    if !req.domains.is_empty() && !req.domains.iter().any(|d| normalize_facet_value(d) == normalize_facet_value(&doc.domain)) { return false; }
    if !req.kinds.is_empty() && !req.kinds.iter().any(|k| normalize_facet_value(k) == normalize_facet_value(&doc.logical_kind)) { return false; }
    if !req.tags.is_empty() {
        let doc_tags = doc.tags.iter().map(|t| normalize_facet_value(t)).collect::<HashSet<_>>();
        if !req.tags.iter().all(|tag| doc_tags.contains(&normalize_facet_value(tag))) { return false; }
    }
    for (key, value) in &req.scopes {
        if !scope_matches(doc, key, value) { return false; }
    }
    ...
    for (key, values) in &req.filters {
        ...
        for value in values {
            if doc.scopes.get(key).map(|v| normalize_facet_value(v)) == Some(normalize_facet_value(value)) { matched = true; break; }
            if facets.contains(&format!("{}__{}", normalize_facet_value(key), normalize_facet_value(value))) { matched = true; break; }
            if let Some(meta_value) = doc.metadata.get(key).and_then(Value::as_str) {
                if normalize_facet_value(meta_value) == normalize_facet_value(value) { matched = true; break; }
            }
        }
...
fn scope_matches(doc: &SearchDocument, key: &str, value: &str) -> bool {
    ...
    if doc.scopes.get(key).map(|v| normalize_facet_value(v)) == Some(want.clone()) { return true; }
    if doc.scopes.get("path").map(|v| normalize_facet_value(v).contains(&want)).unwrap_or(false) { return true; }
    match key {
        "ruleset_id" => doc.scopes.get("scope_type").map(|v| v.as_str()) == Some("ruleset") && doc.scopes.get("scope_id").map(|v| normalize_facet_value(v)) == Some(want),
        "module_id" => doc.scopes.get("scope_type").map(|v| v.as_str()) == Some("module") && doc.scopes.get("scope_id").map(|v| normalize_facet_value(v)) == Some(want),
        "session_id" => doc.scopes.get("scope_type").map(|v| v.as_str()) == Some("session") && doc.scopes.get("scope_id").map(|v| normalize_facet_value(v)) == Some(want),
        "scene_id" => doc.scopes.get("scope_type").map(|v| v.as_str()) == Some("scene") && doc.scopes.get("scope_id").map(|v| normalize_facet_value(v)) == Some(want),
        _ => doc.scopes.get("bundle_id").map(|v| normalize_facet_value(v).contains(&want)).unwrap_or(false),
    }
}
...
fn visibility_allowed(visibility: Visibility, viewer: &VisibilityProfile) -> bool {
    match viewer.viewer_kind {
        ...
        ViewerKind::Npc => matches!(visibility, Visibility::Public | Visibility::PlayerVisible | Visibility::NpcPrivate),
    }
}
...
fn search_hit_from_doc(doc: &SearchDocument, score: f32, req: &SearchRequest) -> SearchHit {
    SearchHit {
        ...
        explain: if req.explain { json!({"score": score, "facets": doc.facets(), "updated_at": doc.updated_at}) } else { json!({}) },
    }
}

fn make_snippet(body: &str, query: &str, max_chars: usize) -> String {
    let body = body.trim();
    if body.len() <= max_chars { return body.to_string(); }
    let lowered = body.to_lowercase();
    let terms = query.split_whitespace().map(|s| s.trim_matches(|c: char| !c.is_alphanumeric()).to_lowercase()).filter(|s| s.len() > 2).collect::<Vec<_>>();
    let pos = terms.iter().filter_map(|t| lowered.find(t)).min().unwrap_or(0);
...
fn add_scope_hints_from_path(rel: &str, scopes: &mut BTreeMap<String, String>) {
    ...
    if let Some(stem) = stem {
        ...
        } else if normalized_path.contains("markdown/rulebooks/") || normalized_path.contains("rulebooks/") {
...
fn relative_to_data_dir(data_dir: &Path, configured: &str) -> PathBuf {
    ...
    if path.is_absolute() { path } else { data_dir.join(path) }
}

fn chunk_text(text: &str, max_chars: usize) -> Vec<String> {
    if text.len() <= max_chars { return vec![text.to_string()]; }
...
fn jsonl_body_text(value: &Value) -> String {
    ...
    if let Some(content) = value.get("content") {
        if let Some(s) = content.get("value").and_then(Value::as_str) { return s.to_string(); }
        if let Some(s) = content.get("text").and_then(Value::as_str) { return s.to_string(); }
...
fn layout_column_gaps(line: &str) -> usize {
    ...
    for c in line.chars() {
        if c == ' ' { run += 1; } else { if run >= 2 { gaps += 1; } run = 0; }
    }
    if run >= 2 { gaps += 1; }
    gaps
}
...
pub fn grep_layout_tables_in(data_dir: &std::path::Path, query: &str, max_hits: usize) -> Vec<LayoutTableHit> {
    let tokens: Vec<String> = query.split_whitespace().filter(|t| t.chars().count() >= 2).map(|t| t.to_lowercase()).collect();
    ...
    for kind in ["rulebooks", "modules"] {
        ...
        let mut files: Vec<std::path::PathBuf> = rd.flatten().map(|e| e.path())
            .filter(|p| p.file_name().map(|n| { let n = n.to_string_lossy(); n.ends_with(".md") && !n.ends_with(".layout.md") }).unwrap_or(false))
            .collect();
        files.sort();
        for p in files {
            let Ok(text) = std::fs::read_to_string(&p) else { continue };
            let source_id = p.file_name().map(|n| n.to_string_lossy().trim_end_matches(".md").to_string()).unwrap_or_default();
            let lines: Vec<&str> = text.lines().collect();
            let mut cur_page: Option<u32> = None;
            for (i, raw) in lines.iter().enumerate() {
                if let Some(pg) = raw.trim().strip_prefix("<!-- source_id=")
                ...
                if trimmed.len() < 4 || trimmed.starts_with("# Page") || trimmed.starts_with("<!--") { continue; }
                let low = line.to_lowercase();
                if tokens.iter().all(|t| low.contains(t.as_str())) {
                    let ctx_start = i.saturating_sub(2);
                    let context: Vec<String> = lines[ctx_start..i].iter().map(|s| s.trim_end().to_string())
                        .filter(|s| { let t = s.trim(); !t.is_empty() && !t.starts_with("<!--") && !t.starts_with("# Page") })
                        .collect();
                    hits.push(LayoutTableHit { source_id: source_id.clone(), source_kind: kind.to_string(), page: cur_page, row: line.to_string(), context, column_score: layout_column_gaps(line) });
                }
            }
        }
    }

...
impl CombatProfilePack {
    pub fn load_dir(path: impl AsRef<Path>) -> Result<Self> {
        ...
        for entry in fs::read_dir(path)
            .with_context(|| format!("failed to read ruleset advice dir {}", path.display()))?
        {
            let entry = entry?;
            let p = entry.path();
            ...
            let text = fs::read_to_string(&p)
                .with_context(|| format!("failed to read combat profile {}", p.display()))?;
            match serde_json::from_str::<RulesetCombatProfile>(&text) {
                Ok(profile) if !profile.profile_id.is_empty() => dir_profiles.push(profile),
                Ok(_) => {
                    tracing::warn!(path = %p.display(), "combat profile missing profile_id; skipped")
                }
                Err(err) => {
                    tracing::warn!(path = %p.display(), error = %err, "failed to parse combat profile; skipped")
                }
            }
        }
        ...
        for d in dir_profiles {
            match profiles
                .iter_mut()
                .find(|e| e.ruleset_id == d.ruleset_id && e.profile_id == d.profile_id)
            {
                Some(slot) => *slot = d,
    ...
    pub fn resolve(&self, ruleset_id: &str, mode_hint: Option<CombatMode>) -> RulesetCombatProfile {
        ...
        self.profiles
            .iter()
            .find(|p| {
                p.ruleset_id == ruleset_id
                    && target_mode
                        .as_ref()
                        .map(|m| p.applies_to_modes.iter().any(|x| x == m))
                        .unwrap_or(true)
            })
            .cloned()
            .or_else(|| {
                self.profiles
                    .iter()
                    .find(|p| p.ruleset_id == ruleset_id)
                    .cloned()
            })
            .or_else(|| {
                self.profiles
                    .iter()
                    .find(|p| p.ruleset_id == "generic")
                    .cloned()
            })
...
impl CombatAgent {
    ...
    pub async fn handle_turn(&self, input: ConflictTurnInput<'_>) -> Result<ConflictTurnResult> {
        ...
        if let Some(frame) = self.active_situation_frame(input.session_id).await? {
            let intent = input
                ...
                .unwrap_or_else(|| {
                    classify_situation_intent(
                        input,
                        Some(&frame),
                        base_profile.investigate_opens_frame,
                    )
                });
        ...
        let intent = input
            ...
            .unwrap_or_else(|| {
                classify_situation_intent(input, None, base_profile.investigate_opens_frame)
            });
        if !should_start_frame(base_profile.low_confidence_frame_start, &intent) {
            return Ok(ConflictTurnResult::not_handled());
        }

        let mode = combat_mode_from_policy(
            kernel.as_ref().and_then(|k| k.combat_mode_policy.as_ref()),
            &intent,
        );
        ...
        let mut frame = self
            .create_frame(input, &profile, mode, Some(intent.clone()))
            .await?;
        let event = self
            .write_frame_event(
                &frame,
        ...
        if should_immediately_open_reaction_gate(&intent) {
            if let Some(advice) = profile.reaction_windows.first().cloned() {
                ...
                InteractionLifecycleKernel::new(self.db.clone())
                    .attach_gate_to_active_frame(
                        input.session_id,
                        &gate.gate_id,
                        Some(&frame.frame_id),
                    )
                ...
                update_combat_working_state_phase(
                    &mut frame,
                    CombatPhase::AwaitingRequiredReaction,
                );
        ...
        if result.gate.is_none() && should_create_action_contract(&intent) {
            let check = self
                .make_combat_check_contract(input, &frame, &intent)
                .await;
            let effect = make_effect_contract(&check, &frame, &intent);
            self.db.insert_check_contract(&check, "created").await.ok();
            self.db
                .insert_effect_contract(&effect, input.session_id, input.turn_id)
                .await
                .ok();
            let action_event = self
                .write_frame_event(
                    &frame,
                    input.turn_id,
                    "combat_action_declared",
                    json!({
    ...
    async fn make_combat_check_contract(
        &self,
        ...
        self.hydrate_combat_check_contract_from_rule_steward(
            input,
            intent,
            &mut check,
            kernel.as_ref(),
    ...
    async fn hydrate_combat_check_contract_from_rule_steward(
        &self,
        input: ConflictTurnInput<'_>,
        intent: &ConflictIntent,
        ...
        let pack = match self
            .db
            .load_character_onboarding_pack(input.ruleset_id)
            .await
        {
            Ok(Some(pack)) => pack,
            _ => return,
        ...
        let matching_formula_ids = formulas
            .iter()
            .filter(|f| {
                let hay = format!(
                    "{} {} {}",
        ...
        if !matching_formula_ids.is_empty()
            || !pack.derived_formula_pack.source_refs.is_empty()
            || !pack.source_refs.is_empty()
        {
            ...
            check
                .learned_packet_ids
                .push(pack.derived_formula_pack.pack_id.clone());
            check
                .advice_refs
                .push("rule_steward.derived_formula_pack.bound_to_combat_check.v1_16_2".into());
            check.advice_refs.extend(
                matching_formula_ids
                    .iter()
                    .map(|id| format!("formula:{id}")),
            );
            ...
            if is_attack || is_defense {
                ...
                let chosen = preferred.iter().find_map(|p| {
                    formulas
                        .iter()
                        .find(|f| f.field_id.to_ascii_lowercase().contains(p))
                });
                ...
                if let (Some(f), Some(actor)) = (chosen, actor.as_ref()) {
                    ...
                    if f.is_provisional_seed() {
                        ...
                        if let Some(c) = compile_formula(&f.field_id, &f.formula, &stats, &skills) {
                            ...
                            check
                                .advice_refs
                                .push(format!("formula.compiled:{}={}", f.field_id, c.expression));
                            for u in &c.unresolved {
                                check
                                    .advice_refs
                                    .push(format!("formula.unbound_modifier:{u}"));
                            }
                ...
                if is_attack && matches!(check.target, CheckTargetModel::UnknownUntilLookup) {
                    if let Some((dv, label)) = resolve_attack_dv(formulas, "close") {
                        check.target = CheckTargetModel::StaticNumber {
                            value: dv,
                            label: label.clone(),
                        };
                        ...
                        check
                            .advice_refs
                            .push(format!("formula.dv_bound:ranged_attack_dv.close={dv}"));
                    }
                }
            }
            ...
            if compiled_expr.is_none() && is_attack && check.dice_expression.trim() == "1d10" {
            ...
            check.ruling_status = if !check.source_refs.is_empty() {
            ...
            if matches!(check.target, CheckTargetModel::UnknownUntilLookup) && is_tech {
                if let Some(dv) = tech_dv_from_config(module_cfg, input.user_input) {
                    check.target = CheckTargetModel::StaticNumber {
                        value: dv,
                        label: "source-backed module technical option DV".into(),
                    };
                    check.opposition = OppositionModel::StaticDc {
                        dc: dv,
                        label: "source-backed module technical option DV".into(),
                    };
                }
    ...
    async fn handle_active_frame(
        ...
        if is_exit_relation(intent.relation_to_active_frame) {
            return self
                .handle_exit_intent(input, frame, intent, intent_event)
                .await;
        }

        if matches!(
            intent.relation_to_active_frame,
            FrameRelation::HideToDisengage | FrameRelation::PauseAndObserve
        ) {
            return self
                .handle_pause_or_direction_gate(input, frame, intent, intent_event)
                .await;
        }
        ...
        if should_open_stalemate_gate(&frame) && !should_create_action_contract(&intent) {
            return self
                .open_stalemate_gate(input, frame, intent, intent_event, "no_decisive_progress")
                .await;
        }
        ...
        if should_open_reaction_gate(&intent) {
            let profile = self
                .profiles
                .resolve(input.ruleset_id, infer_mode_from_frame(&frame));
            if let Some(advice) = profile.reaction_windows.first().cloned() {
                let gate = make_reaction_gate(input, &frame, &advice);
                self.db.insert_interaction_gate(&gate).await.ok();
                InteractionLifecycleKernel::new(self.db.clone())
        ...
        if should_create_action_contract(&intent) {
            ...
            if let Some(event) = &novelty_event {
                update_combat_working_state_event(&mut frame, &event.event_id);
            }
            self.db.upsert_state_frame(&frame).await?;
            let mut result = ConflictTurnResult::handled();
            ...
            result.events.push(intent_event);
            result.events.push(event);
            if let Some(novelty_event) = novelty_event {
                result.events.push(novelty_event);
            }
        ...
        if matches!(
            intent.relation_to_active_frame,
            FrameRelation::InvalidOrAmbiguous
                | FrameRelation::Clarification
            ...
            return self
                .open_stalemate_gate(
                    ...
                    intent,
                    intent_event,
                    "ambiguous_or_outside_active_frame",
                )
                .await;
        }
    ...
    async fn handle_exit_intent(
        &self,
        input: ConflictTurnInput<'_>,
        mut frame: StateFrame,
        intent: ConflictIntent,
        intent_event: FrameEvent,
    ) -> Result<ConflictTurnResult> {
        ...
        if closes_frame {
            ...
            let compaction = self
                .compact_frame(
                    &frame,
                    &format!(
                        "Frame closed by semantic exit contract: {}",
                        exit.success_outcome.as_str()
                    ),
                )
                .await?;
            InteractionLifecycleKernel::new(self.db.clone())
    ...
    async fn handle_pause_or_direction_gate(
        &self,
        input: ConflictTurnInput<'_>,
        mut frame: StateFrame,
        intent: ConflictIntent,
        ...
        let stalemate =
            make_stalemate_contract(&frame, "player_paused_or_hid_without_resolving_frame");
        let gate = make_direction_gate(
            input,
            &frame,
            &stalemate,
            "局势停住了。你是在继续躲藏观察、尝试脱离、谈判，还是重新采取行动？",
        );
        self.db.insert_interaction_gate(&gate).await.ok();
        InteractionLifecycleKernel::new(self.db.clone())
            .attach_gate_to_active_frame(input.session_id, &gate.gate_id, Some(&frame.frame_id))
            .await
            .ok();
        let gate_event = self
            .write_frame_event(
    ...
    async fn create_frame(
        &self,
        input: ConflictTurnInput<'_>,
        profile: &RulesetCombatProfile,
        mode: CombatMode,
        intent: Option<ConflictIntent>,
    ) -> Result<StateFrame> {
        ...
        let pc_hp = match (kernel.as_ref(), hp_id.as_ref()) {
            (Some(k), Some(id)) => {
                self.db
                    .load_resource_current(input.session_id, &actor_id, id, k)
                    .await
            }
            _ => None,
        };
        let npc_hp = match (kernel.as_ref(), hp_id.as_ref()) {
            (Some(k), Some(id)) => {
                self.db
                    .load_resource_current(input.session_id, &npc_actor_id, id, k)
                    .await
            }
            _ => None,
        };
        let working = CombatWorkingState {
            ...
            initiative_order: vec![
                InitiativeSlot {
                    ...
                    display_name: Some("Current PC".into()),
                    initiative_total: None,
                    acted_this_round: false,
                    metadata: json!({"source":"runtime_parameter_hydrator_v1_8"}),
                },
                InitiativeSlot {
                    actor_id: npc_actor_id.clone(),
            ...
            participants: vec![
                CombatParticipantState {
                    ...
                    combat_status: CombatantStatus::Active,
                    morale: Some(100),
                    patience: Some(100),
                    resources: pc_params
                        .as_ref()
                        .map(|p| p.status_json.clone())
                        .unwrap_or_else(
                            || json!({"source_policy":"unresolved_no_synthetic_default"}),
                        ),
                    ..Default::default()
                },
                CombatParticipantState {
            ...
            sides: vec![
                CombatSide {
                    side_id: "pcs".into(),
                    label: "Player characters".into(),
                    actor_ids: vec![actor_id.clone()],
                    disposition: "allied".into(),
                },
                CombatSide {
                    side_id: "opposition".into(),
                    label: "Opposition".into(),
                    actor_ids: vec![npc_actor_id.clone()],
                    disposition: "hostile_or_uncertain".into(),
                },
            ],
            zones: vec![CombatZone {
                zone_id: "zone.scene".into(),
                label: "Current scene".into(),
                summary: "The active situation location. Refine with module/search hits as needed."
                    .into(),
                tags: vec!["scene".into()],
                metadata: json!({}),
            }],
            range_model: RangeModel {
                model_id: "range.theater".into(),
                kind: "theater_of_mind".into(),
                notes: "Use ruleset profile/search hits for exact distances when needed.".into(),
            },
            action_economy: ActionEconomyState {
                profile_id: profile.profile_id.clone(),
                actor_slots: Default::default(),
                notes: profile.action_economy.to_string(),
            },
            ...
            last_events: vec![],
            progress_tracker: FrameProgressTracker {
                turns_elapsed: 0,
                turns_since_decisive_change: 0,
                rounds_since_decisive_change: 0,
                ..Default::default()
            },
        ...
        let frame = StateFrame {
            ...
            scope: Scope {
                ...
                scope_id: input.session_id.to_string(),
            },
            status: FrameStatus::Active,
            title: format!("{} situation frame", input.ruleset_id),
            objective:
                "Track transient situation state and compact durable consequences after resolution."
                    .into(),
        ...
        self.db.upsert_state_frame(&frame).await?;
        InteractionLifecycleKernel::new(self.db.clone())
            .open_context_for_frame(&frame, InteractionContextKind::Frame)
            .await
            .ok();
        Ok(frame)
    }

    async fn write_frame_event(
        &self,
        frame: &StateFrame,
        turn_id: &str,
        kind: &str,
        ...
        let event = FrameEvent {
            event_id: format!("frame_event_{}", Uuid::new_v4().simple()),
    ...
    pub async fn compact_frame(&self, frame: &StateFrame, reason: &str) -> Result<FrameCompaction> {
        let events = self
            .db
            .list_frame_events(&frame.frame_id, 80)
            .await
            .unwrap_or_default();
        ...
        if frame.local_facts.is_empty() {
            ...
            for fact in &frame.local_facts {
                summary.push_str(&format!("- {}\n", fact.summary));
            }
        }
        if let Ok(working) =
            serde_json::from_value::<CombatWorkingState>(frame.working_state.clone())
        {
            if let Some(outcome) = working.current_outcome {
                summary.push_str(&format!("- Final outcome: {}\n", outcome.as_str()));
            }
            summary.push_str(&format!(
                "- Transient turns elapsed: {}\n",
        ...
        let compaction = FrameCompaction {
            compaction_id: format!("frame_compaction_{}", Uuid::new_v4().simple()),
            frame_id: frame.frame_id.clone(),
            session_id: frame.session_id.clone(),
            summary_markdown: summary,
            persistent_world_patches: vec![],
            promoted_fact_ids: frame
                .local_facts
                .iter()
                .map(|f| f.fact_id.clone())
                .collect(),
            archived_event_ids: events.iter().map(|e| e.event_id.clone()).collect(),
            created_at: Utc::now(),
        };
...
fn normalize_semantic_hint_for_frame(mut hint: ConflictIntent, active: bool) -> ConflictIntent {
    ...
    if !hint
        .evidence_terms
        .iter()
        .any(|e| e == "turn_orchestrator_semantic_hint")
    {
        hint.evidence_terms
            .push("turn_orchestrator_semantic_hint".into());
    }
    hint
}

fn classify_situation_intent(
    input: ConflictTurnInput<'_>,
    active_frame: Option<&StateFrame>,
    investigate_opens_frame: bool,
) -> ConflictIntent {
    ...
    let lexical_attack = lexical_any(
        text,
        &[
            "拔枪",
            "还击",
    ...
    let lexical_under_attack = lexical_any(
        text,
        &[
            "朝我开火",
            "对我开火",
    ...
    let lexical_scene_conflict = lexical_any(
        text,
        &[
            "进入战斗",
            "战斗开始",
    ...
    let attack = if lexical_attack {
        attack.max(0.74)
    } else {
        attack
    };
    let under_attack = if lexical_under_attack {
        under_attack.max(0.74)
    } else {
        under_attack
    };
    let scene_conflict = if lexical_scene_conflict {
        scene_conflict.max(0.74)
    } else {
        scene_conflict
    };
    ...
    let mut scored = vec![
        ...
        (
            SituationActionKind::EnemyInitiatedConflict,
            under_attack.max(scene_conflict),
        ),
    ...
    let (action, score) = scored
        .first()
        .copied()
        .unwrap_or((SituationActionKind::Unknown, 0.0));

    let relation = if active {
        if surrender >= 0.48 {
            FrameRelation::Surrender
        } else if deescalate >= 0.46 {
            FrameRelation::DeescalationAttempt
        } else if flee >= 0.46 {
            FrameRelation::Flee
        } else if hide >= 0.46 {
            FrameRelation::HideToDisengage
        } else if observe >= 0.58 {
            FrameRelation::PauseAndObserve
        } else if under_attack >= 0.40 || scene_conflict >= 0.40 {
            FrameRelation::GateResponse
        } else if attack >= 0.42
            || defend >= 0.45
            || hack >= 0.48
        ...
        if attack >= 0.42
            ...
            || hack >= 0.56
            || anomaly >= 0.50
            || investigate_opens_frame_relation(investigate_opens_frame, investigate)
        {
            FrameRelation::InsideFrameAction
        } else {
            FrameRelation::OutsideFrameAction
    ...
    if attack >= 0.42 {
        evidence.push(format!("attack:{attack:.2}"));
    }
    if under_attack >= 0.40 {
        evidence.push(format!("under_attack:{under_attack:.2}"));
    }
    if scene_conflict >= 0.40 {
        evidence.push(format!("scene_conflict:{scene_conflict:.2}"));
    }
    if flee >= 0.42 {
        evidence.push(format!("flee:{flee:.2}"));
    }
    if hide >= 0.42 {
        evidence.push(format!("hide_disengage:{hide:.2}"));
    }
    if deescalate >= 0.42 {
        evidence.push(format!("deescalate:{deescalate:.2}"));
    }
    if surrender >= 0.42 {
        evidence.push(format!("surrender:{surrender:.2}"));
    }
    if hack >= 0.42 {
        evidence.push(format!("hack:{hack:.2}"));
    }
    if investigate >= 0.42 {
        evidence.push(format!("investigate:{investigate:.2}"));
    }
    if anomaly >= 0.42 {
        evidence.push(format!("anomaly:{anomaly:.2}"));
    }
    ...
    ConflictIntent {
        ...
        action_kind: action,
        escalation_level: if attack > 0.6 || under_attack > 0.6 || scene_conflict > 0.6 {
            EscalationLevel::High
        } else if attack > 0.42 || under_attack > 0.40 || scene_conflict > 0.40 || hack > 0.55 {
...
fn should_start_frame(low_confidence_frame_start: bool, intent: &ConflictIntent) -> bool {
    matches!(
        intent.relation_to_active_frame,
        FrameRelation::InsideFrameAction | FrameRelation::GateResponse
    ) && matches!(
        intent.action_kind,
        SituationActionKind::Attack
            | SituationActionKind::UnderAttack
...
fn infer_desired_outcome(
    ...
    match relation {
        ...
        _ => match action {
            ...
            | SituationActionKind::EnemyInitiatedConflict
            | SituationActionKind::SceneEntersConflict => {
                Some("survive and respond to the immediate threat")
            }
            SituationActionKind::Hack => Some("gain control or disable a technical threat"),
            SituationActionKind::InvestigateDuringConflict => {
                Some("gain actionable information under pressure")
            }
            _ => None,
        },
    }
...
fn make_direction_gate(
    input: ConflictTurnInput<'_>,
    frame: &StateFrame,
    stalemate: &StalemateContract,
    prompt_prefix: &str,
) -> InteractionGate {
    let now = Utc::now();
    InteractionGate {
...
fn make_combat_check_contract(
    ...
    let defender_actor = target_actor.clone().unwrap_or_else(|| ActorRef {
    ...
    if matches!(
        ...
        if let Some(value) = env_i32("TRPG_COMBAT_DEFAULT_ATTACK_DV") {
            CheckTargetModel::StaticNumber {
                value,
                label: "table-configured attack DV override".into(),
            }
        } else {
...
fn combat_opposition_for_intent(
    action_kind: &SituationActionKind,
    defender_actor: ActorRef,
) -> OppositionModel {
    if matches!(
        action_kind,
        SituationActionKind::Attack | SituationActionKind::Counterattack
        ...
        if let Some(score) = env_i32("TRPG_CONTEST_DEFAULT_ATTACK_DV") {
            OppositionModel::PassiveDefense {
                defender: defender_actor,
                passive_score_label: "table-configured attack defense/DV override".into(),
                passive_score: score,
            }
        } else {
        ...
        if let Some(dc) = env_i32("TRPG_CONTEST_DEFAULT_DEFENSE_DV") {
            OppositionModel::StaticDc {
                dc,
                label: "table-configured defense/reaction DV override".into(),
            }
        } else {
            OppositionModel::NoMechanicalOpposition
        }
...
fn make_effect_contract(
    ...
    let kind = match intent.action_kind {
        ...
        | SituationActionKind::Dodge
        | SituationActionKind::TakeCover
        | SituationActionKind::Hide => EffectKind::PositionChange,
        SituationActionKind::Hack | SituationActionKind::DisableDevice => {
            EffectKind::NarrativeConsequence
        }
        SituationActionKind::Negotiate | SituationActionKind::Intimidate => {
            EffectKind::MoraleChange
        }
    ...
    EffectContract {
        ...
        effect_kind: kind,
        target_actor_ids: check
            .target_actor
            .as_ref()
            .map(|a| vec![a.actor_id.clone()])
            .unwrap_or_default(),
        source_refs: check.source_refs.clone(),
        learned_packet_ids: check.learned_packet_ids.clone(),
        deterministic_parts: vec![ResolvedEffectPart {
            label: "pending_resolution".into(),
            value_json: json!({"check_id": check.check_id.clone(), "action_kind": intent.action_kind.as_str()}),
            source_refs: vec![],
        }],
        pending_rolls: vec![EffectRollRequest {
            label: check.check_label.clone(),
            dice_expression: check.dice_expression.clone(),
            roll_visibility: check.roll_visibility,
            target_actor_id: check.target_actor.as_ref().map(|a| a.actor_id.clone()),
        }],
...
fn make_exit_contract(
    ...
    let (exit_kind, success, response) = match intent.relation_to_active_frame {
        FrameRelation::Surrender => (
            ExitKind::Surrender,
            FrameOutcome::SurrenderAccepted,
            OpponentResponsePolicy::MayRefuse,
        ),
        FrameRelation::DeescalationAttempt => (
            ExitKind::Negotiate,
            FrameOutcome::NegotiatedTruce,
        ...
        FrameRelation::Flee | FrameRelation::ExitAttempt => (
            ExitKind::FleeArea,
            FrameOutcome::PlayerEscaped,
            OpponentResponsePolicy::MayPursue,
        ),
        FrameRelation::HideToDisengage => (
            ExitKind::HideAndDisengage,
            FrameOutcome::Paused,
            OpponentResponsePolicy::MayPursue,
        ),
        FrameRelation::PauseAndObserve => (
            ExitKind::TakeCoverAndPause,
            FrameOutcome::Paused,
            OpponentResponsePolicy::NoResponseNeeded,
        ),
        _ => (
            ExitKind::LeaveScene,
            FrameOutcome::Closed,
...
fn make_stalemate_contract(frame: &StateFrame, reason: &str) -> StalemateContract {
    let tracker = serde_json::from_value::<CombatWorkingState>(frame.working_state.clone())
        .ok()
        .map(|w| w.progress_tracker)
        .unwrap_or_default();
    StalemateContract {
    ...
    match frame.frame_kind {
        ...
        FrameKind::InvestigationNode => vec![
            action_option(
                "change_method",
                "换一种调查方法",
                "换技能、工具、对象或角度，不再重复同一动作。",
            ),
            action_option(
                "deep_search_cost",
                "花费时间/资源深查",
                "承担时间、风险或资源成本，换取更深层信息。",
            ),
            action_option(
                "leave_node",
                "离开这个节点",
                "接受当前信息，转向其他地点/NPC/线索。",
            ),
        ],
        _ => vec![
            action_option(
...
fn default_objectives_for_mode(mode: CombatMode) -> Vec<CombatObjective> {
    ...
    vec![CombatObjective {
        objective_id: "objective.resolve_situation".into(),
...
fn default_npc_drive_for_mode(mode: CombatMode) -> NpcDriveState {
    ...
    NpcDriveState {
        npc_id: "npc.opposition".into(),
        current_goal: "press the current scene objective".into(),
        current_tactic: "initial_pressure".into(),
        patience,
        morale,
        aggression,
        risk_tolerance: 45,
...
fn update_combat_working_state_event(frame: &mut StateFrame, event_id: &str) {
    if let Ok(mut working) =
        serde_json::from_value::<CombatWorkingState>(frame.working_state.clone())
    {
        working.last_events.push(event_id.to_string());
        if working.last_events.len() > 12 {
            working.last_events.remove(0);
        }
        frame.working_state =
            serde_json::to_value(working).unwrap_or_else(|_| frame.working_state.clone());
    }
    frame.event_count += 1;
    frame.last_event_ids.push(event_id.to_string());
    if frame.last_event_ids.len() > 12 {
        frame.last_event_ids.remove(0);
    }
    frame.updated_at = Utc::now();
}
...
fn update_combat_working_state_outcome(frame: &mut StateFrame, outcome: FrameOutcome) {
        ...
        serde_json::from_value::<CombatWorkingState>(frame.working_state.clone())
    {
        ...
        working.phase = match outcome {
            ...
            | FrameOutcome::SurrenderAccepted
            | FrameOutcome::Closed
            | FrameOutcome::Abandoned => CombatPhase::CombatEndPendingCompaction,
...
fn update_combat_working_state_intent_and_progress(
...
fn update_npc_drive_after_turn(frame: &mut StateFrame, intent: &ConflictIntent, decisive: bool) {
        ...
        serde_json::from_value::<CombatWorkingState>(frame.working_state.clone())
    {
        for drive in &mut working.npc_drive_states {
            ...
            if matches!(
                intent.relation_to_active_frame,
                FrameRelation::DeescalationAttempt | FrameRelation::Surrender
            ) {
                drive.aggression = (drive.aggression - 12).max(0);
            }
            if matches!(
                intent.action_kind,
                SituationActionKind::Attack | SituationActionKind::Intimidate
            ) {
        ...
        .map(|working| {
            working.progress_tracker.turns_since_decisive_change >= 3
                ...
                || working
                    .npc_drive_states
                    .iter()
                    .any(|d| d.patience <= 0 || d.repeated_strategy_count >= 3)
        })
        .unwrap_or(false)
}

fn apply_novelty_director(
    frame: &mut StateFrame,
    intent: &ConflictIntent,
    turn_id: &str,
    decisive: bool,
    ...
    let Ok(mut working) = serde_json::from_value::<CombatWorkingState>(frame.working_state.clone())
    else {
        return decision;
    };
    let actor_id = working
        .npc_drive_states
        .first()
        .map(|d| d.npc_id.clone())
        .unwrap_or_else(|| "npc.opposition".into());
    let previous = working
        .npc_drive_states
        .first()
        .map(|d| d.current_tactic.clone())
        .unwrap_or_else(|| "initial_pressure".into());
    let repeated_action = working.progress_tracker.repeated_action_count >= 1;
    let repeated_npc = working
        .npc_drive_states
        .first()
        .map(|d| d.repeated_strategy_count >= 1)
        .unwrap_or(false);
    let force_shift = repeated_action
        || repeated_npc
        || matches!(
            intent.action_kind,
            SituationActionKind::Attack
    ...
    let selected = if force_shift {
        select_next_tactic(&working, &actor_id, &previous, intent)
    } else {
        previous.clone()
    };
    if selected != previous || force_shift {
        ...
        decision.reason = if repeated_action {
            "player_repeated_action_world_must_change".into()
        } else if repeated_npc {
            "npc_tactic_repetition_prevented".into()
        } else {
            "enemy_or_scene_pressure_requires_fresh_response".into()
        };
        let change = FreshChange {
            change_id: format!("fresh_{}", Uuid::new_v4().simple()),
            change_type: if repeated_action {
                FreshChangeType::ContinuedPressureWithCost
            } else {
                FreshChangeType::NpcTacticShift
            },
        ...
        for drive in &mut working.npc_drive_states {
            if drive.npc_id == actor_id {
                ...
                drive.repeated_strategy_count = if selected == previous {
                    drive.repeated_strategy_count.saturating_add(1)
                } else {
                    0
                };
            }
        ...
        for mem in &mut working.npc_tactic_memory {
            if mem.npc_id == actor_id && mem.tactic_id == selected {
                ...
                mem.consecutive_count = if selected == previous {
                    mem.consecutive_count.saturating_add(1)
                } else {
                    1
                };
        ...
        if !found {
            working.npc_tactic_memory.push(NpcTacticMemory {
                npc_id: actor_id.clone(),
                tactic_id: selected.clone(),
                used_count_in_frame: 1,
                consecutive_count: 1,
                last_result: Some(decision.reason.clone()),
                last_turn_id: Some(turn_id.into()),
            });
        }
        working.novelty_state.recent_beats.push(beat);
        if working.novelty_state.recent_beats.len() > 8 {
            working.novelty_state.recent_beats.remove(0);
        }
        working.novelty_state.last_fresh_change = Some(change);
        working.novelty_state.forced_tactic_shift_count = working
            .novelty_state
            .forced_tactic_shift_count
            .saturating_add(1);
    }
    frame.working_state =
        serde_json::to_value(working).unwrap_or_else(|_| frame.working_state.clone());
...
fn select_next_tactic(
    working: &CombatWorkingState,
    actor_id: &str,
    previous: &str,
    intent: &ConflictIntent,
) -> String {
    let palette = working
        ...
        .or_else(|| working.tactic_palettes.first());
    let Some(palette) = palette else {
        return fallback_tactic_for_intent(intent, previous);
    };
    let recent: BTreeSet<String> = working
        .novelty_state
        .recent_beats
        .iter()
        .rev()
        .take(3)
        .map(|b| b.tactic_id.clone())
        .collect();
    let mut candidates: Vec<&NpcTactic> = palette
        .tactics
        .iter()
        .filter(|t| t.tactic_id != previous && !recent.contains(&t.tactic_id))
        .collect();
    if candidates.is_empty() {
        candidates = palette
            .tactics
            .iter()
            .filter(|t| t.tactic_id != previous)
            .collect();
    }
    if candidates.is_empty() {
        return fallback_tactic_for_intent(intent, previous);
    ...
    let preferred_tag = match intent.action_kind {
        ...
        | SituationActionKind::Intimidate => "social_pressure",
        SituationActionKind::Hack | SituationActionKind::DisableDevice => "objective",
        _ => "pressure",
    };
    candidates
        .iter()
        .find(|t| t.novelty_tags.iter().any(|tag| tag == preferred_tag))
        .copied()
...
fn fallback_tactic_for_intent(intent: &ConflictIntent, previous: &str) -> String {
    let fallback = match intent.action_kind {
        SituationActionKind::Attack | SituationActionKind::Counterattack => {
            "shift_position_and_pressure_objective"
        }
        SituationActionKind::Hide | SituationActionKind::Flee => "pursue_or_cut_off_escape",
        SituationActionKind::Negotiate | SituationActionKind::Surrender => {
            "demand_terms_or_stand_down"
        }
...
fn fresh_change_summary(tactic_id: &str, intent: &ConflictIntent, decisive: bool) -> String {
    let pressure = if decisive {
        "玩家的动作产生了效果，但局势不会原地重复"
    } else {
        "局势没有明显推进，因此世界主动变化"
    };
    match tactic_id {
        "sweep_fire_to_cover" => {
            format!("{pressure}：敌方不再原地互射，而是扫向掩体边缘，迫使位置选择变得重要。")
        }
        "threaten_downed_npc" => {
            format!("{pressure}：敌方转向倒地 NPC 或弱势目标，把战斗从互射变成救援压力。")
        }
        "overheat_or_spark" => format!("{pressure}：设备开始过热或冒火花，现场新增环境危险。"),
        "retreat_to_anchor" => {
            format!("{pressure}：敌方缩向关键锚点/控制源，暴露新的目标但拉开距离。")
        }
        "call_backup_or_alarm" => {
            format!("{pressure}：敌方改变策略，尝试呼叫支援或推进警报 clock。")
        }
        "demand_terms_or_stand_down" => {
            format!("{pressure}：对方不继续重复攻击，而是提出条件或试图改变冲突形式。")
        }
        _ => format!(
            "{pressure}：NPC tactic shifts to `{}` in response to `{}`。",
            tactic_id,
...
fn default_tactic_palette_for_mode(mode: CombatMode) -> TacticPalette {
    let archetype = match mode {
        CombatMode::AnomalyEncounter => "anomaly",
        CombatMode::HorrorEncounter => "horror",
        CombatMode::SocialConflict => "social_opponent",
        _ => "combat_opposition",
    };
    let tactics = match mode {
        CombatMode::AnomalyEncounter => vec![
            ...
            tactic(
                "consume_scene_object",
                "吞噬/改写一个场景物",
                &["new_affordance", "hazard"],
                1,
            ),
            tactic(
                "force_agent_choice",
                "迫使特工选择代价",
                &["social_pressure", "objective"],
                1,
            ),
        ],
...
fn tactic(id: &str, label: &str, tags: &[&str], cooldown: u32) -> NpcTactic {
    NpcTactic {
        ...
        avoid_when: vec![],
        creates: vec![],
        novelty_tags: tags.iter().map(|s| s.to_string()).collect(),
        cooldown_after_use: cooldown,
    }
}
...
fn lexical_any(input: &str, terms: &[&str]) -> bool {
...
fn semantic_cluster_score(input: &str, examples: &[&str]) -> f32 {
    ...
    for example in examples {
        ...
        if denom > 0.0 {
            best = best.max(common_chars / denom * 0.72);
        }
    }
    best
}
...
fn default_profiles() -> CombatProfilePack {
    CombatProfilePack {
        profiles: vec![default_generic_profile()],
    }
}
...
fn generic_required_defense() -> ReactionAdvice {
    ReactionAdvice {
        advice_id: "generic.required_defense_choice.v1_3".into(),
        gate_kind: GateKind::RequiredReactionChoice,
        required: true,
        prompt_public: "你正受到可见攻击。选择反应：闪避/防御、反击，或承受。".into(),
        options: vec![
            ActionOption {
                option_id: "defend_or_dodge".into(),
                label: "闪避 / 防御".into(),
                meaning: "尝试避免或降低攻击影响。".into(),
                is_default: false,
                consequences: json!({}),
            },
            ActionOption {
                option_id: "counter_or_fight_back".into(),
                label: "反击".into(),
                meaning: "用攻击或对抗动作回应，可能改变结算。".into(),
                is_default: false,
                consequences: json!({}),
            },
            ActionOption {
                option_id: "take_the_hit".into(),
                label: "站着挨打".into(),
                meaning: "不消耗反应或不防御，直接进入伤害/后果。".into(),
                is_default: false,
                consequences: json!({}),
            },
        ],

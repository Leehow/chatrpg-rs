...
/// **L-Z 完成度扩面(smokeLY t1/t7 残留 SOFT)**:同一「既成状态被叙述改写」根的**完成程度**子维度——叙述把物体操作
/// 推到与既成完成度不符的程度。smokeLY 实证两子case:(a)t7「仅松脱、未确认切断」的缆线被叙述成「已断开的粗缆」
/// (未完成→夸成已完成);(b)t1「接口线已接入」被倒退叙述成「指尖准备接入」(已完成→倒退回进行中)。同 flag/同 overlay
/// 追加方向③覆盖,additive,OFF 仍字节等价。
pub(crate) const NARRATOR_STATE_PERSISTENCE: &str = "\
【既成状态忠实 · 持久不复活、不反转】本回合叙述里任何已被「连续性锚」或「本回合机械事实」确立的世界/环境/物体状态——它的有无、开关、通断、冷热温度、运转/停止、明灭、损坏/完好、在场/离场——都必须与既成事实**保持一致**,不得在叙述里被悄悄改写成不符的样子。这有**两个方向**都要守:① **不把已完成的变更倒回旧状态**:既成事实记录某状态已**变更/终止**(供电切断、电流/嗡鸣/震动停止、灯熄、火灭、门开、设备关停、警报消音、某物被破坏移除)后,就按其**新状态**续写,绝不再把它**变更前的旧状态**当作仍在持续而复活(既定「供电已切断、粗缆嗡鸣停止」后就不要再写那根缆'仍有低沉电流嗡鸣传来')。② **不把仍持续的既成状态反转**:既成事实确立某状态**仍在持续**(粗缆仍在发热、设备仍在运转、某门仍开着)时,就不要在叙述里把它写成它的反面(既定粗缆'仍从设备暗处带出热量、正在发热'就不要写成'隔着布料仍能感觉到外皮硬冷')。残留的**物理痕迹**可以如实描写(切断后的缆线仍有余温、焦味、断口),但不要借此把**已终止的活动状态**(通电、运转、发声)写成死灰复燃,也不要拿它否定一个**仍持续的状态**。若确有一个**另外的、新的**来源产生类似现象(更深处另有一台仍通电的设备),须明确叙述为一个**区别于既成那个状态的新来源**,而非让旧状态本身复活或反转;且不得为此凭空捏造既成事实之外的来源。世界状态只朝既成事实记录的方向走,绝不在叙述里被无声地倒回或反转。③ **不把未完成的操作叙述成已完成、也不把已完成的操作倒退成进行中**:物体/世界状态的改变常是**分程度、分阶段**的——一根缆线可能被「刮出痕/松动了一截/部分剥开」但**尚未确认切断**,一条接口线可能「正在对位/指尖刚搭上」但**尚未接入**,一扇门可能「撬开一道缝」但**尚未打开**。叙述必须严格按「连续性锚」与「本回合机械事实」所确立的**那个完成程度**来写,不得擅自把它**推到下一个程度**:(a)既成事实只记某操作为**部分/尝试中/未确认成功**(仅松脱、未确认切断、只刮浅痕、尚未接入、未完全开启)时,绝不在叙述里把它写成**已彻底完成**的样子(写成「已断开的粗缆」「已接入」「门已敞开」)——它当下是什么程度就写什么程度,要推进到完成须经下一回合的真实机械结果;(b)反过来,既成事实已记某操作**刚刚完成**(接口线已接入、缆已切断、门已开)后,也绝不在下一句或下一回合把它**倒退回**「正准备做/指尖刚要搭上/还没接好」的样子——已完成就保持完成,不在叙述里无声地把进度往回拨。完成度只随既成机械结果前进,不夸张、不倒退。";

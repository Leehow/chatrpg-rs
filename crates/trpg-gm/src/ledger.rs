...
use trpg_model::{CheckContract, CheckResultRecord, EffectContract, InteractionGate, ParameterImpact, RollVisibility};
...
impl TurnLedger {
    ...
    /// 滑动缓冲过滤集合（收窄版，lowercase 去重）：仅 roll_id 与原始点数
    /// （result/outcome JSON 中的数值 token）。不再逐字复用
    /// private_roll_leak_tokens——其含通用 JSON 键名（"total"/"success"/"band"）
    /// 与骰式表达式（"1d100"），子串替换会把叙事 "successfully" 涂成 "■fully"。
    /// 泄密方向仍 fail-closed：点数与 roll_id 全过滤；短数字误伤由
    ...
    pub fn private_roll_tokens(&self) -> Vec<String> {
        ...
        for result in &self.snapshot.check_results {
            if result.roll.visibility != RollVisibility::PrivateGmRoll { continue; }
            push(result.roll.roll_id.clone());
            collect_numeric_tokens(&result.roll.result, &mut push);
            collect_numeric_tokens(&result.outcome, &mut push);
        }
        for roll in &self.snapshot.dice_rolls {
            if roll.visibility != RollVisibility::PrivateGmRoll { continue; }
...
fn collect_numeric_tokens(value: &Value, sink: &mut impl FnMut(String)) {
    match value {
        ...
        Value::Array(items) => for item in items { collect_numeric_tokens(item, sink); },
        Value::Object(map) => for item in map.values() { collect_numeric_tokens(item, sink); },
        _ => {}
    }
}
...
mod tests {
    ...
    fn record_execution_records_primary_followups_and_rolls() {
        ...
        // （result/outcome JSON 数值；outcome "success" 是 Bool 不产 token）。
        // 通用 JSON 键名（"total"/"success"/"band"）与骰式表达式（"1d100"）
        // 一律剔除——它们是叙事高频词，子串替换会把 "successfully" 涂成 "■fully"。
        assert_eq!(ledger.private_roll_tokens(), vec!["roll_b".to_string(), "34".to_string()]);
        assert!(!ledger.private_roll_tokens().iter().any(|t| t == "total" || t == "success" || t == "1d100" || t == "band"));

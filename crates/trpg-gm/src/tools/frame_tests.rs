...
use std::sync::Mutex;
use trpg_db::Db;
use trpg_model::{ContextRequest, FrameKind, FrameStatus, RuntimeState, TokenBudget, VisibilityProfile};
use trpg_runtime::RuntimeEngine;

fn temp_data_dir() -> PathBuf {
    let dir = std::env::temp_dir().join(format!("gm_frame_test_{}_{}", std::process::id(), uuid::Uuid::new_v4().simple()));
...
async fn cleanup(engine: &RuntimeEngine, session_id: &str) {
    let _ = sqlx::query("delete from frame_compactions where session_id = $1").bind(session_id).execute(&engine.db.pool).await;
    let _ = sqlx::query("delete from frame_events where session_id = $1").bind(session_id).execute(&engine.db.pool).await;
    let _ = sqlx::query("delete from interaction_contexts where session_id = $1").bind(session_id).execute(&engine.db.pool).await;
    let _ = sqlx::query("delete from state_frames where session_id = $1").bind(session_id).execute(&engine.db.pool).await;
}
...
async fn open_combat_frame_validates_arguments() {
    ...
    let err = typed_err(OpenCombatFrameTool.call(&ctx, &mut ledger, json!({"participants":[],"stakes":"伏击"})).await);
    assert_eq!(err.code, "invalid_arguments");
...
async fn open_combat_frame_creates_live_combat_frame_with_bp3_projection() {
    ...
    let frame_id = output.result.get("frame_id").and_then(|v| v.as_str()).expect("frame_id in output").to_string();
    // db 真有一个 live Combat frame。
    let frames = engine.db.list_active_state_frames(&request.session_id, 8).await.unwrap();
    ...
    assert_eq!(frame.working_state.pointer("/participants/1").and_then(|v| v.as_str()), Some("npc.ghoul"));
    assert_eq!(frame.working_state.get("stakes").and_then(|v| v.as_str()), Some("食尸鬼从墓穴扑出"));
    assert!(frame.working_state.get("npc_tactic_memory").is_some(), "tactic memory bucket must be seeded");
    ...
    assert!(rendered.contains("npc.ghoul"), "participants must reach the BP3 projection block: {rendered}");
    // 该 frame 即姿态投影：current_mode 推导为 combat。
    assert_eq!(crate::mode::current_mode(&frames), Some("combat".to_string()));
...
async fn open_combat_frame_enriches_existing_mode_frame_instead_of_duplicating() {
    ...
    OpenCombatFrameTool.call(&ctx, &mut ledger, json!({"participants":["pc.current","npc.cultist"],"stakes":"教徒拔刀"})).await.expect("open enriches");
    let after = engine.db.list_active_state_frames(&request.session_id, 8).await.unwrap();
    assert_eq!(after.len(), 1, "must enrich the existing combat frame, not duplicate");
    assert_eq!(after[0].frame_id, before[0].frame_id);
    assert_eq!(after[0].working_state.pointer("/participants/1").and_then(|v| v.as_str()), Some("npc.cultist"));
    // enter_mode 写入的进入语义被保留（merge 不覆盖无关键）。
    assert_eq!(after[0].working_state.get("entered_reason").and_then(|v| v.as_str()), Some("ambush"));
    cleanup(&engine, &request.session_id).await;
    fs::remove_dir_all(dir).ok();
}
...
async fn close_frame_compacts_closes_and_settles_exit_obligations() {
    ...
    assert_eq!(output.result.get("closed_frame_id").and_then(|v| v.as_str()), Some(frame_id.as_str()));
    let compaction_id = output.result.get("compaction_id").and_then(|v| v.as_str()).expect("compaction_id in output").to_string();
    // 压缩真落库（复用一期 conflict kernel 原语 insert_frame_compaction）。
    let count: i64 = sqlx::query_scalar("select count(*)::bigint from frame_compactions where compaction_id = $1 and frame_id = $2")
        .bind(&compaction_id).bind(&frame_id).fetch_one(&engine.db.pool).await.unwrap();
    assert_eq!(count, 1, "compaction row must exist");
    // frame 已关：active 列表为空，下一回合头部回落叙事姿态。
    assert!(engine.db.list_active_state_frames(&request.session_id, 8).await.unwrap().is_empty());
    // close_frame 成功 = combat 退出义务的确定性闭合事件 → exit_mode 放行。
    assert!(cell.lock().unwrap().exit_blocking().is_empty(), "close_frame must settle combat mode_exit obligations");
    let exit = crate::mode::ExitModeTool.call(&ctx, &mut ledger, json!({"reason":"战斗落幕"})).await.expect("exit_mode passes after close_frame");
    assert_eq!(exit.result.get("exited_mode").and_then(|v| v.as_str()), Some("combat"));
    cleanup(&engine, &request.session_id).await;
    fs::remove_dir_all(dir).ok();
}
...
fn novelty_block_renders_used_tactics_and_is_fail_closed() {
    let mk = |ws: serde_json::Value, status: FrameStatus| {
        ...
        trpg_model::StateFrame {
            frame_id: "f1".into(), frame_kind: FrameKind::Combat, session_id: "s".into(), ruleset_id: "rs".into(),
            module_id: None, parent_frame_id: None,
            scope: trpg_model::Scope { scope_type: trpg_model::ScopeType::Session, scope_id: "s".into() },
            status, title: "t".into(), objective: "o".into(), static_refs: vec![], working_state: ws,
    ...
    let block = novelty_block(&[mk(tactics.clone(), FrameStatus::Active)]).expect("tactic memory must render a block");
    ...
    assert!(novelty_block(&[mk(json!({"npc_tactic_memory": "garbage"}), FrameStatus::Active)]).is_none());
}
...
fn cluster_debt_opens_blocks_and_clears() {
    ...
    ledger.waive("cluster.t", "玩家中途逃跑，后果延后结算", WaiveScope::Turn).expect("cluster debt must be waivable");
    assert!(ledger.blocking().is_empty());
...
fn combat_manifest_loads_with_frame_tools_and_tight_tempo() {
    ...
    let manifest = crate::mode::load_mode_manifest(&data_dir, "combat").expect("combat manifest must load from data/");
    assert_eq!(manifest.mode_id, "combat");
    assert_eq!(manifest.frame_kind, "combat");
    assert_eq!(manifest.extra_tools, vec!["open_combat_frame".to_string(), "close_frame".to_string()]);
    assert_eq!(manifest.tempo.effect_closure_per_cluster, Some(true), "combat tempo must tighten cluster closure");
    assert!(!manifest.exit_obligations.is_empty(), "combat must declare exit obligations");
    // 目录过滤器选中战斗类条目、排除幕间/无关条目（与批3 downtime 夹具对偶）。
    let filter = &manifest.catalog_filter;
    assert!(filter.matches("combat_procedure", &["combat_round".to_string()], &["initiative".to_string()]), "filter must select combat entries: {filter:?}");
...
fn combat_mode_prompts_merge_for_both_rulesets() {
    ...
    assert!(dnd.contains("[combat-mode]"), "combat global.md marker missing for dnd5e");
    assert!(dnd.contains("[combat-mode:dnd5e]"), "dnd5e 战斗提示未并入");
    assert!(!dnd.contains("[combat-mode:call_of_cthulhu_7e]"), "ruleset 提示串扰");
    let coc = crate::prompts::load_gm_skill_with_mode(&data_dir, "call_of_cthulhu_7e", Some("combat")).unwrap();
    assert!(coc.contains("[combat-mode]"));
    assert!(coc.contains("[combat-mode:call_of_cthulhu_7e]"), "CoC 战斗提示未并入");
    assert!(!coc.contains("[combat-mode:dnd5e]"));
    // 呈现层硬约束必须写进电影化准则（评测 grep 断言的同源词）。
    assert!(dnd.contains("先攻表") || dnd.contains("轮次"), "电影化准则必须明令不报轮次表/先攻表");
}

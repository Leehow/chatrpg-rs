...
use trpg_model::{
    FrameKind, FrameStatus, InteractionContextKind, NpcTacticMemory, Scope, ScopeType, StateFrame,
};
use uuid::Uuid;
...
fn required_str_vec(args: &Value, key: &str, tool: &str) -> Result<Vec<String>> {
    let items = args.get(key).and_then(Value::as_array).ok_or_else(|| {
        ToolError::recoverable(
            "invalid_arguments",
            format!("{tool} requires '{key}' as an array of actor ids"),
            None,
        )
    })?;
    ...
    if out.is_empty() || out.len() != items.len() {
        return Err(ToolError::recoverable(
            "invalid_arguments",
            format!("{tool} '{key}' must be a non-empty array of non-empty strings"),
            None,
        ));
    }
    Ok(out)
...
pub fn novelty_block(frames: &[StateFrame]) -> Option<String> {
    ...
    for frame in frames.iter().filter(|f| frame_is_live(f)) {
        let Some(raw) = frame.working_state.get("npc_tactic_memory") else {
            continue;
        };
        ...
        for e in &entries {
            lines.push(format!(
                "- {}: 已用战术 {}（本帧 ×{}，连续 {} 次）",
                e.npc_id, e.tactic_id, e.used_count_in_frame, e.consecutive_count
            ));
        }
    }
    if lines.is_empty() {
...
impl GmTool for OpenCombatFrameTool {
    ...
    async fn call(
        ...
        let existing = frames
            .into_iter()
            .find(|f| frame_is_live(f) && matches!(f.frame_kind, FrameKind::Combat));
        let now = Utc::now();
        ...
        let mut ws: Map<String, Value> =
            frame.working_state.as_object().cloned().unwrap_or_default();
        ...
        ws.entry("npc_tactic_memory".to_string())
            .or_insert(json!([]));
        ...
        ctx.engine
            ...
            .map_err(|e| {
                ToolError::fatal(
                    "internal_error",
                    format!("failed to open combat frame: {e}"),
                )
            })?;
...
impl GmTool for CloseFrameTool {
    ...
    async fn call(
        ...
        let target = match ctx.current_mode {
            Some(mode) => frames
                .into_iter()
                .find(|f| frame_is_live(f) && f.frame_kind.as_str() == mode),
            None => frames
                .into_iter()
                .find(|f| frame_is_live(f) && matches!(f.frame_kind, FrameKind::Combat)),
        };
        ...
        let compaction = CombatAgent::from_env_or_default(ctx.engine.db.clone())
            ...
            .await
            .map_err(|e| {
                ToolError::fatal("internal_error", format!("frame compaction failed: {e}"))
            })?;
        ...
        InteractionLifecycleKernel::new(ctx.engine.db.clone())
            .close_frame(
                ...
                &frame.frame_id,
                format!("close_frame: {summary}"),
            )
            .await
            .map_err(|e| {
                ToolError::fatal("internal_error", format!("failed to close frame: {e}"))
            })?;

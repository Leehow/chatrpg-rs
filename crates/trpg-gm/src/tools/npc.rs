...
pub struct GetActorArgs {
    pub actor_id: String,
}
#[derive(Debug, Clone, Deserialize)]
pub struct EnsureNpcParamArgs {
    pub npc_id: String,
    pub bucket: String,
    pub param: String,
    pub context: String,
...
impl GmTool for GetActorTool {
    ...
    async fn call(
        ...
        let args: GetActorArgs = serde_json::from_value(args).map_err(|e| {
            ToolError::recoverable(
                "invalid_arguments",
                format!("get_actor arguments invalid: {e}"),
                None,
            )
        })?;
        ...
        Ok(ToolOutput::ok(
            json!({"actor_id": actor.actor_id, "display_name": actor.display_name, "sheet_json": actor.sheet_json, "mechanical_profile": actor.mechanical_profile, "status_json": actor.status_json}),
        ))
    }
}
...
impl GmTool for EnsureNpcParamTool {
    fn spec(&self) -> ToolSpec {
        ToolSpec {
            name: "ensure_npc_param",
            schema: json!({"type":"function","function":{"name":"ensure_npc_param","description":"Materialize an NPC parameter before contest resolution.","parameters":{"type":"object","properties":{"npc_id":{"type":"string"},"bucket":{"type":"string"},"param":{"type":"string"},"context":{"type":"string"}},"required":["npc_id","bucket","param","context"]}}}),
        }
    }
    async fn call(
        &self,
        ...
        let args: EnsureNpcParamArgs = serde_json::from_value(args).map_err(|e| {
            ToolError::recoverable(
                "invalid_arguments",
                format!("ensure_npc_param arguments invalid: {e}"),
                None,
            )
        })?;
        let module_id = ctx.request.module_id.as_ref().ok_or_else(|| {
            ToolError::recoverable(
                "no_module_loaded",
                "ensure_npc_param requires a module",
                None,
            )
        })?;
        let graph = ctx
        ...
        // R2 note: GM agent tool 的现搓属工具调用副作用（AI 决策驱动按需现搓），
        // 不经 NeedBus——NeedBus 收口的是回合头部 Rust 确定性发 Need（opposed_prepass
        // 的自动现搓已迁移到 EntityNeedResolver）。此处刻意保持直连 ensure_npc_parameter
        // （同理 tools/check.rs 的 opposed check 工具内现搓）。
        let value = ctx
            .engine
            ...
            .await?;
        Ok(ToolOutput::ok(
            json!({"npc_id": args.npc_id, "bucket": args.bucket, "param": args.param, "value": value}),
        ))
    }
}
...
mod tests {
    ...
    fn missing_npc_is_structured_npc_not_found_not_fail_open() {
        ...
        let tool = err
            ...
            .expect("must be structured ToolError");

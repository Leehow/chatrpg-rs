...
pub fn parse_apply_effect_args(value: Value) -> Result<ApplyEffectArgs> {
    let args: ApplyEffectArgs = serde_json::from_value(value).map_err(|e| {
        ToolError::recoverable(
            "invalid_arguments",
            format!("apply_effect arguments invalid: {e}"),
            None,
        )
    })?;
    let path_count = args
    ...
    if path_count != 1 {
        return Err(ToolError::recoverable(
            "invalid_arguments",
            "provide exactly one of parameter_path or track_id",
            Some(
                "Use parameter_path for actor fields or track_id for resources/tracks.".to_string(),
            ),
        ));
    }
    Ok(args)
...
pub fn parse_change_track_args(value: Value) -> Result<ChangeTrackArgs> {
    serde_json::from_value(value).map_err(|e| {
        ToolError::recoverable(
            "invalid_arguments",
            format!("change_track arguments invalid: {e}"),
            None,
        )
    })
}
...
pub fn effect_parameter_path(args: &ApplyEffectArgs) -> String {
        ...
        .filter(|s| !s.is_empty())
    {
        ...
        None => format!(
            "resources.{}.current",
            args.track_id.as_deref().unwrap_or_default().trim()
        ),
    }
}
...
impl GmTool for ApplyEffectTool {
    ...
    async fn call(
        ...
        Ok(ToolOutput::ok(
            json!({"effect_id": outcome.effect.effect_id, "impact_count": outcome.impacts.len(), "patch_count": outcome.patches.len()}),
        ))
    }
}
...
impl GmTool for ChangeTrackTool {
    fn spec(&self) -> ToolSpec {
        ToolSpec {
            name: "change_track",
            schema: json!({"type":"function","function":{"name":"change_track","description":"Apply growth or explicit track movement.","parameters":{"type":"object","properties":{"bucket":{"type":"string","enum":["tracks","stats","skills","field"]},"id":{"type":"string"},"op":{"type":"string","enum":["set","add"]},"amount":{"type":"number"},"value":{"type":"string"},"kind":{"type":"string"},"category":{"type":"string"}},"required":["bucket","id","op"]}}}),
        }
    }
    ...
    async fn call(
        ...
        let ok = ctx
            ...
            .await?;
        Ok(ToolOutput::ok(
            json!({"changed": ok, "bucket": args.bucket, "id": args.id}),
        ))
    }
}
...
mod tests {
    ...
    fn apply_effect_requires_one_target_path() {
        let err = parse_apply_effect_args(
            json!({"target_actor":"npc.opposition","op":"subtract","amount":3,"reason":"hit"}),
        )
        .unwrap_err();
    ...
    fn synthesized_track_path_resolves_against_sample_kernel() {
        ...
        let kernel = RuleKernel {
            resource_tracks: vec![
                json!({"id": "sanity", "name": "Sanity"}),
                json!({"id": "hit_points", "kind": "hp"}),
            ],
            ..Default::default()
        };
        ...
        assert_eq!(
            resolve_resource_track_id(&path, &kernel).as_deref(),
            Some("sanity")
        );
    }
    ...
    fn change_track_accepts_string_value_without_amount() {
        let args = parse_change_track_args(
            json!({"bucket":"field","id":"class","op":"set","value":"solo"}),
        )
        .unwrap();

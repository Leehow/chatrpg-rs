...
pub fn find_mechanic<'a>(catalog: &'a [MechanicEntry], id: &str) -> Option<&'a MechanicEntry> {
    ...
    catalog.iter().find(|e| e.id.trim().to_lowercase() == needle)
}
...
impl GmTool for LookupMechanicTool {
    fn spec(&self) -> ToolSpec {
        ToolSpec { name: "lookup_mechanic", schema: json!({"type":"function","function":{"name":"lookup_mechanic","description":"Fetch the full mechanics-catalog entry (procedure, hooks, followups, sources) by id from the BP1 index.","parameters":{"type":"object","properties":{"id":{"type":"string"}},"required":["id"]}}}) }
    }
    ...
    async fn call(&self, ctx: &ToolCtx<'_>, _ledger: &mut TurnLedger, args: Value) -> Result<ToolOutput> {
        let args: LookupArgs = serde_json::from_value(args)
            .map_err(|e| ToolError::recoverable("invalid_arguments", format!("lookup_mechanic arguments invalid: {e}"), Some("Provide the catalog entry id.".to_string())))?;
        if args.id.trim().is_empty() {
            return Err(ToolError::recoverable("invalid_arguments", "id is required", Some("Provide the catalog entry id from the BP1 mechanics index.".to_string())));
        }
        ...
        match entry {
            ...
            Some(e) => Ok(ToolOutput::ok(json!({"entry": e, "tier": tier_label(expressiveness_tier(e))}))),
...
fn parse_scope(raw: Option<&str>) -> Result<WaiveScope> {
    match raw.map(|s| s.trim().to_lowercase()) {
        ...
        Some(other) => Err(ToolError::recoverable("invalid_arguments", format!("unknown waive scope: {other}"), Some("Use \"turn\" or \"scene\".".to_string()))),
    }
}
...
impl GmTool for WaiveObligationTool {
    fn spec(&self) -> ToolSpec {
        ToolSpec { name: "waive_obligation", schema: json!({"type":"function","function":{"name":"waive_obligation","description":"Explicitly waive a pending mechanical obligation (due/check/debt) WITH a reason. Waivers are audited.","parameters":{"type":"object","properties":{"target_id":{"type":"string"},"reason":{"type":"string"},"scope":{"type":"string","enum":["turn","scene"],"default":"turn"}},"required":["target_id","reason"]}}}) }
    }

    async fn call(&self, ctx: &ToolCtx<'_>, _ledger: &mut TurnLedger, args: Value) -> Result<ToolOutput> {
        let args: WaiveArgs = serde_json::from_value(args)
            .map_err(|e| ToolError::recoverable("invalid_arguments", format!("waive_obligation arguments invalid: {e}"), Some("Provide target_id and a non-empty reason.".to_string())))?;
        let target_id = args.target_id.trim().to_string();
        if target_id.is_empty() {
            return Err(ToolError::recoverable("invalid_arguments", "target_id is required", Some("Use an id from the [obligations] observation.".to_string())));
        }
        // reason 空串拦截：waiver 是审计对象，无理由不豁免。
        let reason = args.reason.trim().to_string();
        if reason.is_empty() {
            return Err(ToolError::recoverable("invalid_arguments", "reason must not be empty: waivers are audited", Some("State why this obligation is being waived.".to_string())));
        }
        let scope = parse_scope(args.scope.as_deref())?;
        let Some(cell) = ctx.obligations else {
            return Err(ToolError::recoverable("obligation_not_found", "no obligation ledger is attached to this turn", Some("Handle the obligation via roll_check/apply_effect instead.".to_string())));
        };
        ...
        let (kind, record) = {
            ...
            let Some(view) = obligations.blocking().into_iter().find(|v| v.target_id == target_id) else {
                return Err(ToolError::recoverable("obligation_not_found", format!("no outstanding obligation with id: {target_id}"), Some("Use an id from the [obligations] observation.".to_string())));
            };
            let record = obligations.waive(&target_id, &reason, scope)
        ...
        if kind == "due" {
            if let Err(err) = ctx.engine.db.update_mechanic_due_status(&target_id, "waived", Some(record.reason.as_str()), Some(record.scope.as_str())).await {
        ...
        Ok(ToolOutput::ok(json!({"waived": {"target_id": record.target_id, "reason": record.reason, "scope": record.scope.as_str()}, "audit_tags": audit_tags})))
    }
}
...
mod tests {
    ...
    fn entry(id: &str) -> MechanicEntry {
        MechanicEntry { id: id.to_string(), name: id.to_string(), ..Default::default() }
    }
    ...
    async fn lookup_missing_id_is_mechanic_not_found() {
        ...
        assert!(tool_err.hint.as_deref().unwrap_or("").contains("retrieve_rules"));
    }
}

...
impl CatalogFilter {
    ...
    pub fn matches(&self, kind: &str, hooks: &[String], tags: &[String]) -> bool {
        ...
        self.kinds.iter().any(|k| k.eq_ignore_ascii_case(kind))
            || self.hooks.iter().any(|h| hooks.iter().any(|x| x.eq_ignore_ascii_case(h)))
            || self.semantic_tags.iter().any(|t| tags.iter().any(|x| x.eq_ignore_ascii_case(t)))
    }
}
...
pub fn current_mode(frames: &[StateFrame]) -> Option<String> {
    frames.iter().find(|f| frame_is_live(f)).map(|f| f.frame_kind.as_str().to_string())
}
...
pub fn load_mode_manifest(data_dir: &Path, mode: &str) -> Result<ModeManifest> {
    ...
    if !path.exists() {
        return Err(anyhow!("mode manifest missing: {} (mode '{mode}' is not installed under agent/gm_skill/modes)", path.display()));
    }
...
pub fn active_mode_manifest(data_dir: &Path, frames: &[StateFrame]) -> Result<Option<ModeManifest>> {
...
fn ctx_data_dir<'a>(ctx: &ToolCtx<'a>, tool: &str) -> Result<&'a Path> {
    ctx.data_dir.ok_or_else(|| ToolError::fatal("internal_error", format!("{tool} requires a data_dir on the tool context (assembler wiring bug)")))
}
...
impl GmTool for EnterModeTool {
    ...
    async fn call(&self, ctx: &ToolCtx<'_>, _ledger: &mut TurnLedger, args: Value) -> Result<ToolOutput> {
        ...
        let manifest = load_mode_manifest(data_dir, mode).map_err(|e| {
            ToolError::recoverable("mode_not_found", format!("mode '{mode}' unavailable: {e}"), Some("Only installed modes under agent/gm_skill/modes can be entered.".to_string()))
        })?;
        ...
        if let Some(active) = active_mode_manifest(data_dir, &frames).map_err(|e| ToolError::fatal("internal_error", e.to_string()))? {
        ...
        ctx.engine.db.upsert_state_frame(&frame).await
            .map_err(|e| ToolError::fatal("internal_error", format!("failed to open mode frame: {e}")))?;
...
impl GmTool for ExitModeTool {
    ...
    async fn call(&self, ctx: &ToolCtx<'_>, _ledger: &mut TurnLedger, args: Value) -> Result<ToolOutput> {
        ...
        let manifest = load_mode_manifest(data_dir, &mode_id).map_err(|e| ToolError::fatal("internal_error", format!("active mode '{mode_id}' has no loadable manifest: {e}")))?;
        let frames = ctx.engine.db.list_active_state_frames(&ctx.request.session_id, 8).await.unwrap_or_default();
        let live = frames.into_iter().find(|f| frame_is_live(f) && f.frame_kind.as_str() == manifest.frame_kind);
        let closed_frame_id = match live {
            Some(frame) => {
                trpg_interaction::InteractionLifecycleKernel::new(ctx.engine.db.clone())
                    .close_frame(&ctx.request.session_id, &frame.frame_id, format!("exit_mode: {reason}"))
                    .await
                    .map_err(|e| ToolError::fatal("internal_error", format!("failed to close mode frame: {e}")))?;
                Some(frame.frame_id)
            }
            None => None,
        };
        if let Some(cell) = ctx.obligations {
            cell.lock().unwrap_or_else(|p| p.into_inner()).clear_mode_exit_obligations(&mode_id);
        }
        Ok(ToolOutput::ok(json!({
            "exited_mode": mode_id,

...
use trpg_db::Db;
use trpg_model::{
    ContextRequest, DueSource, DueStatus, MechanicDue, RuntimeState, TokenBudget, VisibilityProfile,
};
...
fn due(id: &str, desc: &str) -> MechanicDue {
    MechanicDue {
        ...
        followup_procedure_id: Some("proc.followup".to_string()),
...
fn blocking_view_excludes_settled_and_waived() {
    ...
    ledger
        .waive("due_1", "narrative override", WaiveScope::Turn)
        .expect("due_1 exists");
...
async fn waive_unknown_target_errs() {
    ...
    ledger.begin_turn("t");
    assert!(ledger
        .waive("不存在的 id", "reason", WaiveScope::Turn)
        .is_err());
    ...
    let err = match crate::tools::mechanic::WaiveObligationTool
        .call(
            &ctx,
            &mut turn_ledger,
            json!({"target_id":"due_missing","reason":"r"}),
        )
        .await
    {
...
fn resolve_dues_for_mechanic_clears_matching_open_dues_only() {
    ...
    assert_eq!(
        remaining,
        vec!["due_other".to_string(), "due_unbound".to_string()]
    );
    // 再次结算同机制：无匹配 → 空（幂等）。
    assert!(ledger
        .resolve_dues_for_mechanic("coc.sanity_roll_and_loss")
        .is_empty());
}
...
fn scene_waivers_cleared_on_scene_change() {
    ...
    ledger
        .waive("due_scene", "calm scene", WaiveScope::Scene)
        .expect("due_scene exists");
    ledger
        .waive("check_turn", "narrative override", WaiveScope::Turn)
        .expect("check_turn exists");
    assert!(
        ledger.blocking().is_empty(),
        "both waived → nothing blocking"
    );
    ...
    assert_eq!(
        blocking.len(),
        1,
        "scene-waived due must block again: {blocking:?}"
    );
...
fn settle_retro_debts_consumes_each_effect_once_fifo() {
    ...
    ledger.absorb_retro_debts(vec![
        RetroactiveEffectDebt {
            debt_id: "debt_a".to_string(),
            turn_id: "t0".to_string(),
            finding_detail: "d".to_string(),
            kind: RetroDebtKind::Effect,
            created_at: Utc::now(),
        },
        RetroactiveEffectDebt {
            debt_id: "debt_b".to_string(),
            turn_id: "t0".to_string(),
            finding_detail: "d".to_string(),
            kind: RetroDebtKind::Effect,
            created_at: Utc::now(),
    ...
    assert_eq!(
        blocking.len(),
        1,
        "one effect settles exactly one debt: {blocking:?}"
    );
    assert_eq!(
        blocking[0].target_id, "debt_b",
        "FIFO: oldest debt settles first"
    );
...
fn check_debts_settle_only_with_checks_and_effects_dont_cross() {
    ...
    ledger.absorb_retro_debts(vec![
        RetroactiveEffectDebt {
            debt_id: "debt_eff".to_string(),
            turn_id: "t0".to_string(),
            finding_detail: "claimed damage".to_string(),
            kind: RetroDebtKind::Effect,
            created_at: Utc::now(),
        },
        RetroactiveEffectDebt {
            debt_id: "debt_chk".to_string(),
            turn_id: "t0".to_string(),
            finding_detail: "missed sanity roll".to_string(),
            kind: RetroDebtKind::Check,
            created_at: Utc::now(),
        },
    ]);
    ...
    assert!(
        summaries.iter().any(|s| s.contains("roll_check")),
        "check debt must hint roll_check: {summaries:?}"
    );
...
fn carryover_block_lists_unresolved_dues_with_evidence() {
    ...
    let block = ledger
        .carryover_block()
        .expect("unresolved dues must carry over");
    // 含 threshold_desc 与 evidence 摘要（前后值）+ followup_procedure_id。
    assert!(
        block.contains("距临时疯狂阈值一步"),
        "missing threshold_desc: {block}"
    );
    assert!(
        block.contains("38") && block.contains("32"),
        "missing evidence summary: {block}"
    );
    assert!(
        block.contains("proc.followup"),
        "missing followup_procedure_id: {block}"
    );

...
use trpg_model::{
    ChatMessage, ContextRequest, RuleKernel, RuntimeState, TokenBudget, VisibilityProfile,
};
...
async fn fixture(
    turn1_script: Vec<StreamEvent>,
) -> Result<(GmLoop, Arc<MockLlm>, String, String, RuleKernel)> {
...
fn assert_single_retro_debt(gm: &GmLoop) -> String {
    ...
    assert_eq!(
        blocking.len(),
        1,
        "exactly one retro debt expected after turn 1: {blocking:?}"
    );
    assert_eq!(
        blocking[0].kind, "debt",
        "obligation kind must be debt: {blocking:?}"
    );
    assert!(
...
fn assert_turn2_tail_carries_debt(llm: &MockLlm, debt_id: &str) {
    ...
    let turn2_first = requests
        .get(1)
        .expect("turn 2 must have issued at least one request")
        .clone();
    let tail = turn2_first
        .last()
    ...
    assert!(
        ...
        "turn 2 tail missing obligations block: {tail}"
    );
    assert!(
        tail.contains(debt_id),
        "turn 2 tail missing debt id {debt_id}: {tail}"
    );
    assert!(
        tail.contains("fallback:substring_scan"),
        "turn 2 tail missing finding summary: {tail}"
    );
}
...
async fn retro_debt_settled_by_apply_effect_changes_db_state() -> Result<()> {
    ...
    assert!(
        matches!(out1, TurnOutcome::Narration(_)),
        "turn 1 must end as narration"
    );
    ...
    let before = gm
        .engine
        .db
        .load_resource_current(&session_id, "pc.current", &hp_id, &kernel)
        .await
        .unwrap_or(0);
    ...
    match &out2 {
        TurnOutcome::Narration(text) => {
            assert!(text.contains("包扎"), "turn 2 narration mismatch: {text}")
        }
        other => panic!("turn 2 must reach narration after settlement, got {other:?}"),
    }
    assert!(
        gm.obligations.blocking().is_empty(),
        "debt must be settled by the apply_effect ledger entry: {:?}",
        gm.obligations.blocking()
    );
    ...
    let after_n = after
        ...
        .unwrap_or_else(|| panic!("no generic_parameter_states row for {path}: {after:?}"));
    assert_eq!(
        after_n,
        i64::from(before) - 3,
        "HP current must drop by exactly 3 (before={before})"
    );
...
async fn retro_debt_waived_with_reason_unblocks_and_audits() -> Result<()> {
    ...
    llm.push_script(vec![
        tool_call(
            "waive_obligation",
            json!({"target_id": debt_id, "reason": "叙事中已收回该说法"}),
        ),
        done("tool_calls"),
    ]);
    ...
    match &out2 {
        TurnOutcome::Narration(text) => {
            assert!(text.contains("安静"), "turn 2 narration mismatch: {text}")
        }
    ...
    let waive_rows: i64 = sqlx::query_scalar(
        "select count(*) from memory_events where session_id=$1 and 'gm_waive'=any(tags)",
    )
    .bind(&session_id)
    ...
    assert!(
        waive_rows >= 1,
        "expected at least one gm_waive audit row, got {waive_rows}"
    );

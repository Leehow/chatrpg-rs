...
impl LlmClient for MockLlm {
    ...
    async fn stream_chat_with_tools(&self, _: Vec<Value>, _: Vec<Value>, _: ToolChoice) -> Result<Pin<Box<dyn Stream<Item = Result<StreamEvent>> + Send>>> {
        let mut scripts = self.scripts.lock().unwrap();
        assert!(!scripts.is_empty(), "MockLlm script exhausted: the loop asked for one more round than scripted");
...
async fn fixture() -> Result<(GmLoop, Arc<MockLlm>, String, String)> {
    ...
    db.load_rule_kernel(&ruleset).await?.unwrap_or_else(|| panic!("no active kernel for {ruleset}; run parse-all first"));
    let engine = RuntimeEngine::new(db.clone());
    let session_id = engine.start_session(&ruleset, None).await?;
    println!("SESSION_ID={session_id}");
    let llm = Arc::new(MockLlm { scripts: Mutex::new(Vec::new()) });
    let data_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../data");
    let gm = GmLoop::new(engine, llm.clone(), ToolRegistry::standard(), LoopConfig::default(), data_dir);
    Ok((gm, llm, session_id, ruleset))
}

async fn run_turn(gm: &mut GmLoop, session_id: &str, ruleset: &str, user_input: &str) -> Result<TurnOutcome> {
    ...
    gm.run_gm_turn(GmTurnInput { request: &request, state: &state, user_input, history: &[], recent_transcript: None }, &mut |_| {}).await
}
...
async fn dice_roll_count(db: &Db, session_id: &str) -> Result<i64> {
    Ok(sqlx::query_scalar("select count(*) from dice_rolls where session_id=$1")
        .bind(session_id)
        .fetch_one(&db.pool)
        .await?)
}
...
async fn request_player_roll_honors_table_dice_policy_both_branches() -> Result<()> {
    ...
    llm.push_script(vec![tool_call("request_player_roll", player_roll_args()), done("tool_calls")]);
    let out_a = run_turn(&mut gm, &session_a, &ruleset, "我朝那东西开枪").await?;
    match &out_a {
        TurnOutcome::AwaitingPlayerRoll { prompt_public, .. } => {
            assert!(!prompt_public.trim().is_empty(), "gate prompt must be non-empty");
        }
    ...
    assert!(pending.is_some(), "an open pending check must exist under player_rolls policy");
    assert_eq!(dice_roll_count(&gm.engine.db, &session_a).await?, 0, "no system roll may happen under player_rolls policy");
    ...
    llm.push_script(vec![tool_call("request_player_roll", player_roll_args()), done("tool_calls")]);
    llm.push_script(vec![content("枪声未落，那东西已扑到半空——你堪堪侧身让开。"), done("stop")]);
    let out_b = run_turn(&mut gm, &session_b, &ruleset, "我朝那东西开枪").await?;
    match &out_b {
        TurnOutcome::Narration(text) => assert!(!text.trim().is_empty(), "narration must be non-empty"),
        other => panic!("system_rolls_visible policy must settle in-turn and reach narration, got {other:?}"),
    }
    assert!(dice_roll_count(&gm.engine.db, &session_b).await? >= 1, "the converted check must record at least one dice roll");
    ...
    .await?;
    assert_eq!(authority.as_deref(), Some("system"), "stored contract must carry system authority after conversion");
    assert!(gm.engine.db.get_open_pending_check(&session_b).await?.is_none(), "no gate may stay open under system_rolls_visible policy");

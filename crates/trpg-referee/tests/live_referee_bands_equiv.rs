...
async fn coc_bands_equiv() {
    ...
    let Some(kernel) = db
        ...
        eprintln!("SKIP: no call_of_cthulhu_7e kernel");
        return;
    };
    let bands = kernel
        .referee_value_bands
    ...
    assert_eq!(
        bands.damage_family, "generic_trpg_damage",
...
async fn cyberpunk_bands_equiv() {
    ...
    let Some(kernel) = db.load_rule_kernel("cyberpunk_red").await.unwrap_or(None) else {
        eprintln!("SKIP: no cyberpunk_red kernel");
        return;
    };
    let bands = kernel
        .referee_value_bands
    ...
    assert_eq!(
        bands.damage_plausible_range,
        (1, 12),
        "cyberpunk: orig max was 12"
    );
    ...
    assert!(
        !(50 >= bands.difficulty_plausible_range.0 && 50 <= bands.difficulty_plausible_range.1)
    );
}

...
impl PlayerValueRefereeService {
    ...
    pub async fn inspect_turn(
        ...
        if !enabled() {
            return Ok(PlayerValueRefereeResult::default());
        }
        let world_tick = None;
        let claims = detect_claims(
            session_id, turn_id, ruleset_id, module_id, user_input, world_tick,
        );
        if claims.is_empty() {
            return Ok(PlayerValueRefereeResult::default());
        }
        ...
        let mut result = PlayerValueRefereeResult {
            handled: true,
            phases: vec!["player_value_referee".into()],
            ..Default::default()
        };
        ...
        for claim in claims {
            ...
            self.db
                .insert_player_value_verification(&verification)
                .await
                .ok();
            let override_agreement = if verification.status
                == PlayerValueVerificationStatus::AcceptedAsTablePreference
            {
                let agreement = TableOverrideAgreement {
                    ...
                    warning_public: verification
                        .warning_public
                        .clone()
                        .unwrap_or_else(|| "这会作为本桌规则偏好记录，可能改变系统平衡。".into()),
                    created_at_tick: world_tick,
                    created_at: Utc::now(),
                };
                self.db
                    .insert_table_override_agreement(&agreement)
                    .await
                    .ok();
                Some(agreement)
            } else {
            ...
            if let Some(agreement) = override_agreement {
                result.table_overrides.push(agreement);
            }
        }
...
fn detect_claims(
    ...
    for cap in dice_total_re.captures_iter(input) {
        ...
        out.push(make_claim(
            ...
            "player supplied dice total",
            json!({"dice_expression": dice, "total": total}),
            evidence,
            input,
            world_tick,
        ));
    ...
    for cap in damage_expr_re.captures_iter(input) {
        ...
        out.push(make_claim(
            ...
            ruleset_id,
            module_id,
            PlayerSuppliedValueKind::WeaponDamage,
            "player supplied weapon/ability damage",
            json!({"damage_expression": expr}),
            evidence,
            input,
            world_tick,
        ));
    ...
    let dv_re =
        Regex::new(r"(?i)(?:dv|dc|tn|难度|目标值|命中\s*dv|距离\s*dv)[^0-9]{0,12}(\d{1,3})")
            .unwrap();
    for cap in dv_re.captures_iter(input) {
        ...
        out.push(make_claim(
            ...
            ruleset_id,
            module_id,
            PlayerSuppliedValueKind::DifficultyValue,
            "player supplied difficulty/target number",
            json!({"value": value}),
            evidence,
            input,
            world_tick,
    ...
    let hp_re =
        Regex::new(r"(?i)(?:hp|血量|生命值|剩\s*血|剩下|还剩)[^0-9]{0,12}(\d{1,3})").unwrap();
    for cap in hp_re.captures_iter(input) {
        ...
        out.push(make_claim(
            ...
            ruleset_id,
            module_id,
            PlayerSuppliedValueKind::HitPoints,
            "player supplied hit point value",
            json!({"value": value}),
            evidence,
            input,
            world_tick,
    ...
    let armor_re =
        Regex::new(r"(?i)(?:armor|armour|sp|ac|护甲|装甲|防御)[^0-9]{0,12}(\d{1,3})").unwrap();
    for cap in armor_re.captures_iter(input) {
        ...
        out.push(make_claim(
            ...
            ruleset_id,
            module_id,
            PlayerSuppliedValueKind::ArmorValue,
            "player supplied armor/defense value",
            json!({"value": value}),
            evidence,
            input,
            world_tick,
...
fn verify_claim(
    claim: &PlayerSuppliedValueClaim,
    kernel: &trpg_model::RuleKernel,
    input: &str,
    player_insists: bool,
    ...
    match claim.value_kind {
        PlayerSuppliedValueKind::DamageRollTotal => {
            ...
            comparison =
                json!({"supplied_total": total, "inside_bounds": total >= min && total <= max});
            if total >= min && total <= max {
                ...
                warning = Some(format!(
                    "这个数值不合法：{} 的结果不可能是 {}，合法范围是 {}–{}。",
                    expr, total, min, max
                ));
                suggestion = Some(format!(
                    "请重新给出 {} 的实际结果，或明确说你要把 {} 作为本桌特例。",
                    expr, total
                ));
        ...
        PlayerSuppliedValueKind::WeaponDamage | PlayerSuppliedValueKind::DamageExpression => {
            ...
            if plausible {
                ...
                suggestion = Some(format!(
                    "建议先按规则表中同类武器/能力的常见区间处理：{}。",
                    common_damage_band_from_kernel(kernel)
                ));
                ...
                suggestion = Some(format!(
                    "建议用同类条目的参数范围作为临时值：{}；如果你坚持，我会记录为本桌特例。",
                    common_damage_band_from_kernel(kernel)
                ));
        ...
        PlayerSuppliedValueKind::DifficultyValue | PlayerSuppliedValueKind::RangeDifficulty => {
            ...
            if plausible {
                ...
                suggestion = Some(
                    "该难度值在当前规则常见区间内，但仍应优先查对应距离/难度/动作程序表。".into(),
                );
            } else if player_insists {
                status = PlayerValueVerificationStatus::AcceptedAsTablePreference;
                warning = Some(format!("你给的目标值 {} 不在当前规则常见难度区间内。若本桌坚持，我会记录为 table override；这可能让行动过难或过易。", value));
                suggestion =
                    Some("建议改用规则中的标准 DV/DC/TN 或由 GM 根据距离、掩体和风险裁定。".into());
                balance_risk = Some("out_of_band_target_value".into());
                canonical = json!({"table_override_target_value": value});
            } else {
                status = PlayerValueVerificationStatus::UnreasonableNeedsWarning;
                warning = Some(format!(
                    "我不会直接采纳目标值 {}；它需要规则程序/距离表确认。",
                    value
                ));
                suggestion =
                    Some("建议由 GM 查规则或根据同类难度表给出目标值，而不是让玩家提供。".into());
            }
        }
        PlayerSuppliedValueKind::ArmorValue
        | PlayerSuppliedValueKind::HitPoints
        | PlayerSuppliedValueKind::ResourceAmount
        | PlayerSuppliedValueKind::AbilityCost
        | PlayerSuppliedValueKind::GenericParameter => {
            let value = claim
                .supplied_value_json
                .get("value")
                .and_then(|v| v.as_i64())
...
fn parse_dice_count(expr: &str) -> Option<i64> {
    Regex::new(r"(?i)(\d+)\s*d\s*(\d+)")
        .ok()?
        .captures(expr)
...
fn bands_for(kernel: &trpg_model::RuleKernel) -> &trpg_model::RefereeValueBands {
    kernel
        .referee_value_bands
        .as_ref()
        .unwrap_or(&trpg_model::GENERIC_REFEREE_BANDS)
}
...
fn damage_plausible_from_kernel(kernel: &trpg_model::RuleKernel, expr: &str) -> bool {
    let Some(n) = parse_dice_count(expr) else {
        return false;
    };
...
fn generic_value_plausible(value: i64) -> bool {
...
fn render_referee_context(result: &PlayerValueRefereeResult) -> String {
    ...
    for v in &result.verifications {
        lines.push(format!(
            "- claim={} status={} warning={:?} suggestion={:?}",
            v.claim_id,
            v.status.as_str(),
            v.warning_public,
            v.suggestion_public
...
mod referee_bands_tests {
    use super::*;
    use trpg_model::{RefereeValueBands, RuleKernel, GENERIC_REFEREE_BANDS};

    fn kernel_with_bands(bands: RefereeValueBands) -> RuleKernel {
        RuleKernel {
            referee_value_bands: Some(bands),
            ..Default::default()
        }
    }
    fn kernel_no_bands() -> RuleKernel {
        RuleKernel {
            referee_value_bands: None,
            ..Default::default()
        }
    }
    ...
    fn damage_family_falls_back_to_generic() {
        ...
        assert_eq!(
            damage_family_from_kernel(&k),
            GENERIC_REFEREE_BANDS.damage_family.as_str()
        );
    }
    ...
    fn no_ruleset_name_in_referee_src() {
        ...
        assert!(
            !src.contains(&["fn ", "ruleset_damage_family("].concat()),
            "old hardcoded fn must be deleted: ruleset_damage_family"
        );
        assert!(
            !src.contains(&["fn ", "common_damage_band("].concat()),
            "old hardcoded fn must be deleted: common_damage_band"
        );
        assert!(
            !src.contains(&["fn ", "damage_plausible_for_ruleset("].concat()),
            "old hardcoded fn must be deleted: damage_plausible_for_ruleset"
        );
        assert!(
            !src.contains(&["fn ", "common_difficulty_band_json("].concat()),
            "old hardcoded fn must be deleted: common_difficulty_band_json"
        );
        assert!(
            !src.contains(&["fn ", "difficulty_plausible_for_ruleset("].concat()),

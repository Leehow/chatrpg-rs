        ...
        match v {
            Value::Number(n) => {
                if let Some(f) = n.as_f64() {
                    ctx.insert(prefix.to_string(), Num::N(f));
                }
            }
            Value::String(sv) => {
                let t = sv.trim();
                if let Ok(f) = t.parse::<f64>() {
                    ...
                    ctx.insert(prefix.to_string(), Num::Str(t.to_string()));
                }
            }
            Value::Object(m) => {
                for (k, vv) in m {
...
fn record_exprs(r: &Value) -> Vec<String> {
    let mut out = Vec::new();
    if let Some(e) = r.get("expr").and_then(|v| v.as_str()) {
        out.push(e.to_string());
    }
    if let Some(e) = r.get("attr_derived").and_then(|v| v.as_str()) {
        out.push(e.to_string());
    }
    if let Some(allocs) = r.get("allocations").and_then(|v| v.as_array()) {
        for a in allocs {
            if let Some(e) = a.get("input").and_then(|v| v.as_str()) {
                out.push(e.to_string());
            }
        }
    }
...
fn topo(records: &[Value]) -> (Vec<usize>, Vec<String>) {
    let id_of: HashMap<String, usize> = records
        .iter()
        .enumerate()
        .map(|(i, r)| (key(&s(r, "id")), i))
        .collect();
    ...
    for i in 0..records.len() {
        if state[i] == 0 {
            dfs(i, &deps, &mut state, &mut order, &mut cyc_idx);
        }
    }
    for i in cyc_idx {
        cycles.push(s(&records[i], "id"));
    }
    (order, cycles)
}
...
pub fn evaluate_chargen(records: &[Value], inputs: &Map<String, Value>) -> ChargenReport {
    ...
    report.gaps = cyc
        .iter()
        .map(|c| format!("dependency cycle at `{c}`"))
        .collect();
    ...
    for idx in order {
        ...
        let role = s(r, "role");
        let recompute = if s(r, "recompute").is_empty() {
            "live".into()
        } else {
            s(r, "recompute")
        };
        let result_type = if s(r, "result_type").is_empty() {
            "int".into()
        } else {
            s(r, "result_type")
        };
        let tables = r.get("lookup_tables").cloned().unwrap_or_else(|| json!({}));
        let declared_status = if s(r, "status").is_empty() {
            "source_backed".into()
        } else {
            s(r, "status")
        };
        ...
        for e in &required {
            for refr in expr::extract_refs(e) {
                let k = key(refr.trim_start_matches("derived."));
                if !ctx.contains_key(&k) && !ctx.contains_key(&key(&refr)) {
                    unresolved.push(refr);
                }
            }
        }
        unresolved.sort();
        ...
        let mk = |value: Option<Value>, breakdown: Value, unresolved: Vec<String>| {
            let status = if !unresolved.is_empty() || in_cycle.contains(&key(&id)) {
                "provisional".to_string()
            } else {
                declared_status.clone()
            };
            EvalResult {
                id: id.clone(),
                role: role.clone(),
        ...
        if !unresolved.is_empty() || in_cycle.contains(&key(&id)) {
            report.values.push(mk(
                None,
                json!({"unresolved": unresolved.clone()}),
                unresolved,
            ));
            continue;
        }
        ...
        let res = if r.get("expr").is_some() {
            eval_one(&s(r, "expr"), &eval_ctx, &tables, &tracks).map(|n| {
                (
                    as_int_or_dice(&n, &result_type),
                    json!({"final": as_int_or_dice(&n, &result_type)}),
                    n,
                )
            })
        } else {
            ...
            Ok((mut val, breakdown, num)) => {
                ...
                if let Num::N(_) = num {
                    if let Some(f) = val.as_f64() {
                        ctx.insert(key(&id), Num::N(f));
                        ctx.insert(format!("derived.{}", key(&id)), Num::N(f));
                    }
                }
...
fn resolve_ctx(ctx: &HashMap<String, Num>) -> HashMap<String, Num> {
    ...
    for (k, v) in ctx.iter() {
        if !k.contains('.') {
            m.entry(format!("derived.{k}")).or_insert_with(|| v.clone());
        }
    }
    m
}
...
fn eval_one(
    e: &str,
    ctx: &HashMap<String, Num>,
    tables: &Value,
    tracks: &Value,
) -> Result<Num, String> {
    expr::eval(e, ctx, tables, tracks)
}
...
fn eval_hybrid(
    ...
    result_type: &str,
    tracks: &Value,
) -> Result<(Value, Value, Num), String> {
    let attr = match r.get("attr_derived").and_then(|v| v.as_str()) {
        Some(e) => eval_one(e, ctx, tables, tracks)?.num_or_err()?,
        None => 0.0,
    };
    ...
    if let Some(allocs) = r.get("allocations").and_then(|v| v.as_array()) {
        for a in allocs {
            let v = match a.get("input").and_then(|x| x.as_str()) {
                Some(e) => eval_one(e, ctx, tables, tracks)
                    .and_then(|n| n.num_or_err())
                    .unwrap_or(0.0),
                None => 0.0,
            };
            alloc_sum += v;
            alloc_bd.push(
                json!({"source": a.get("source").cloned().unwrap_or(Value::Null), "value": v}),
            );
        }
    }
    let floor_total = attr + base + alloc_sum;
    if s(r, "role") == "skill" && alloc_sum == 0.0 {
        let id = key(&s(r, "id"));
        let provided = ctx.get(&id).and_then(|n| n.num_or_err().ok());
        let resolve_limit = |x: &Value| -> Option<f64> {
            x.as_f64().or_else(|| {
                x.as_str()
                    .and_then(|key| ctx.get(&key.to_ascii_lowercase()))
                    .and_then(|n| n.num_or_err().ok())
            })
        };
        let max = r
            .get("max")
            .and_then(resolve_limit)
            .or_else(|| r.get("clamp_max").and_then(resolve_limit));
        if let Some(total) = provided
            .filter(|total| *total >= floor_total && max.map(|m| *total <= m).unwrap_or(true))
        {
            let final_v = if result_type == "float" {
                json!(total)
            } else {
                json!(total.floor() as i64)
            };
            let breakdown = json!({
                "attr_derived": attr,
                "base": base,
                "allocations": alloc_bd,
                "preserved_player_total": final_v,
                "final": final_v
            });
            return Ok((final_v, breakdown, Num::N(total)));
        }
    }
    let total = floor_total;
    let final_v = if result_type == "float" {
        json!(total)
    } else {
        json!(total.floor() as i64)
    ...
    let breakdown =
        json!({"attr_derived": attr, "base": base, "allocations": alloc_bd, "final": final_v});
    Ok((final_v, breakdown, Num::N(total)))
}

fn clamp_value(val: Value, r: &Value, ctx: &HashMap<String, Num>) -> Value {
    let Some(mut f) = val.as_f64() else {
        return val;
    };
    let resolve = |x: &Value| -> Option<f64> {
        x.as_f64().or_else(|| {
            x.as_str().and_then(|s2| {
                ctx.get(&key(s2))
                    .and_then(|n| if let Num::N(v) = n { Some(*v) } else { None })
            })
        })
    };
    if let Some(mx) = r.get("max").and_then(resolve) {
        if f > mx {
            f = mx;
        }
    }
    if let Some(mn) = r.get("min").and_then(resolve) {
        if f < mn {
            f = mn;
        }
    }
    if let Some(cm) = r.get("clamp_max").and_then(resolve) {
        if f > cm {
            f = cm;
        }
    }
...
impl Num {
    fn num_or_err(&self) -> Result<f64, String> {
        match self {
            Num::N(n) => Ok(*n),
            Num::Dice(d) => Err(format!("dice `{d}` in hybrid segment")),
            Num::Str(s) => Err(format!("categorical `{s}` in hybrid segment")),
        }
    }
}

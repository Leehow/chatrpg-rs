...
impl Num {
    fn num(&self) -> Result<f64, String> {
        match self {
            ...
            Num::Str(s) => Err(format!(
                "categorical value `{s}` cannot be used in arithmetic"
            )),
        }
    }
}
...
fn lex(s: &str) -> Result<Vec<Tok>, String> {
    ...
    while i < b.len() {
        ...
        if c == '{' && i + 1 < b.len() && b[i + 1] == '{' {
            ...
            if j + 1 >= b.len() {
                return Err("unterminated {{ref}}".into());
            }
        ...
        match c {
            ...
            '<' | '>' | '=' => {
                if i + 1 < b.len() && b[i + 1] == '=' {
                    ...
                    return Err("use == for comparison".into());
                } else {
                    out.push(Tok::Op(c.to_string()));
                }
            }
...
impl<'a> Parser<'a> {
    ...
    fn unary(&mut self) -> Result<Num, String> {
        if let Some(Tok::Op(o)) = self.peek() {
            if o == "-" {
                self.i += 1;
                return Ok(Num::N(-self.unary()?.num()?));
            }
        }
        self.atom()
    }
    ...
    fn call(&mut self, name: &str) -> Result<Num, String> {
        ...
        if name == "lookup" {
            ...
            let table = match self.next() {
                Some(Tok::Ident(t)) => t,
                other => return Err(format!("lookup() table must be a name, got {other:?}")),
            };
            match self.next() {
                Some(Tok::Comma) => {}
                _ => return Err("lookup() expects (table, key)".into()),
            }
            let key = self.expr()?;
            match self.next() {
                Some(Tok::RParen) => {}
        ...
        let nums = nums?;
        let one = |f: &dyn Fn(f64) -> f64| -> Result<Num, String> {
            if nums.len() != 1 {
        ...
        match name {
            ...
            "min" => nums
                ...
                .reduce(f64::min)
                .map(Num::N)
                .ok_or_else(|| "min() needs args".into()),
            "max" => nums
                .iter()
                .cloned()
                .reduce(f64::max)
                .map(Num::N)
                .ok_or_else(|| "max() needs args".into()),
            "if" => {
                if nums.len() != 3 {
                    return Err("if(cond,a,b)".into());
                }
                Ok(Num::N(if nums[0] != 0.0 { nums[1] } else { nums[2] }))
            }
...
fn range_value(r: &Value) -> Result<Num, String> {
    match r.get("value") {
        ...
        Some(Value::String(s)) => match s.trim().parse::<f64>() {
            Ok(n) => Ok(Num::N(n)),
            Err(_) => Ok(Num::Dice(s.trim().to_string())),
        },
...
fn lookup_cat(tables: &Value, table: &str, key: &str) -> Result<Num, String> {
    ...
    for r in ranges {
        let hit = ["key", "min", "max"].iter().any(|f| {
            r.get(*f)
                .and_then(|v| v.as_str())
                .map(|s| s.trim().to_ascii_lowercase() == k)
                .unwrap_or(false)
        });
        if hit {
            return range_value(r);
    ...
    Err(format!(
        "lookup `{table}` categorical key `{key}` matched no entry"
    ))
}
...
pub fn extract_refs(s: &str) -> Vec<String> {
    lex(s)
        .map(|toks| {
            toks.into_iter()
                .filter_map(|t| if let Tok::Ref(r) = t { Some(r) } else { None })
                .collect()
        })
        .unwrap_or_default()
}

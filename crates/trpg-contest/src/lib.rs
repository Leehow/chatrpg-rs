...
impl ContestService {
    ...
    pub async fn resolve_outcome(
        ...
        } = &profile.resolution_model
        {
            ...
            if let (Some(compare), Some(dt)) = (kc_opt, def_total) {
                let (t, s, d) = opposed::resolve_opposed(
                    &compare,
                    &bands,
                    total,
                    *attacker_value,
        ...
        } = &profile.resolution_model
        {
            if let Some(def_roll) = defender_roll {
                ...
                let (t, s, d) =
                    opposed::resolve_pool_opposed(*target_face, *threshold, &rolls, &def_rolls);
                target = t;
                success = s;
                degree = d;
            }
        ...
        if success.is_some() {
            ...
            } = &profile.resolution_model
            {
                ...
                outcome[outcome_fields::POOL_MISS_COUNT] =
                    json!(rolls.len().saturating_sub(atk_hits));
                outcome["dice"] = json!(rolls);
                outcome["opposed"] = json!({
    ...
    pub async fn ensure_contest_profile(
        &self,
        contract: &CheckContract,
        roll: Option<&DiceRollRecord>,
        _total: i64,
    ) -> Result<ContestProfile> {
        if let Some(existing) = self.load_profile_for_check(&contract.check_id).await? {
            return Ok(existing);
        }
        let model = self.resolve_model(contract).await;
        let unresolved_model = matches!(
            &model,
            CheckResolutionModel::Provisional { .. }
                | CheckResolutionModel::RulesetProcedureLookup { .. }
        );
        ...
        let profile = ContestProfile {
            ...
            confidence: if unresolved_model
            ...
            verification_status: if unresolved_model
                || matches!(&contract.target, CheckTargetModel::UnknownUntilLookup)
                    && matches!(
                        &contract.opposition,
                        OppositionModel::NoMechanicalOpposition
                    ) {
                ContestVerificationStatus::ProvisionalNeedsBinding
            } else {
                ContestVerificationStatus::VerifiedPartial
            },
            provisional_reason: provisional_reason_for_contract(contract)
                .or_else(|| provisional_reason_for_model(&model)),
            world_tick,
            created_at: Utc::now(),
        };
    ...
    async fn kernel_resolution_model(
        &self,
        contract: &CheckContract,
    ) -> Option<CheckResolutionModel> {
        let kernel = self
            .db
        ...
        match compare {
            "count_faces" => {
                ...
                let face = match count_faces_target_face(dc) {
                    Some(f) => f as i32,
                    None => return Some(count_faces_model(dc)),
                };
                ...
                if contract_is_opposed(contract) {
                    let def_expr = dc
                        .get("dice")
                        .and_then(|v| v.as_str())
                        .unwrap_or(&contract.dice_expression)
                        .to_string();
                    return Some(build_pool_opposed_model(
                        contract, face, threshold, &def_expr,
                    ));
                }
                Some(CheckResolutionModel::DicePoolCount {
                    target_face: face,
                    threshold,
            ...
            "roll_under" => {
                ...
                if contract_is_opposed(contract) {
                    let (atk, def, def_expr) =
                        self.resolve_opposed_values(contract, &kernel, dc).await;
                ...
                if let Some((label, value)) =
                    self.resolve_percentile_target(contract, &kernel).await
                {
                    Some(CheckResolutionModel::PercentileRollUnder {
                        ability_label: label,
                        ability_value: value,
                    })
                    ...
                    Some(CheckResolutionModel::PercentileRollUnder {
                        ability_label: contract.check_label.clone(),
                        ability_value: v,
                    })
                } else {
            ...
            "meet_or_beat" => {
                ...
                tnum.map(|v| CheckResolutionModel::StaticTargetNumber {
                    value: v,
    ...
    async fn resolve_opposed_values(
        ...
        let ta = match contract.target_actor.as_ref() {
            Some(a) => a,
            None => return (None, None, contract.dice_expression.clone()),
        };
        let atk = self
            .resolve_percentile_target_for(
    ...
    async fn resolve_percentile_target(
        &self,
        contract: &CheckContract,
        kernel: &RuleKernel,
    ) -> Option<(String, i32)> {
    ...
    async fn resolve_percentile_target_for(
        ...
        match src {
            ...
            TestedSource::Skill { key } => {
                value_from_profile(mech.get("skills"), &key).map(|v| (key, v))
            }
    ...
    async fn read_track_value(
        ...
        self.db
            .load_resource_current(session_id, actor_id, track_id, kernel)
            .await
    }
        ...
        match &contract.target {
            CheckTargetModel::StaticNumber { value, label } => {
                if is_attack_contract(contract) {
                    CheckResolutionModel::AttackVsDefense {
                        attack_expression: contract.dice_expression.clone(),
                        defender_actor_id: defender_for_contract(contract),
                        defense_label: label.clone(),
                        defense_value: *value,
                    }
                } else {
                    CheckResolutionModel::StaticTargetNumber {
                        value: *value,
                        label: label.clone(),
                    }
                }
            ...
            CheckTargetModel::Opposed {
                ...
                attacker_value: None,
                defender_actor_id: Some(opponent_id.clone()),
                defender_expression: opponent_check.clone(),
                defender_value: None,
                defender_roll_visibility: RollVisibility::PrivateGmRoll,
            },
            CheckTargetModel::DegreeOnly => CheckResolutionModel::RulesetProcedureLookup {
                procedure_label: "degree_only".into(),
                unresolved_fields: vec!["success_band".into()],
            },
            CheckTargetModel::SuccessCount { threshold } => {
                CheckResolutionModel::StaticTargetNumber {
                    value: *threshold,
                    label: "success_count_threshold".into(),
                }
            }
            CheckTargetModel::DicePoolCount {
                ...
                if contract_is_opposed(contract) {
                    build_pool_opposed_model(
                        contract,
                        *target_face,
                        *threshold,
                        &contract.dice_expression,
                    )
                } else {
    ...
    async fn load_profile_for_check(&self, check_id: &str) -> Result<Option<ContestProfile>> {
        ...
        Ok(row.and_then(|r| {
            serde_json::from_value::<ContestProfile>(r.get::<Value, _>("profile_json")).ok()
        }))
    }
    ...
    pub async fn contest_context_block(
        &self,
        session_id: &str,
        world_tick: i64,
    ) -> Result<ContextBlock> {
        ...
        let profiles: Vec<Value> = rows
            .into_iter()
            .filter_map(|r| r.try_get::<Value, _>("profile_json").ok())
            .collect();
...
fn roll_total(roll: &DiceRollRecord) -> i64 {
    roll.result
        ...
        .or_else(|| {
            roll.result
                .get("mode")
                .and_then(|_| roll.result.get("total"))
                .and_then(|v| v.as_i64())
        })
        .unwrap_or_default()
}
...
fn resolve_against_model(
    model: &CheckResolutionModel,
    total: i64,
    rolls: &[i64],
) -> (Option<i64>, Option<bool>, Option<String>, Option<String>) {
    match model {
...
fn infer_ruleset_default_model(contract: &CheckContract) -> CheckResolutionModel {
    ...
    if let Some(dc) = env_i32("TRPG_CONTEST_DEFAULT_STATIC_TARGET") {
        return CheckResolutionModel::StaticTargetNumber {
            value: dc,
            label: "table-configured static target override".into(),
        };
    }
    CheckResolutionModel::Provisional {
        reason: format!(
            "no source-backed target number/opposition model was bound for ruleset `{}`",
            contract.ruleset_id
        ),
        suggested_target: None,
    }
    ...
    contract
        ...
        .or_else(|| {
            contract
                ...
                .iter()
                .find(|id| {
                    id.starts_with("npc.") || id.contains("opposition") || id.contains("enemy")
                })
                .cloned()
        })
        .or_else(|| {
...
fn target_summary(contract: &CheckContract) -> String {
    match &contract.target {
        CheckTargetModel::StaticNumber { value, label } => {
            format!("static target {} ({})", value, label)
        }
        CheckTargetModel::Opposed {
            opponent_id,
            opponent_check,
        } => format!("opposed by {} using {}", opponent_id, opponent_check),
        CheckTargetModel::DegreeOnly => "degree only".into(),
        CheckTargetModel::SuccessCount { threshold } => {
            format!("success count threshold {}", threshold)
        }
        CheckTargetModel::DicePoolCount {
            target_face,
            threshold,
            ..
        } => format!("dice pool: >= {} dice showing {}", threshold, target_face),
        CheckTargetModel::UnknownUntilLookup => "inferred by Contest/Opposition Kernel".into(),
    }
}

fn contest_kind_for_contract(
    contract: &CheckContract,
    model: &CheckResolutionModel,
) -> ContestKind {
    match model {
...
fn provisional_reason_for_model(model: &CheckResolutionModel) -> Option<String> {
    match model {
        ...
        CheckResolutionModel::RulesetProcedureLookup {
            ...
            "ruleset procedure `{}` needs source-backed binding for {:?}",
            procedure_label, unresolved_fields
        )),
        _ => None,
    }
}
...
fn world_tick_from_roll_or_contract(
    _roll: Option<&DiceRollRecord>,
...
fn band_rank(b: &Value) -> i64 {
    b.get("rank").and_then(|v| v.as_i64()).unwrap_or(0)
}
...
fn band_test_passes(b: &Value, total: i64, target: i64) -> bool {
    let test = match b.get("test") {
        Some(t) => t,
        None => return false,
    };
    if let Some(g) = test.get("when") {
        if !band_guard_ok(g, target) {
            return false;
        }
    }
    if let Some(g) = test.get("unless") {
        if band_guard_ok(g, target) {
            return false;
        }
    }
...
fn band_guard_ok(g: &Value, target: i64) -> bool {
    if let Some(n) = g.get("target_lt").and_then(|v| v.as_i64()) {
        if target >= n {
            return false;
        }
    }
    if let Some(n) = g.get("target_lte").and_then(|v| v.as_i64()) {
        if target > n {
            return false;
        }
    }
    if let Some(n) = g.get("target_gte").and_then(|v| v.as_i64()) {
        if target < n {
            return false;
        }
    }
    if let Some(n) = g.get("target_gt").and_then(|v| v.as_i64()) {
        if target <= n {
            return false;
        }
    }
    ...
    for track in &kernel.resource_tracks {
        ...
        if let Some(arr) = track.get("on_outcome").and_then(|v| v.as_array()) {
            for r in arr {
                ...
                if r.get("trigger").and_then(|v| v.as_str()) == Some("always") {
                    continue;
                }
        ...
        let hit = match hint {
            Some(h) => aliases
                .iter()
                .any(|a| a == h || h.contains(a.as_str()) || a.contains(h)),
        ...
        if hit {
            ...
            return Some(TestedSource::Track {
                id: id.to_string(),
                label,
            });
        }
...
mod tested_param_tests {
    ...
    fn skill_check_resolves_to_the_named_skill() {
        let src = derive_tested_source(
            ...
            &coc_kernel(),
            &coc_mech(),
            "Spot Hidden (core mechanic)",
            "搜索房间",
            "ability:skill_use",
        );
        match src {
            Some(TestedSource::Skill { key }) => assert_eq!(key, "Spot Hidden"),
            other => panic!("expected skill, got {:?}", other.is_some()),
        }
        assert_eq!(
            value_from_profile(coc_mech().get("skills"), "Spot Hidden"),
            Some(75)
        );
    }
    ...
    fn tier(total: i64, target: i64) -> String {
        success_tier_for(&coc_bands(), total, target).unwrap().0
    }

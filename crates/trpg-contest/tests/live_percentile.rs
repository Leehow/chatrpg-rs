...
async fn percentile_reads_real_actor_value_not_fifty() {
    ...
    let params = trpg_params::RuntimeParameterService::new(db.clone())
        ...
        .ok()
        .flatten();
    let has_fixture = params
        .as_ref()
        .and_then(|p| p.mechanical_profile.get("skills"))
        .and_then(|s| s.get("Spot Hidden"))
        .is_some();
    if !has_fixture {
        eprintln!("SKIP: CoC test fixture (session/{ACTOR} Spot Hidden) not found in this DB");
        return;
    }
    ...
    let out = svc
        .resolve_outcome(&contract("Spot Hidden", "Spot Hidden"), &roll(30), None)
        .await
        .expect("resolve");
    let target = out.get("target").and_then(|v| v.as_i64());
    let success = out.get("success").and_then(|v| v.as_bool());
    println!(
        "[Spot Hidden] roll=30 target={:?} success={:?} label={:?}",
        target,
        success,
        out.get("check_label")
    );
    assert_eq!(
        target,
        Some(75),
        "Spot Hidden must resolve against the sheet value 75, not 50"
    );
    assert_eq!(success, Some(true), "30 <= 75 must succeed");

    // Sanity is a live resource track — assert it read a REAL value, not 50.
    let out = svc
        .resolve_outcome(&contract("理智", "理智检定"), &roll(90), None)
        .await
        .expect("resolve");
    let san = out.get("target").and_then(|v| v.as_i64());
    let success = out.get("success").and_then(|v| v.as_bool());
    println!(
        "[Sanity/理智] roll=90 target={:?} success={:?} (resolution_model={:?})",
        san,
        success,
        out.get("resolution_model")
            .and_then(|m| m.get("PercentileRollUnder"))
    );
    let san = san.expect("sanity target resolved");
    assert!(
        (0..=99).contains(&san),
        "sanity target {} must be a real track value",
        san
    );
    assert_ne!(
        san, 50,
        "sanity must NOT resolve against the fabricated env default 50"
    );
...
async fn percentile_emits_coc_success_tiers() {
    ...
    let params = trpg_params::RuntimeParameterService::new(db.clone())
        ...
        .ok()
        .flatten();
    let has_bands = params
        .as_ref()
        .and_then(|p| p.mechanical_profile.get("skills"))
        .and_then(|s| s.get("Spot Hidden"))
        .is_some();
    if !has_bands {
        eprintln!("SKIP: CoC fixture not found");
        return;
    }
    let svc = ContestService::new(db.clone());
    ...
    for (total, want) in cases {
        let out = svc
            .resolve_outcome(&contract("Spot Hidden", "Spot Hidden"), &roll(total), None)
            .await
            .expect("resolve");
        let tier = out.get("success_tier").and_then(|v| v.as_str());
        let rank = out.get("success_tier_rank").and_then(|v| v.as_i64());
        println!(
            "[tier] Spot Hidden(75) roll={:<3} -> tier={:?} rank={:?}",
            total, tier, rank
        );
        assert_eq!(
            tier,
            Some(want),
            "roll {} vs 75 should be tier {}",
            total,
            want
        );
    }

...
async fn meet_or_beat_opposed_reads_npc_defense_and_yields_verdict() {
    ...
    assert_eq!(
        model_kind,
        Some("opposed_roll"),
        "meet_or_beat 对抗契约必须建 OpposedRoll(本期补的读卡支路),实际={:?}",
        model_kind
    );
    ...
    assert!(
        success.is_some(),
        "对抗必须给出胜负(success != null),而非 Provisional-null(消费层失败)"
    );
    ...
    assert_eq!(
        dv,
        Some(12),
        "outcome.opposed.defender_value 必须=12(消费了 NPC 卡的 defense)"
    );
    let av = opposed
    ...
    sqlx::query("delete from runtime_actor_parameters where session_id=$1")
        ...
        .ok();
    println!(
        "PASS: meet_or_beat opposed 读到 NPC defense={:?},attacker(14) 命中,success={:?}",
        dv, success
    );
}
...
async fn meet_or_beat_opposed_without_defense_emits_awaiting_binding() {
    ...
    params
        .upsert_actor_parameters(&seed_actor(
            ...
            "npc.boss",
            ActorKind::Npc,
            json!({"stats": {"BODY": 5}, "skills": {}}),
        ))
        .await
        .expect("seed npc no-defense");
    ...
    assert!(
        success.map(|v| v.is_null()).unwrap_or(true),
        "防御值缺 → success 必须 null(不乱判命中),实际={:?}",
        success
    );
    ...
    assert!(
        awaiting.is_some(),
        "防御值缺 → outcome 必须带 awaiting_binding 信号(非静默 success=null),实际 keys={:?}",
        out.as_object().map(|o| o.keys().collect::<Vec<_>>())
    );
    assert!(
        !awaiting.unwrap().trim().is_empty(),
        "awaiting_binding 理由不得为空"
    );
    ...
    sqlx::query("delete from runtime_actor_parameters where session_id=$1")
        ...
        .ok();
    println!(
        "PASS: 防御值缺 → success=null 且 awaiting_binding={:?}(显式待绑非静默 miss)",
        awaiting
    );
}

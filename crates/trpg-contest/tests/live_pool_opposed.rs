...
async fn count_faces_opposed_resolves_by_hit_counts_with_real_defender_roll() {
    ...
    let atk = pool_roll(&session, "pc.current", "player_character", &[3, 3, 1, 2, 4, 3]);
    let def = pool_roll(&session, "npc.rival", "npc", &[3, 1, 2, 2, 4, 1]);
    let out = svc.resolve_outcome(&opposed_pool_contract(&session), &atk, Some(&def))
    ...
    assert_eq!(model_kind, Some("dice_pool_opposed"),
        "count_faces 对抗契约必须建 DicePoolOpposed,实际={:?}", model_kind);
    ...
    assert_eq!(opp.get("attacker_hits").and_then(|v| v.as_i64()), Some(3), "攻击方 3 hits");
    assert_eq!(opp.get("defender_hits").and_then(|v| v.as_i64()), Some(1), "防御方 1 hit(真掷池数出)");
    assert_eq!(opp.get("winner").and_then(|v| v.as_str()), Some("attacker"));
    assert_eq!(opp.get("defender_dice").and_then(|v| v.as_array()).map(|a| a.len()), Some(6),
...
async fn count_faces_opposed_without_defender_roll_emits_awaiting_binding() {
    ...
    let svc = ContestService::new(db.clone());
    let atk = pool_roll(&session, "pc.current", "player_character", &[3, 3, 3, 1, 2, 4]);
    let out = svc.resolve_outcome(&opposed_pool_contract(&session), &atk, None)
        .await.expect("resolve_outcome must not error");
    println!("[pool-unbound] {}", serde_json::to_string(&out).unwrap());

    // 仍是 DicePoolOpposed(建了对抗模型),但无防御骰 → success 维持 null(不乱判命中)。
    assert!(out.get("success").map(|v| v.is_null()).unwrap_or(true),
        "无防御骰 → success 必须 null,实际={:?}", out.get("success"));
    let awaiting = out.get("awaiting_binding").and_then(|v| v.as_str());
    assert!(awaiting.map(|s| !s.trim().is_empty()).unwrap_or(false),
        "无防御骰 → 必须带非空 awaiting_binding(显式待绑非静默 miss),keys={:?}",
        out.as_object().map(|o| o.keys().collect::<Vec<_>>()));

    println!("PASS: 无防御骰 → success=null 且 awaiting_binding={:?}", awaiting);
}

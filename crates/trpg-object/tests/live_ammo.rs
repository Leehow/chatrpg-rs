...
const OBJ: &str =
    "obj.session_03718b108b9b44188163752d6d788e7d.npc_opposition.cyberpunk_red_weapon_heavy_pistol";

fn attack(intent: &str) -> CheckContract {
    attack_for(SESSION, ACTOR, "cyberpunk_red", ActorKind::Npc, intent)
}
...
async fn ammo(db: &Db) -> Option<i64> {
    sqlx::query_scalar::<_, serde_json::Value>(
        "select mechanical_state from object_instances where object_id=$1",
    )
    .bind(OBJ)
...
fn object_ammo(obj: &ObjectInstance) -> Option<i64> {
    obj.mechanical_state
        .get("ammo_current")
        .and_then(|v| v.as_i64())
}

#[tokio::test]
async fn created_character_sheet_weapon_seeds_and_decrements_ammo() {
    ...
    let session_id = format!("session_test_ammo_{}", uuid::Uuid::new_v4().simple());
    let actor_id = "pc.current";
    let sheet = json!({
    ...
    let seeded = svc
        .seed_actor_inventory_from_sheet(&session_id, "call_of_cthulhu_7e", actor_id, &sheet, 0)
        .await
        .unwrap();
    println!(
        "[created-inventory] seeded: {:?}",
        seeded
            .iter()
            .map(|o| (&o.display_name, object_ammo(o)))
            .collect::<Vec<_>>()
    );
    assert_eq!(seeded.len(), 1);
    assert_eq!(seeded[0].display_name, "Colt M1911");
    assert_eq!(object_ammo(&seeded[0]), Some(7));

    let before = svc
        .find_owned_object_by_reference(&session_id, actor_id, "fire the Colt M1911")
        .await
        .unwrap()
        .expect("seeded weapon is owned");
    assert_eq!(object_ammo(&before), Some(7));
    assert_eq!(
        before
            .mechanical_state
            ...
            .and_then(|v| v.as_str()),
        Some("1d10+2"),
        "created-character M1911 must hydrate source-backed .45 Automatic damage"
    );
    let mut initiative = attack_for(
        &session_id,
        actor_id,
        "call_of_cthulhu_7e",
    ...
    initiative.check_label = "战斗顺序".into();
    initiative.action_summary = "玩家拔出 M1911 开枪打持枪手".into();
    initiative.tested_parameter = Some(TestedParameter {
        domain: None,
        key: "DEX".into(),
        label: "DEX".into(),
    });
    svc.apply_object_rule_effects(&session_id, &initiative)
        .await
        .unwrap();
    let after_initiative = svc
        .find_owned_object_by_reference(&session_id, actor_id, "fire the Colt M1911")
        .await
        .unwrap()
        .expect("seeded weapon remains owned");
    assert_eq!(
        object_ammo(&after_initiative),
        Some(7),
        "initiative/order checks must not spend ammo"
    );

    let mut shot = attack_for(
        &session_id,
        actor_id,
        "call_of_cthulhu_7e",
        ActorKind::PlayerCharacter,
    ...
    shot.check_label = "M1911 瞄准射击".into();
    shot.action_summary = "开枪打持枪手".into();
    shot.tested_parameter = Some(TestedParameter {
        domain: None,
        key: "Firearms (Handgun)".into(),
        label: "Firearms (Handgun)".into(),
    });
    svc.apply_object_rule_effects(&session_id, &shot)
        .await
        .unwrap();
    let after = svc
        .find_owned_object_by_reference(&session_id, actor_id, "fire the Colt M1911")
        ...
        .expect("seeded weapon remains owned");
    println!(
        "[created-inventory] ammo after attack: {:?}",
        object_ammo(&after)
    );
    assert_eq!(
        object_ammo(&after),
        Some(6),
        "created-character weapon ammo must decrement for firearm shots even when intent lacks literal `attack`"
    );

    let effect = attack_for(
        &session_id,
        actor_id,
        "call_of_cthulhu_7e",
        ActorKind::PlayerCharacter,
    ...
    svc.apply_object_rule_effects(&session_id, &effect)
        .await
        .unwrap();
    let after_effect = svc
        .find_owned_object_by_reference(&session_id, actor_id, "fire the Colt M1911")
        .await
        .unwrap()
        .expect("seeded weapon remains owned");
    assert_eq!(
        object_ammo(&after_effect),
        Some(6),
        "effect/damage follow-up must not consume another round"
    );
...
async fn firing_decrements_ammo_generically() {
    ...
    if ammo(&db).await.is_none() {
        eprintln!("SKIP: {OBJ} fixture not found");
        return;
    }
    ...
    svc.apply_object_rule_effects(SESSION, &attack("attack"))
        .await
        .unwrap();
    ...
    svc.apply_object_rule_effects(SESSION, &attack("effect_roll"))
        .await
        .unwrap();
...
async fn use_resolves_against_the_actors_own_object() {
    ...
    let found = svc
        .find_owned_object_by_reference(SESSION, "npc.opposition", "use the heavy pistol")
        .await
        .unwrap();
    println!(
        "[owned] 'use the heavy pistol' -> {:?}",
        found.as_ref().map(|o| &o.display_name)
    );
    assert_eq!(
        found.as_ref().map(|o| o.display_name.as_str()),
        Some("Heavy Pistol")
    );

    // An unrelated reference must NOT match (so the caller falls back to seeding).
    let none = svc
        .find_owned_object_by_reference(SESSION, "npc.opposition", "我念一段咒语")
        .await
        .unwrap();
    println!(
        "[owned] unrelated input -> {:?}",
        none.as_ref().map(|o| &o.display_name)
    );
    assert!(
        none.is_none(),
        "no owned object matches an unrelated reference"
    );
...
async fn loot_source_a_transfers_targets_owned_items() {
    ...
    let owned = svc
        .list_owned_objects(SESSION, "npc.opposition")
        .await
        .unwrap();
    println!(
        "[loot] npc.opposition owns: {:?}",
        owned.iter().map(|o| &o.display_name).collect::<Vec<_>>()
    );
    assert!(
        owned
            .iter()
            .any(|o| o.display_name.contains("Heavy Pistol")),
        "npc owns the pistol"
    );
    assert!(
        owned.iter().all(|o| !o.object_id.ends_with(".body")),
        "body container excluded from loot"
    );

    // 搜身 builds reveal + transfer patches moving each item to the looter (pc.current).
    let patches = svc
        .loot_owned_patches(SESSION, "npc.opposition", "pc.current")
        .await
        .unwrap();
    let transfers: Vec<(String, ObjectLocation)> = patches
        .iter()
        .filter_map(|p| match p {
            ObjectPatch::TransferObject { object_id, to, .. } => {
                Some((object_id.clone(), to.clone()))
            }
            _ => None,
        })
        .collect();
    println!(
        "[loot] transfers: {:?}",
        transfers.iter().map(|(id, _)| id).collect::<Vec<_>>()
    );
    assert!(
        transfers.iter().any(|(id, _)| id.contains("heavy_pistol")),
        "pistol is transferred to looter"
    );
    ...
    assert!(
        !pc_owned
            .iter()
            .any(|o| o.object_id.contains("npc_opposition")),
        "npc items are not enumerated as pc-owned"
    );
...
async fn weapon_params_come_from_source_def_not_hardcoded_table() {
    ...
    let found = svc
        .find_materialized_object_def("cyberpunk_red", "我从裤裆掏出一把改装手炮顶住他")
        .await
        .unwrap();
    println!(
        "[srcparam] '改装手炮' -> {:?}",
        found.as_ref().map(|(id, n, p)| (id, n, p.get("damage")))
    );
    let (id, name, profile) = found.expect("source-backed def matched by name");
    assert_eq!(id, def_id);
    assert_eq!(name, "改装手炮");
    assert_eq!(
        profile.get("damage").and_then(|v| v.as_str()),
        Some("5d6"),
        "params come from the SOURCE def, not weapon_definition_for"
    );

    // An unrelated item has no source def -> None (caller falls back / the GM clarifies).
    let none = svc
        .find_materialized_object_def("cyberpunk_red", "我捡起一根普通的树枝")
        .await
        .unwrap();
    println!(
        "[srcparam] unrelated item -> {:?}",
        none.as_ref().map(|(_, n, _)| n)
    );
    assert!(
        none.is_none(),
        "no fabricated params for an item the source doesn't cover"
    );

    sqlx::query("delete from object_definitions where object_def_id=$1")
        .bind(def_id)
        .execute(&db.pool)
        .await
        .unwrap();
